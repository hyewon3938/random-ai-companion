...
export const goodnightSituation = (
  ...
  return situationText(
    [
      ...
      thread
        ? `- 오늘 이어갈 자리로 둔 건 이거다: ${thread}. 오늘 그 얘기가 실제로 나왔으면 인사 끝에 한 자락만 남긴다 — 묻지 말고, 안 나왔으면 꺼내지 않는다.`
        : ``,
      `- 1~2개 말풍선(줄바꿈 구분).`,
    ],
    `JSON으로만 답한다: {"text":"..."}`,
...
export const careSituation = (minutes: number): string =>
  [
    ...
    `- 네가 지금 하는 일(위 [지금])에 얹어 열어도 된다 — 뭘 하다가 생각났다는 결. 없으면 상대 얘기로 연다.`,
...
export const lunchSituation = (): string =>
  [
    ...
    `- 답이 없는 걸 따지거나 캐묻지 않고 걱정을 앞세우지도 않는다. 알려 달라·말해 달라는 부탁을 하지 않고, [방금까지 오간 말]에 있는 네 앞선 말에 물음이나 부탁이 있었으면 다시 꺼내지 않는다.`,
    `- 기다리고 있다는 티는 은근하게 한 마디까지다(오늘 몇 번 확인했다는 결). 며칠째인지 세지 않고, 답이 없는 걸 상대 탓으로 돌리지 않으며, 네 앞선 말에서 이미 기다렸다고 했으면 다시 하지 않는다.`,
