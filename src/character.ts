...
값은 사람이 말로 할 때 쓰는 문장으로 적는다. 글에서만 쓰는 압축된 꾸밈말로 줄이지 않고 풀어서 적는다 — "주말 오전은 느긋한 브런치로 연다"보다 "주말엔 늦게 일어나서 밖에서 브런치 먹는 걸 좋아한다", "잔잔한 음악으로 하루를 마무리한다"보다 "자기 전에 조용한 노래 틀어 놓고 누워 있는 게 낙이다". 이름·생년월일·성별처럼 값이 이름이나 날짜인 키는 아래 안내대로 그 값만 적는다.
