...
// 정체성 값은 사람이 말로 할 때 쓰는 문장으로 적게 한다. 글말로 줄인 값은 캐릭터의 말과 하루
// 각본의 활동 이름에 그대로 옮겨 가기 때문이다(이슈 #443).
...
[정체성 — 아래 키를 전부 채운다]
키는 "영역/무엇" 꼴이고, 값은 한두 문장의 사실이다.
값은 사람이 말로 할 때 쓰는 문장으로 적는다. 글에서만 쓰는 압축된 꾸밈말로 줄이지 않고 풀어서 적는다 — "주말 오전은 느긋한 브런치로 연다"보다 "주말엔 늦게 일어나서 밖에서 브런치 먹는 걸 좋아한다", "잔잔한 음악으로 하루를 마무리한다"보다 "자기 전에 조용한 노래 틀어 놓고 누워 있는 게 낙이다". 말하듯 적되 문장은 "~다"로 끝나는 평서문으로 두고, 캐릭터가 상대에게 하는 말투(존댓말·반말)나 상대를 부르는 말로 쓰지 않는다. 나이·시각·기간 같은 숫자는 숫자로 적는다. 이름·생년월일·성별처럼 값이 이름이나 날짜인 키는 아래 안내대로 그 값만 적는다.
