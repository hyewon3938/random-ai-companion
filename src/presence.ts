...
// 연속된 불가 사이의 경계는 2분 전~12분 후다. 30분은 예고가 붙는 구간의 길이지 미리 알리는
// 시간이 아니다 — 앞서 알리는 폭은 10분이다(AWAY_MIN_BLOCK_MIN·AWAY_BEFORE_MIN, 설계 원본 §8).
//
// 블록당 1회, 하루 AWAY_DAILY_MAX회까지다. 돌아와서 하는 인사와 이어지는 구간 사이의 사이
// 예고는 bot.ts가 보내고 이 셈에서 뺀다(awayNoticeCountToday) — 이미 알린 자리를 잇는 말이라
// 새로 알리는 한 통이 아니다. 선톡 하루 합계에도 넣지 않는다: 캐릭터가 자리를 비우는 상황에
// 붙는 한 마디여서 먼저 거는 연락과 성격이 다르다(proactive-policy의 OFF_BUDGET, 설계 원본
// §7). 공적 불가는 끝나고 연락한다는 톤으로 쓴다.
//
...
//
// 이렇게 접은 자리는 사유와 함께 트레이스 표에 남긴다(traceAwaySkip). 예고가 안 나가는
