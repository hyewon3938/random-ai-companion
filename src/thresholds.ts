...
//
// 선톡·답장·기억·삶·새벽 정리 모듈 대부분이 이 모듈을 참조한다. 선톡 문안 호출의 출력 상한도
// 여기 한 값으로 두고 선톡을 만드는 모듈 넷(followup.ts · presence.ts · glance.ts ·
// pending-handlers.ts)이 같이 쓴다. 아직 옮기지 않은 모듈은 각자 다시 쓰는 세션에서 리터럴을
// 이 모듈 참조로 바꾼다. 스키마만 배포하는
...
/** 상대 상태 판정 호출의 출력 상한(토큰). 같은 호출이 캐릭터의 마음 키 넷(바뀜·종류·세기·이유)을
 * 늘 함께 적게 되면서 260에서 올렸다(이슈 #473). 상태 한 문장과 이유 한 줄이 둘 다 길게 와도
 * JSON 한 줄이 잘리지 않을 만큼이다. */
export const USER_STATE_MAX_TOKENS = 400;
...
/** 선톡 문안 호출의 출력 상한. 문안은 생각 과정을 켠 채로 만들고 생각에 쓴 토큰도 이 상한에
 * 들어가서, 문안 길이만 보고 300~500으로 잡으면 생각이 길어진 호출이 문안을 쓰기 전에 상한에
 * 닿는다. 정상으로 끝난 의도 선톡이 생각과 문안에 쓴 토큰이 많아야 460 정도라 두 배 넘게
 * 잡았다(이슈 #471). 실제로 쓴 만큼만 과금된다. */
export const PROACTIVE_DRAFT_MAX_TOKENS = 1000;
 ...
 * 카드가 붙을 때마다 프롬프트에 그대로 실리는 값이라 카드 하나가 길어지지 않게 함께 묶는다.
 */
export const WORK_FACT_MAX_PER_NIGHT = 3;
export const WORK_FACT_SCENE_MAX = 5;

/**
 * 프롬프트 한 번에 싣는 작품 사실 카드 수 상한(#458).
 *
 * 카드는 최근 대화·꺼낸 기억·각본 같은 곳에 제목이 나오면 붙는다. 한 대화에 작품 얘기가 여러 편
 * 오가도 카드 장면이 프롬프트를 채우지 않게 묶고, 넘치면 지금 오가는 얘기에 가까운 곳에서 찾은
 * 작품부터 남긴다(context/input.ts의 readWorkFacts).
 */
export const WORK_FACT_MAX_PER_REPLY = 3;

/**
 * 이 글자 수 이하인 작품 제목은 글에서 찾지 않고, 각본 작품 칸이나 태그와 똑같을 때만 카드를
 * 붙인다(#458). 띄어쓰기를 뺀 글자 수로 센다. 두 글자 제목은 평범한 낱말과 겹쳐서 글에서 찾으면
 * 그 작품 얘기가 아닌 말에도 카드가 붙는다.
 */
export const WORK_TITLE_SHORT_MAX = 2;
