...
//
// 지금 이 모듈을 참조하는 곳은 memory.ts뿐이다. 나머지 모듈은 각자 다시 쓰는 세션에서
// 리터럴을 이 모듈 참조로 바꾼다 — 스키마만 배포하는 회차에 실행 중인 모듈 여덟 개를
// 함께 건드리면 배포 위험만 커진다.
//
 ...
 *
 * 표에 있는 값은 진행 3 · 예정 3 · 사실 5 · 일기 2다. 주변 인물은 표가 아직 값을 정하지
 * 않아 진행 중인 일과 같은 3으로 두고, 실제 프롬프트 길이를 보면서 표와 함께 조절한다.
 */
