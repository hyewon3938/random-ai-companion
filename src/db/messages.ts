...
//
// 캐릭터 말은 종류를 가리지 않고 마지막 한 통을 쓴다 — 답장이든 선톡이든 캐릭터가 말한 뒤
// 기다린 시간이라 텀을 재는 자리로 맞고, 선톡을 빼면 짝이 어긋난다(이슈 #460). 아래 유저 말
// 조회가 last.id 뒤 첫 통을 찾으므로, 선톡을 걸러 그 앞 답장을 lastChar로 잡으면 이미 주고받은
// 옛 유저 말이 firstUser로 잡혀 몇 분짜리 텀이 나온다. 09:00 답장·09:05 유저·12:00 선톡·18:00
// 유저인 대화에서 6시간이 아니라 5분이 되는 식이다. 선톡이 잦아 텀이 자주 끊기는 것이 문제로
// 보이면 걸러내지 말고 연락 텀 문안 쪽에서 고른다(ADR-0019).
...
export const getAssistantMetaSince = (
  ...
  since: string,
  f: AssistantMetaFilter = {},
): { sent_at: string; meta_json: string | null }[] => {
  const w = assistantMetaWhere(chatId, characterId, since, f);
  return db
