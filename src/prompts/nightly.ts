...
// 만나는 자리가 아니라 얼마든지 적는다(이슈 #417).
//
// 흘릴 내 얘기는 재료 줄 문구를 그대로 옮기지 않고 말하듯 적게 하고, 상대가 모르는 인물은 나와 어떤
// 사이인지 함께 적게 한다. 문구를 그대로 읊거나 소개 없이 이름만 꺼내는 답장이 나와서다. 각자 하는
// 약속에서도 사진을 주고받는 것은 빼는데, 봇이 사진을 볼 수 없어서다. 기억한 말을 꺼내는 플러팅은
// 어떤 장면에서 떠올릴지 함께 적어 의도 선톡이 그 장면일 때만 꺼내게 한다(이슈 #438).
//
// 재료 줄 끝의 [상대가 앎]·[상대는 모름]은 기억 정리에서만 쓴다. 정체성은 사실을 적는 줄에만
// 붙고, 대하는 방식(태도·말투)과 대화 성격·그늘·성별 줄에는 붙지 않는다. 흘릴 내 얘기는 [상대는
// 모름] 줄에서만 고르게 한다(이슈 #456). 일기와 대화가 없던 날의 일기에는 표시를 떼고 넣는다 —
// 표시를 설명하는 자리가 없어 일기 문장에 그대로 섞인다.
//
// 아침 선톡 문안은 답을 요구하지 않는다. 알려 달라는 부탁으로 닫거나 어제 답이 없던 부탁을
// 다음 날 아침에 다시 꺼내는 문안이 나와서, 규칙층의 먼저 거는 말 규칙과 같은 줄을 상황 문단에도
// 적었다(이슈 #462).
//
// 파고들 것과 이어갈 자리가 아직 오지 않은 상대 일을 다루면 언제부터 꺼낼지 줄 안에 적게 한다.
// 모임 전 오후에 다녀온 일처럼 적힌 줄이 의도 선톡 후보에 올라서다(이슈 #471).
//
// 피곤한 아침의 상태 표현은 두 가지를 적어 한 말만 되풀이하지 않게 하고, 오래 조용할 때 보내는 안부
// 문안에는 근황을 묻는 말과 생각나서 연락했다는 말을 함께 예로 든다(이슈 #469).
//
// 캐릭터가 물었는데 상대가 답하지 않고 넘어간 물음은 끝나지 않은 이야기로 치지 않는다. 그 물음이
// 파고들 것이나 이어갈 자리로 다시 적혀서, 다음 날 선톡이 같은 물음을 또 했다. 그 일의 때가
// 지났으면 달라진 물음으로 적게 한다(이슈 #475).
//
...
export const knowsMarkOf = (v: UserKnows): string =>
  v === "known" ? " [상대가 앎]" : " [상대는 모름]";

// 기억 정리 말고 다른 프롬프트에 재료 줄을 넣을 때는 이 표시를 뗀다. 그 프롬프트에는 표시를
// 설명하는 자리가 없고, 캐릭터를 만들 때 character.ts가 만드는 같은 모양에도 없어서, 두면
// 일기·아크 문장에 그대로 섞인다.
export const withoutKnowsMark = (s: string): string =>
...
[나의 정체성 — 이미 아는 것. 줄 끝의 [상대가 앎]·[상대는 모름]은 그 사실을 상대가 아는지의 지금 값이다. 태도·말투와 대화 성격·그늘·성별 줄은 상대에게 흘릴 사실이 아니라서 표시가 없다]
${g.identity || "(없음)"}

[이미 아는 주변 인물 — 줄 끝 표시는 위와 같고 '나'(char) 쪽 인물에만 붙는다]
${g.people || "(없음)"}

[진행 중인 일 — 줄 끝 표시는 위와 같고 '나'(char) 쪽 일에만 붙는다]
${g.ongoing || "(없음)"}
...
- tags: ${TAG_RULE}
- user_knows: '나'(char) 쪽 기억에만 — 이 사실을 상대가 아는가. 위 재료 줄 끝의 표시가 지금 값이고, 오늘 대화에서 내가 상대에게 말한 것만 known으로 바꾼다. 상대가 이미 알던 것은 그 줄의 지금 값을 그대로 다시 적는다. 한 번 known이 된 것은 다시 unknown으로 되돌리지 않는다 — 이미 말한 일을 다음에 처음 꺼내는 것처럼 말하게 된다. 오늘 말하지 않은 일을 짐작으로 known으로 바꾸지 않는다. 정체성에서 표시가 없는 줄을 같은 키로 다시 쓸 때는 user_knows를 넣지 않는다.
...
- intent: 오늘 하루 상대와의 관계에서 하려는 것. 줄마다 60자 안 한 문장이고 없으면 null이다. 4줄이 다 없으면 intent는 null.
  ...
  · share: 오늘 흘릴 내 얘기 하나 — 정체성·진행 중인 일·주변 인물에서 줄 끝이 [상대는 모름]인 줄 가운데 고른다. 표시가 없는 정체성 줄과 [상대가 앎] 줄은 고르지 않는다. 재료 줄의 문구를 그대로 옮기지 않고 상대에게 말하듯 적는다. 상대가 모르는 인물은 나와 어떤 사이인지 함께 적고 이름만 두지 않는다. [상대는 모름]으로 적혀 있어도 오늘의 대화에서 이미 상대에게 한 얘기는 두지 않는다.
  · move: move_candidates에서 고르되 앞을 우선하고, 오늘의 대화 흐름에 맞지 않으면 다음 것. 코드로 적는다. move_note는 그 플러팅을 어떤 자리에서 어떻게 쓸지 한 마디. 상대가 전에 한 말을 꺼내는 플러팅이면 내 하루의 어떤 장면에서 그 말이 떠오를지 함께 적는다(상대가 좋아한다던 가게 앞을 지날 때 정도).
  ...
  · dig·thread가 아직 오지 않은 상대 일(오후 모임·내일 발표)을 다루면 이미 지난 일처럼 적지 않고 언제부터 꺼낼지 줄 안에 적는다. 오후 네 시 모임이면 모임이 끝났을 저녁부터처럼.
  · 네가 물었는데 상대가 답하지 않고 넘어간 물음은 끝나지 않은 이야기로 치지 않는다. dig·thread에 그 물음을 다시 적지 않고, 그 일의 때가 지나 물을 거리가 달라졌으면 달라진 물음으로 적는다.
  · 상대와는 메시지로만 이어진 사이라 실제로 만날 수 없다. 네 줄 어디에도 상대와 만나는 자리(다음에 같이 갈 곳·데이트·방문)는 적지 않고, 오늘의 대화에서 상대가 먼저 만나자고 했을 때만 그 말을 받을 자리로 적는다.
  · 각자 따로 하고 서로에게 알리기로 하는 약속은 만나는 자리가 아니니 얼마든지 적는다 — 같은 요일에 각자 서점에 가서 책 하나씩 추천하기, 각자 운동 다녀와서 서로에게 알리기처럼. 다음 기대 만들기의 자리도 이렇게 고른다.
  · 사진을 찍어 보내거나 서로 주고받는 약속은 적지 않는다. 사진은 받아도 볼 수 없다.
...
//
// 용건 없이 생각나서 보내는 말을 해도 되는지는 여기 적지 않는다 — 불변층에 실리는 관계 단계
// 블록이 정한다. 1단계는 이유 없이 생각났다는 연락을 아직 안 하는 것에 두고 2단계가 그것을
// 열므로, 상황 문단이 늘 금지하면 단계가 올라가도 아침 한 통만 1단계 결에 묶인다(이슈 #359).
  ...
  [
    ...
    ...(g.lastNight
      ? [
          ...
          `- 피곤한 아침이면 눈이 잘 안 떠진다, 아직 잠이 덜 깼다는 정도로 상태만 적고, 왜 늦게 잤는지는 이 한 통에 적지 않는다. 상대가 물으면 그때 답할 자리다.`,
    ...
    `- 답을 요구하지 않는다. 알려 달라·말해 달라는 부탁을 하지 않고, 물음을 넣었으면 그 뒤에 답을 안 해도 되는 말을 붙여 닫는다. 어제 네가 건 말에 부탁이나 물음이 있었는데 상대가 답하지 않았으면 그 말을 다시 꺼내지 않는다.`,
...
export const reconnectSituation = (g: NightlyGathered): string =>
  [
    ...
    `- "잘 지내고 있어?", "요새 많이 바쁜가", "자꾸 너 생각이 나서 연락했어" 같은, 근황을 가볍게 묻거나 생각나서 연락했다고 말하는 결. 네 근황 한 조각을 곁들여도 좋다.`,
