...
- occurred_on: 값이 가리키는 일이 있었던 날. 오늘 있었던 일이면 ${g.diaryDate}, 며칠 전 일이면 그 날을 YYYY-MM-DD로 적는다. 언제인지 대화에서 안 나오거나 평소 그렇다는 성향·계속되는 상태면 넣지 않는다 — 어림으로 채운 날짜는 비워 두는 것보다 나쁘다. 앞 값과 같은 일을 다시 적는 것이면 처음 있었던 날을 그대로 둔다.
