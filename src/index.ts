...
//
// 07~23시만 돌던 때는 자정에 시작하는 불가 블록이 예고 없이 지나갔다 — 틱이 없으니 예고도
// 안 나가고 접은 사유도 안 남아서, 유저 쪽에서는 말 없이 사라진 것이 된다(이슈 #329).
// 자는 블록은 presence가 이미 걸러내므로(day-plan의 isAwayUnavail) 하루 전체로 열어도
// 잠자는 동안 예고가 나가지는 않는다.
cron.schedule(
  "*/10 * * * *",
  () => {
...
// 슬랙 채널에 남긴 표시 수집: 10분 틱, LLM 콜 없음. 채널에 올라간 게시글 본문과 스레드 안 글에
// 단 리액션(이모지 종류는 가리지 않는다)과 스레드 답글을 읽어 쌓는다. 리액션을 뗀 것도 다시 읽어 맞춘다.
cron.schedule(
  "*/10 * * * *",
  () => {
...
// 외부 스케줄러, 05:40 폴백)가 끝난 뒤에 돌고, 한 번 올라가면 같은 주 나머지 시각은 중복 키로 건너뛴다.
// 매시 두드리는 것은 배포·재시작으로 한 시각을 놓쳐도 그날 안에 올리려는 것이다.
cron.schedule(
  "0 10-23 * * 1",
  () => {
    try {
