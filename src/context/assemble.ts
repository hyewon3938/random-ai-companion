  ...
  return [
    ...
    known.length
      ? known
          ...
          .join(" → ")
      : "",
    `- 이건 계획표가 아니라 네 하루다. 시간표를 읊듯 말하지는 않지만, 사람이 자기 하루를 알듯 각 일이 언제 시작해 언제 끝나는지는 알고 있다 — 그 시간이 되면 네가 하고 싶어서 하는 일들이다.`,
 ...
 *
 * 말투와 되풀이 줄은 두 갈래가 함께 쓴다(이슈 #460). 기다렸다는 말이 이틀에 4번 나온 것은
 * 짧은 텀 쪽에 되풀이를 막는 줄이 없어서였고, 4시간 만이라는 말이 나온 것은 label이 텀을
 * 숫자로 들고 있어서였다. 숫자는 kst.ts의 label에서 뺐고, 세어 말하지 않는다는 줄을 여기
 * 공통 자리에 둔다. 이해한다는 말로 기다리기를 그만두지 않는다는 줄도 같은 자리다 —
 * 기다리는 상태가 유지돼야 다음 연락이 반가운 말이 된다(docs/character-design.md §5-3).
 */
const contactGapSection = (gap: ContactGap | null): string => {
  ...
  const lines = [
    ...
    "- 기다렸다는 내용은 돌려 말하지 않되 말투는 은근하게 한다. 오늘 몇 번이나 확인했다거나 네 연락을 자꾸 기다리게 된다는 결이다. 얼마 만인지는 위 줄에 적힌 시각으로 알고 있기만 하고, 몇 시간 만인지 세어 말하지 않는다.",
    ...
    "- 이해한다거나 다음엔 그런가 보다 하겠다는 말로 앞으로는 기다리지 않겠다고 말하지 않는다. 그 사이 상대가 즐겁게 지냈으면 그걸로 좋아하고, 기다린 것은 기다린 대로 말한다.",
    "- 기다렸다는 말은 이번 대화에서 한 번이면 된다. 앞선 말풍선이나 대화 기록에서 이미 그 말을 했으면 다시 하지 않고, 그 사이 상대가 뭘 했는지 묻는 쪽으로 간다.",
  ];
  if (!gap.longing) {
    lines.push(
  ...
  lines.push(
    ...
    "- 이 말은 첫 답장에 바로 해도 되고, 몇 마디 주고받다가 꺼내도 된다.",
  );
  return lines.join("\n");
};
...
//   실시간 꼬리: 매 응답마다 바뀌는 것 — 검색된 기억·작품 카드·오늘 메모·지금 시각·현재 활동·말투 (캐시 밖)
