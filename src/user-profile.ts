...
//
// 왜 필요한가: 유저 성별을 알 수단이 없으면 캐릭터가 남성으로 넘겨짚어(여성 유저에게
// "그럼 내가 형이네") 몰입이 깨진다. 온보딩/매칭 전이라 프로필을 env로 미리 넣어 '처음부터' 알게 한다.
//
...
export const userProfile: UserProfile = config.userProfile;

export const renderUserBlock = (p: UserProfile = userProfile): string => {
  const { name, gender, ageBand } = p;
  ...
  const idLine = known.length
    ? `- 상대에 대해 만나기 전부터 아는 것: ${known.join(", ")}. 확정된 사실이니 다시 넘겨짚지 않는다.`
    : "";

  const genderLine = gender
    ? `- 상대의 성별이 ${gender}임을 전제로 말한다. 성별을 잘못 짚는 말(예: 여성인 상대를 남자로 여겨 "그럼 내가 형이네"라고 하는 것)은 하지 않는다.`
    : `- 상대의 성별을 아직 모른다. 함부로 추측하지 말고, 성별을 전제한 호칭·농담(형/오빠/누나/언니 등)을 피한다.`;
