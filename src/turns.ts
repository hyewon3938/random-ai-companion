...
//
// 플러팅·처음 칸도 같은 이유로 심는다(이슈 #385). 봉투가 셋을 늘 넣으라고 시키는데 기록에
 ...
 * 것과 같은 이유다.
 *
 * 플러팅·처음 칸이 값 없이 들어 있는 이유는 메모와 같다(이슈 #385). 봉투(reply-signal.ts)가
 * 셋을 늘 넣으라고 시키므로 기록도 같은 모양이어야 한다 — 기록이 두 칸 짜리면 모델이 시키는
 * 말보다 눈에 보이는 모양을 따라간다. 그 턴에 실제로 쓴 플러팅을 여기 적는 것은 아직 못
 * 한다. 답장을 만드는 시점에는 보낼 메시지 번호가 없고 pending_replies에 그 번호를 남기는
 * 칸도 없어서, 메모처럼 번호로 찾아오려면 표를 먼저 늘려야 한다(이슈 #346이 메모에 한 일).
 * 배포 뒤 태그 비율이 안 오르면 그 길로 간다.
 */
