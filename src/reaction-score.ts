...
// 목록을 만들고, 저장(applyRelationOutput)은 같은 표본을 다시 세어 트랜잭션 안에서 적는다. 어제
// 하루는 이미 지난 창이라 두 번 세도 같은 값이고, 일기가 이미 있는 날은 저장 전체를 건너뛰어 같은
// 날이 두 번 반영되지 않는다. 키가 대화방과 플러팅이라 캐릭터를 바꿔도 점수가 이어진다. 플러팅마다의
