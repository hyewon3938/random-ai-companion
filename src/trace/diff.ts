...
//
// lineDiff는 하루 고정 프롬프트 덩이처럼 줄이 여럿인 글에 쓴다. 바뀐 줄만 `[빠짐]`·`[더함]`을
// 앞에 붙여 남기고 같은 줄은 뺀다. 표시를 `- `·`+ `로 두면 정체성층처럼 `- `로 시작하는 목록을
// 비교할 때 빠진 줄이 `- - `로 보여 어느 쪽인지 헷갈린다(이슈 #347). wordDiff는 관계 항목·기억
// 값·진행 중인 일처럼 줄바꿈 없는 한 문단에 쓴다. 전문을 한 번만 적고 그 안에서 빠진 말을
// `[-…-]`, 더한 말을 `{+…+}`로 감싼다 — 이전 값 전문과 새 값 전문을 나란히 두면 어디가
// 바뀌었는지 두 문단을 눈으로 견줘야 해서다(이슈 #312). 둘 다 가장 긴 공통 부분열로 같은
// 자리를 찾고, 표가 너무 커지면 통째로 바뀐 것으로 적는다.
...
/** 줄 단위 비교. 바뀐 줄만 `[빠짐]`·`[더함]`을 앞에 붙여 적고, maxLines를 넘으면 뒤를 줄여 건수만 적는다. */
