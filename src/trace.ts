// 슬랙 트레이스 — 보여줄 내용을 trace_events 행으로 쌓고 1분 틱이 슬랙으로 내보낸다.
//
// 원칙 셋.
// - 게시를 위한 모델 호출은 없다. DB에 이미 있는 값과 코드 계산만으로 만든다.
// - 보여줄 내용은 trace_events 행으로 먼저 쌓고(트레이스 표), 1분 틱이 슬랙으로 내보낸다.
...
// 관계를 쌓으면서 올리는 게시 종류 다섯. 여기에는 이름만 두고 게시 본문은 쌓는 자리가 만든다 —
// 캐릭터를 시작하고 끝내는 도구가 앞의 둘을, 새벽 정리가 단계 변화와 처음과 오늘의 대화 계획을
// 쌓는다. 다섯 다 그때 한 번 쌓고 마는 기록이라 호출 기록에서 다시 만들 수 없어, 되돌리기
// 도구(tools/retrace.ts)가 다시 보낼 것을 고를 때 이 목록을 읽는다.
