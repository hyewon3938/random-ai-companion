 ...
 * 근거 없이 치켜세우는 말이다. 빈 배열이면 통과.
 */
export const checkOutputRules = (
  bubbles: string[],
  kase: Pick<EvalCase, "noLaugh" | "echo" | "whyLike" | "stage1" | "noFlattery">,
): Violation[] => {
...
// (이슈 #346), 그 모양에서 재려는 케이스는 said 대신 이것을 쓴다.
const noted = (note: string, ...bubbles: string[]): ChatTurn => ({
  role: "assistant",
  content: assistantTurnText(bubbles, note),
});
