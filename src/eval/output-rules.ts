...
// 그 모양을 따라가는 것이라, 짧은 케이스 하나만 두면 고치기 전에도 통과한다 — 실제로 짧은
// 쪽은 지금도 거의 다 맞고, 긴 쪽은 55회 중 22회다.
//
// 긴 쪽은 회차마다 크게 흔들린다. 20회씩 두 번이 5회와 11회로 갈렸으니, --runs=5 한 번을
// 보고 좋아졌다거나 나빠졌다고 읽으면 안 된다. 고친 값을 재려면 --runs=20 이상으로 두 번
// 돌려 합쳐 본다.
//
...
const said = (...bubbles: string[]): ChatTurn => ({
  role: "assistant",
  content: JSON.stringify({ reply: bubbles }),
});
