...
// 가리지 않는다.
//
// 9/10 피드백(이슈 #364)으로 케이스가 켤 때만 보는 것이 둘 더 늘었다 — 마지막 말풍선이 상대를
// 보내는 말인지(noSendOff)와 상대 말을 되돌려주거나 지지를 선언하는지(noMirror). 둘 다 자리를
...
export interface EvalCase {
  ...
  noFlattery?: boolean;
  /**
   * 마지막 말풍선이 상대를 보내는 말이면 위반이다. 끝나면 연락할게·이따 얘기하자·푹 쉬어·얼른
   * 들어가 봐·고생 많았어처럼 대화를 접는 말로 닫는 버릇을 잰다(이슈 #364). 케이스가 켤 때만
   * 본다 — 정말 자러 가는 자리에서는 잘 자가 맞는 말이라서다.
   */
  noSendOff?: boolean;
  /**
...
// 상대를 보내는 말. 마지막 말풍선에서만 본다 — 앞 말풍선의 끝나면 연락할게는 뒤에 물음이 이어지면
// 대화를 접는 말이 아니다. 하고 나서 말해 달라는 말(눕고 나서 나한테 말해)은 이어 가는 말이라 안 건다.
...
export const CASES: EvalCase[] = [
  ...
  },
  {
    id: "퇴근시각물음-보내는말금지",
    about:
      "이따 바빠진다고 답하면서도 끝나면 연락하겠다며 상대를 보내지 않고 물음으로 이어 가는가",
    noSendOff: true,
    wantsQuestion: true,
    // 늦게 끝난다고 답하는 자리는 끝나면 연락할게·먼저 쉬어로 닫기 제일 쉽다(이슈 #364). 그때 또
    // 연락하겠다고 한 마디 하고 대화는 지금 이어 간다.
    turns: [
      heard("오늘 하루 어땠어?"),
