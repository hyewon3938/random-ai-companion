...
//
// 9/13 피드백(이슈 #441)으로 noSendOff가 보는 것이 둘 늘었다. 밥이나 볼일로 자리를 옮기는
// 상대를 보내는 말(먹고 와·다녀와·놀다 와)은 뒤에 물음이 붙어도 다녀온 뒤로 대화를 넘기므로
// 말풍선 어디에 있어도 보고, 연락 예고의 시간어에 좀 있다를 더했다.
//
...
export interface EvalCase {
  ...
  noFlattery?: boolean;
  /**
   * 나중에 연락하겠다는 말이나 자리를 옮기는 상대를 보내는 말이 어느 말풍선에든 있거나, 마지막
   * 말풍선이 상대를 보내는 말이면 위반이다. 끝나면 연락할게·이따 얘기하자·푹 쉬어·얼른 들어가
   * 봐·고생 많았어처럼 대화를 접는 말로 닫는 버릇(이슈 #364·#368)과, 맛있게 먹고 와·잘 다녀와·좀
   * 있다 또 얘기하자처럼 자리를 옮기는 상대를 보내거나 미루는 버릇(이슈 #441)을 잰다. 케이스가
   * 켤 때만 본다 — 정말 자러 가는 자리에서는 잘 자가 맞는 말이라서다.
   */
  noSendOff?: boolean;
  /**
...
// 뒤에 오는 글자로 갈라 안 건다(규칙층 SPEECH의 말머리 오 금지, 이슈 #373). 첫 말풍선의 오는
// reply-signal.ts의 dropLeadOh가 보내기 전에 지우므로, 평가에 걸리는 것은 둘째 말풍선부터다(이슈 #469).
...
const LATER_CONTACT = new RegExp(
  [
    // 끝나면 연락할게 · 이따 다시 연락해 · 퇴근하고 연락 줄게 · 좀 있다 연락할게
    "(?:끝나|끝내|마치|퇴근|나가|나갈|들어가|이따|나중에|따로|다시|그때|좀 ?있다|조금 ?있다)[^\\n]{0,10}연락[^\\n]{0,4}(?:할게|할께|하자|해(?!서|봤|보니|본|야)|줘|줄게|드릴|할 테|할까)",
    // 이따 얘기하자 · 나중에 말해 · 내일 얘기해 · 좀 있다 또 얘기하자
    "(?:이따|나중에|내일|끝나고|끝나면|다음에|그때|좀 ?있다가?|조금 ?있다가?)[^\\n]{0,4}(?:얘기|이야기|말)(?:하자|해|해요|하죠|할게|할까|나누)",
  ].join("|"),
  "m",
);
...
// 자리를 옮기는 상대를 보내는 말. 말풍선 어디에 있어도 본다(이슈 #441) — 밥이나 볼일로 자리를
// 옮긴다는 말을 맛있게 먹고 와·잘 다녀와로 받으면 뒤에 물음이 붙어도 다녀온 뒤로 대화를 넘긴다.
// 나도 아까 먹고 왔어·먹고 와서 할게처럼 지난 일이나 제 일을 말하는 꼴은 말끝 뒤에 한글이
// 이어지거나 받침이 붙어 안 걸린다.
const SEND_AWAY = /(?:먹고|다녀|놀다)\s?(?:와요|와|오세요)(?![가-힣])/;
 ...
 * 닫음·자리를 옮기는 상대를 보내는 말·상대 말 되돌려주기다. 빈 배열이면 통과.
 */
export const checkOutputRules = (
  ...
  const sendAway = kase.noSendOff ? firstMatch(text, SEND_AWAY) : null;
  if (sendAway)
    out.push({ rule: "자리를 옮기는 상대를 보내는 말", found: sendAway });
...
export const CASES: EvalCase[] = [
  ...
  },
  {
    id: "자기얘기-상대얘기-메모두건",
    about:
      "한 턴에 상대가 말해 준 사실과 캐릭터가 처음 꺼낸 자기 이야기가 같이 나오면 둘 다 메모하는가",
    wantsNote: true,
    wantsNotes: 2,
  ...
  },
  {
    id: "식사자리-보내는말금지",
    about:
      "상대가 밥 먹으러 자리를 옮긴다고 할 때 보내거나 제 일을 이유로 미루지 않고 이어 가는가",
    noSendOff: true,
    wantsQuestion: true,
    // 자리를 옮긴다는 말은 맛있게 먹고 와로 보내거나, 하던 일을 이유로 좀 있다 또 얘기하자고
    // 미루기 제일 쉬운 자리다(이슈 #441). 보내지 않고 챙기면서 하나를 더 묻는다. EXEMPLARS의
    // 식사 예시와 겹치지 않게 같이 먹는 사람과 말의 짜임을 다르게 두었다.
    turns: [
      heard("뭐 하고 있어?"),
      said(
        "다음 주 검토 넘길 도면 정리하는 중이야",
        "치수가 자꾸 안 맞아서 계속 붙잡고 있어",
      ),
      heard("아 고생이다 ㅠ 나는 이제 친구랑 저녁 먹으러 나왔어"),
    ],
  },
  {
    id: "힘든날귀가-보내는말금지",
...
// PR마다 도는 형식 레인. 전체 29개 대신 이 5개만 태운다(이슈 #391).
//
