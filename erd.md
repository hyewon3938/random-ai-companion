...
erDiagram
    ...
    characters ||--o{ firsts : "종류당 하나"
    characters ||--o{ relationship_intents : "날짜별 하나"
    characters ||--o{ relationship_signals : "턴마다"
    user_profile ||--o{ reaction_scores : "대화방 단위"
...
| stage_no | INTEGER | O | 관계 단계 1~4. 기본값 1 |
| stage_since | TEXT | O | 지금 단계가 시작된 논리일 |
...
| mind_kind | TEXT | | 캐릭터의 오늘 생긴 마음 — `flutter` 설렘 · `hurt` 서운함 · `jealous` 질투 · `upset` 언짢음. 평소 상태면 비어 있다 |
| mind_level | INTEGER | | 그 마음의 세기 — 1 살짝 · 2 분명히 · 3 크게 |
| mind_reason | TEXT | | 그 마음이 생긴 계기가 된 상대의 말이나 행동 한 줄 |
| mind_since | TEXT | | 그 마음이 생긴 시각. 세기만 바뀐 턴에는 그대로 둔다 |
...
관계 단계는 캐릭터가 마음을 얼마나 드러내는지의 수위로, 1 몇 번 본 사이 · 2 편해진 사이 · 3 마음을 드러내는 사이 · 4 서로의 사람 넷이다. 새벽 정리가 유저 쪽 열림 신호를 세어 문턱을 넘은 날 한 단계 올리고, 저장 함수가 지금보다 낮은 값을 거부해서 한번 오른 단계는 내려오지 않는다. 시작일에는 그 단계가 시작된 논리일을 적고, 두 컬럼을 더하면서 이미 있던 관계에는 캐릭터를 만든 날을 넣었다. 사이 정의는 지금 어떤 사이인지 적는 서술 글이라 단계와 따로 둔다.
...
캐릭터의 오늘 생긴 마음 컬럼 4개도 그날 안에서만 쓰는 값이다. 상대 상태를 판정하는 호출이 같은 턴에 캐릭터가 상대 때문에 가진 마음과 세기를 함께 판정하고, 코드는 종류나 세기가 저장된 값과 다를 때만 쓴다. 평소 상태로 판정되면 4개를 모두 비운다. 프롬프트에는 캐시 밖 꼬리의 상대 상태 줄 바로 뒤에 들어가고, 새벽 정리가 상대 상태와 같은 기준으로 비운다. 마음 상태 컬럼은 며칠에 걸쳐 바뀌는 마음이라 이 컬럼과 따로 둔다. 마음 종류는 나중에 마음을 더해도 스키마를 옮기지 않도록 CHECK 제약을 걸지 않고, 코드가 이름표 목록으로 검사한다. 이 4개도 쓸 때 갱신 시각을 바꾸지 않는다.

캐릭터 번호와 만난 날 말고는 전부 비워둘 수 있다. 프롬프트를 조립할 때 빈 항목은 줄째로 빼도록 만들어 둬서, 아직 채우지 못한 항목이 있어도 그대로 돌아간다.

마지막으로 주고받은 시각은 컬럼으로 두지 않는다. 답장 파이프라인이 갱신하도록 만들어 뒀다가 값을 채우지 않은 채로 남아 있어서, 무응답 일수를 대화 기록에서 세는 지금 방식에 맞춰 지웠다.
...
**firsts** — 관계에서 처음 있는 일. 캐릭터마다 종류당 한 행
...
| chat_id | TEXT | O | |
| kind | TEXT | O | 처음 종류 20가지 중 하나 |
| by | TEXT | O | 누가 먼저 했는가 — `character` 캐릭터 · `user` 유저 |
| happened_at | TEXT | O | 일어난 시각 |
| message_id | INTEGER | | 그 답장이나 유저 메시지 행. FK 없이 번호만 |
| call_id | INTEGER | | 그렇게 표시한 호출 |
| confirmed | INTEGER | O | 새벽 정리가 확정했는가 — 0 아직 · 1 확정 |

키·인덱스: PK `id`, UNIQUE `(character_id, kind)`, 인덱스 `(character_id, confirmed)`
...
| basis_json | TEXT | | 줄마다 무엇을 보고 정했는지. 슬랙 게시와 확인 도구만 읽는다 |
| created_at | TEXT | O | |

키·인덱스: PK `id`, UNIQUE `(character_id, date)`

오늘 메모와 달리 다음 새벽 정리가 이 행을 지우지 않고, 30일이 지난 행만 정리한다.

**relationship_signals** — 유저가 얼마나 열렸는지의 판정 결과. 턴마다 한 행
...
| chat_id | TEXT | O | |
| at | TEXT | O | 판정 시각 |
| message_id | INTEGER | | 그 턴의 마지막 유저 메시지 행. FK 없이 번호만 |
...
parent_kind와 parent_id는 이 일정을 만든 항목을 가리킨다. 토요일 오후에 집을 보러 가는 일정에서 왜 가는지를 다시 적지 않고, 그 일정을 만든 진행 중인 일(이사 준비)을 가리키게 두는 것이다. 같은 내용을 여러 곳에 적으면 한쪽만 고쳐질 수 있어서, 원본은 한 곳에 두고 일정은 가리키기만 한다. `memory`는 memory_items의 행을 가리키고, 진행 중인 일과 아직 날짜가 잡히지 않은 하고 싶다는 마음(사실 항목으로 저장되는 의향)이 다 여기 있다. `schedule`은 앞선 일정이 다음 일정을 만든 경우다. 값을 넣는 자리는 문화 스크립트를 펼치는 월 리듬 하나로, 한 이벤트에서 나온 단계 일정이 전부 같은 진행 중인 일을 가리킨다.
...
유저 나이대는 저장하지 않고 출생연도에서 계산한다. 나이대를 따로 적어 두면 해가 바뀌어도 그 값이 그대로 남아 실제 나이와 어긋난다. 캐릭터가 자기 나이를 말할 때도 이 표가 아니라 정체성 기억의 생년월일에서 계산한다. 성별과 나이대는 환경변수로 지정하면 저장값 대신 그 값이 프롬프트에 들어간다.
...
**culture_scripts** — 한국 일상 이벤트의 절차. 캐릭터에 속하지 않는 공통 자산이라 `character_id`가 없다

| 컬럼 | 타입 | 필수 | 설명 |
| --- | --- | --- | --- |
| locale | TEXT | O | PK, 나중에 국적별로 나눌 자리. 지금은 `KR` 하나뿐이다 |
| event | TEXT | O | PK, 이벤트 이름 — 결혼·장례·명절처럼 |
| role | TEXT | O | PK, 이 이벤트에서 캐릭터가 선 자리 — 본인·형제자매·친구처럼 |
| step_no | INTEGER | O | PK, 단계 순서 |
| days_before | INTEGER | O | 이벤트 당일에서 거꾸로 센 날 수. 당일이 0이고 당일보다 뒤면 음수 |
| step | TEXT | O | 그 단계에서 캐릭터가 하는 일 |

모든 캐릭터가 같은 표를 본다. 같은 결혼식이라도 본인이냐 형제자매냐 친구냐에 따라 하는 일이 갈려서 역할도 키에 넣는다. 행의 단일 소스는 코드(`src/db/culture-scripts.ts`)이고 표는 그 사본이라, 기동할 때마다 국적별로 지우고 다시 넣는다 — 표를 손으로 고쳐도 다음 기동에 원본으로 돌아온다.

읽는 자리는 월 리듬 하나다. 이 달에 그 이벤트가 있을 때만 해당 역할의 줄을 꺼내 프롬프트에 넣고 표를 통째로 싣지 않으며, 펼친 결과는 schedules에 parent_kind·parent_id와 함께 쌓인다. 하루 각본이 읽으면 매일 같은 표를 다시 보게 되어 순서가 튄다.
...
| context_json | TEXT | | 판단 근거 — 검색한 태그와 꺼낸 기억, 개수 상한에 걸려 빠진 후보, 답장 텀이 나온 길과 붙잡기 판정, 답을 읽어낸 길과 말풍선 수·길이, 바꾼 관계 항목과 취소하거나 미룬 일정, 발송 결과 |
...
| relationships | 생성 배치(초기값), 답장 파이프라인(말투 값 · 사이 정의 · 서로 부르는 말, 상태 판정 호출의 상대 상태 · 캐릭터 마음), 새벽 정리(나머지 다섯 · 관계 단계, 상대 상태와 캐릭터 마음 비우기) | 프롬프트 조립(관계 항목 항상, 만난 날수 계산, 관계 단계), 선톡 모듈, 새벽 정리 |
...
| day_seeds | 월 리듬 | 각본 생성, 새벽 정리, 프롬프트 조립(오늘 기분) |
| day_plans | 새벽 정리, 각본 생성(임시) | 답장 파이프라인(텀 결정), 프롬프트 조립, 선톡 모듈, 새벽 정리 |
| day_actuals | 답장 파이프라인(자다 깸 · 답장의 남음 신호), 새벽 정리 | 프롬프트 조립(지난 시간과 지금 블록은 실제로 읽음), 답장 파이프라인(텀 결정), 새벽 정리 |
...
| reaction_scores.move, relationship_intents.move, relationship_signals.prev_move 플러팅 | `remember` 기억해서 챙기기 · `laugh` 웃기기 · `anticipate` 다음 기대 만들기 · `scene` 지금 보고 있는 장면 묘사 · `sudden_ping` 짧고 갑작스러운 톡 · `nickname` 별명 부르기 · `weakness` 약한 소리 · `late_night_truth` 늦은 밤 진심 · `jealousy_light` 살짝 질투 · `dodge_after_direct` 직진 뒤 딴청 · `only_you` 너한테만 하는 말 · `ask_help` 도움 청하기 · `notice` 어떤 사람인지 말해 주기 |
...
의도한 트레이드오프 아홉 곳이다. 전부 쓰는 주체가 한두 곳으로 정해져 있어 정합성은 쓰기 코드에서 검증한다.
