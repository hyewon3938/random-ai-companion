...
| 10 | 새 구조로 읽은 대화 확인 — 08-29~08-30에 읽기 경로를 바꾼 여덟(#92 규칙층·#117 태그 고르기·#124 태도 이동·#130 하루 경계·#138 JSON 객체·#139 일기 태그·#140 일정 태그·#144 관계 갱신)을 대화로 확인하고, 인물 데이터를 프롬프트에 얼마나 내보낼지(#102)를 여기서 정한다 | 「배포 순서」 4, 「채점표」 | Fable |
| 11 | 옛 경로 삭제 — 이슈 #156의 목록에 옛 바이블 생성 경로(createDaepyoCharacter·DAEPYO_BIBLE·DAEPYO_CAST·addCastMember), render-map.ts의 옛 형식 파싱, saveUserPreferences와 user_preferences를 더해 지운다 | 「배포 순서」 5 | Opus |
| 12 | 개편 전후 비교의 확인 열 채우기 — 채우기 전에 개편 후 열 세 줄(캐릭터 기본 설정·규칙층·캐릭터와 유저의 관계)을 지금 설계로 먼저 고친다 | 「배포 순서」 6, 「개편 전후 비교」 | Fable |
...
**10번을 여는 조건 (2026-08-30 정리)** — 읽기 경로를 바꾸는 작업이 08-30까지 이어졌다. 마지막 변경이 배포되고 그 코드로 새벽 정리가 2~3회 돈 뒤에 연다. 이슈 #145가 아직 배포 전이고, CLAUDE.md 「배포·확인 대기」의 읽기 경로 넷(#138·#139·#140·#144)도 확인이 닫혀야 한다. 그전에 열면 답이 이상하게 나와도 어느 변경 때문인지 가릴 수 없다.
