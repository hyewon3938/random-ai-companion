...
- **출처 값은 creation과 conversation으로 짓는다.** 씨앗을 뜻하던 seed는 컨디션 시드(day_seeds)와 이름이 겹쳐서 바꿨다. creation 행은 저장 함수가 수정을 거부해서, 생성 때 정한 큰 정체성이 대화로 바뀌지 않는다는 원칙이 지시문이 아니라 쓰기 규칙으로 지켜진다.
- **interest_level은 지운다.** 읽는 코드가 없다.
- **user_knows는 캐릭터 쪽 행에서만 의미가 있다.** 유저 쪽 행은 유저 자신의 이야기라 known으로 고정한다.
- **extra_json은 항목별로 키를 정해 둔다.** person은 어떤 사이·만나는 방식·사는 곳·마지막 등장 넷, ongoing은 끝나는 조건 하나, fact는 비워 둔다. 정해진 키 밖의 값은 쓰기 코드가 버려서, 새 키가 필요해지면 설계를 바꾸는 일이 된다.
