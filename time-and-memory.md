...
2026-08-27에 캐릭터 생성과 기억 구조의 방향을 바꿨다. 본문은 그 전까지의 설계 그대로 두고, 바뀐 결정은 맨 끝 [「V2 — 유저가 만드는 캐릭터와 기억 통합」](#v2--유저가-만드는-캐릭터와-기억-통합-확정-2026-08-27) 절에 모았으며, 본문과 어긋나는 자리에는 인용 표시로 알렸다. 두 서술이 다르면 V2 절이 맞다.
...
**각본 블록은 5개 필드다.** 시각(시작~끝), 활동, 답장 여건(즉답·틈틈이·불가), 활동 성격(개인·사회·공적), 출처. 여가 블록의 활동은 무엇을 보고 읽는지 제목까지 구체적으로 적는다. 항상 들어가는 취향 값에 구체 제목이 하나뿐이면 모델이 영화 이야기마다 그 제목으로 수렴해서(운영에서 같은 영화가 되풀이 관측됨), 그날의 제목은 각본 생성이 정해 준다. 성격은 지금 구현에서 옵셔널이라 비어 있으면 코드가 활동 이름에서 추측하는데, 새 구조에서는 각본 생성이 값으로 적고 추측 코드는 안전망으로만 남긴다. 출처는 이 블록이 어느 예정된 일이나 매주 루틴에서 펼쳐졌는지 가리키는 필드다. 각본은 원본이 아니라 그날치로 펼친 사본이라, 접거나 미룰 때 필요한 '유저가 아는가' 같은 판정 재료는 출처를 따라 원본에서 읽는다. 원본 규칙의 각본 쪽 답이 이것으로 정해진다.
...
**이 판정에는 모델을 한 번 부른다.** 답장 텀은 그동안 코드만으로 정했는데, 붙잡는 말인지는 내용을 읽어야 알 수 있다. 개인·사회의 답장 불가 블록에 유저 메시지가 왔을 때만 짧은 판정을 부르고, 나머지 경우는 코드가 표만 보고 정한다. 공적 일정은 돌아가지 못하므로 판정 자체를 부르지 않는다. 판정 프롬프트에는 유저의 마지막 말과 지금 하는 일 한 줄만 넣는다. 답장을 먼저 생성해 태그로 가르는 방법도 있는데, 그냥 답장이었을 때 몇 시간 뒤에 나갈 문장을 미리 써 두게 되어 취하지 않았다. 판정 문안은 하던 일을 멈춰 달라는 말만 붙잡음으로 적어 두면 지금 자느냐고 묻는 말이 아님으로 기울어서, 상대가 지금 대화를 요청하는 말인지를 묻는 것으로 바꾸고 가지 말라는 말, 곁에 있어 달라는 뜻이 담긴 말, 재촉, 답 없이 몇 분에 걸쳐 이어 보낸 짧은 말을 요청으로, 가벼운 질문과 한 번 물은 안부를 아님으로 적은 경계 예시를 두었다. 답장 규칙층이 붙잡는 말을 정의한 것과 같은 결이다. 상대가 답을 못 받은 채로 몇 통을 몇 분 동안 보냈는지도 함께 넣는다(2026-09-09, 이슈 #336).
...
**블록의 두 태그를 그대로 읽지 않는 경우가 둘이고, 둘 다 표의 다른 칸을 빌려 바로 답한다.** 유저가 붙잡아 일정을 취소·미룬 상태(그 일정이 끝날 시각까지), 자는 시간에 온 연락. 둘째가 잠 정책이다. 새벽에는 유저가 마음먹고 대화하러 오는 때가 많아서, 그 시간만큼은 자고 있어도 일어나 유저와 있기로 한다. 깨어나는 첫 한 통은 폰을 집어 드는 만큼만 두고 틈틈이·개인 칸(20초~2분 30초)에서 뽑으며, 한 번 깬 뒤로는 즉답 칸을 쓴다. 실제 발송은 여기에 앞 구간에서 도착을 기다린 20~40초가 더해진 뒤다. 이 판정이 지금은 각본 프롬프트와 텀 코드 두 곳에 흩어져 있는데 한 곳으로 모으고 프롬프트 쪽은 결과만 받아 쓴다.
...
**근황 선톡의 조건을 4시간 무응답 하나로 줄인다(2026-08-25 다시 정함).** 지금은 약 2시간 무응답에 각본이 다음 활동으로 넘어가는 지점인지까지 겹쳐 보고, 연속 무응답 두 번째에서 멈춘다. 조건이 셋이라 언제 오는 말인지 설명하기 어렵고, 2시간은 낮에 흔한 간격이라 답이 늦은 것과 대화가 끝난 것을 가르지 못한다. 유저가 4시간 답이 없을 때 지금 무엇을 하는지 한 통 보내고, 그날은 더 보내지 않는다. 네 시간은 유저의 마지막 말과 캐릭터의 마지막 말 둘 다에서 잰다(2026-09-04 더함, 이슈 #274). 자리 비움 예고와 복귀 인사가 방금 나간 위에 근황이 겹쳐 답 없는 말이 한 시간 안에 셋 쌓인 날이 있어서, 캐릭터가 무슨 말이든 했으면 상대가 그 말에 답할 네 시간을 먼저 준다. 오늘 안에 유저가 말한 적이 있어야 한다는 조건은 뺐다(2026-09-07 더함, 이슈 #314). 어제 대화가 끊긴 채 하루가 시작되면 그 조건을 영영 통과하지 못해 첫날부터 아침 한 통으로 그날 연락이 끝난다. 대신 그날 미리 만들어 둔 선톡이 아직 안 나갔으면 그 통이 먼저 나가도록 기다리고, 점심 선톡이 나간 날에는 겹쳐 보내지 않는다.
...
**자리비움 선톡은 하루 세 번까지 보낸다(2026-08-29 다시 정함).** 8/25에는 상한을 없애고 그날 답장이 안 되는 일정 수가 횟수를 정하게 두기로 했는데, 일정이 많은 날은 나갔다 온다는 말만 하루 종일 오간다. 상한을 세 번으로 두되 발송 쪽에서만 막지 않고, 하루 각본을 만들 때부터 20분 넘게 자리를 비우는 일정을 세 개까지만 넣게 한다. 각본이 먼저 지키면 상한에 걸려 말없이 사라지는 일정이 거의 생기지 않는다. 세는 것은 나가면서 알리는 한 통뿐이고, 돌아와서 하는 인사는 이미 알린 구간을 마무리하는 말이라 빼고 센다. 선톡 전체의 하루 여섯 통 상한에서 자리비움을 빼는 것은 그대로 둔다. 그 상한은 먼저 말을 거는 횟수가 과한지를 막는 장치인데, 자리비움은 일정에 들어가고 나오는 것을 알리는 말이라 성격이 다르다.
...
**바뀌는 코드는 넷이다.** 아침 선톡의 조건은 새벽 정리의 문안 준비와 dispatch, 근황 선톡과 밤 인사 선톡은 followup, 자리비움 선톡은 presence, 자리를 비우는 일정 수 상한은 하루 각본을 만드는 day-plan이다. 무응답 일수로 그날 무엇을 보낼지 정하는 판정은 침묵 백오프를 맡은 관제탑에 함수 하나로 두고, 문안을 준비할 때와 보내기 직전에 같은 함수를 부른다. 점심 선톡은 2026-09-07에 followup으로 옮겼다(이슈 #314) — 무응답 이틀째 점심에 그 시각의 각본을 보고 문안을 만든다.
...
비용은 캐시가 막는다. 아침 선톡과 안부 선톡은 새벽 정리가 준비하는데 기본 경로가 외부 스케줄러라 호출 자체가 없고, 실시간으로 부르는 셋은 유저가 네 시간 안에 말한 적이 있어야 뜬다. 캐시가 한 시간이라 미스가 날 수 있고 미스 나면 그 한 통이 비싼데, 근황과 밤 인사는 하루 한 통이고 자리비움도 하루 세 통까지라 하루에 몇 통 되지 않는다.
...
**답장 불가 구간은 예외다 (확정 2026-08-28).** 즉답과 틈틈이는 텀이 몇 분이라 미리 만들어도 상황이 그대로지만, 불가는 몇 시간 뒤에야 답이 나간다. 회의 시작에 만든 답장을 회의가 끝나고 보내면 그 사이 유저가 보낸 말은 못 읽은 채고, 방금 본 것처럼 말하는 결도 거짓이 된다. 그래서 불가 구간에 온 메시지에는 답장을 만들지 않고 깨우기 표시만 걸어 두었다가, 구간이 끝나는 시각에 깨어나 그 사이 쌓인 메시지를 한 번에 읽고 몰아 답한다. 답장을 만드는 시점이 곧 보내는 시점이라, 일정을 막 끝내고 이제 메시지를 봤다는 전제가 사실이 된다. 쌓인 말이 없어도 자리 비움을 예고하고 나간 자리면 같은 깨우기가 복귀 인사를 보내므로, 일정이 끝난 뒤를 챙기는 자리는 이 하나다.

### 캐릭터와 유저의 관계 칸에 드는 다섯 (확정 2026-08-25)

> **V2에서 대체(2026-08-27).** 관계는 relationships 테이블의 컬럼 일곱 항목으로 옮겼다. 맨 끝 V2 절 참조.
...
> **V2에서 일부 대체(2026-08-27).** 추출을 멈추는 결정은 유지하고, 선호를 캐릭터에게 비밀로 하는 방향은 버렸다. 맨 끝 V2 절 참조.
...
분모는 캐릭터가 꺼낸 횟수이고 분자는 유저의 반응이다. 저장하는 값은 관심 수준이고 많음 · 보통 · 적음 3단계면 충분하다. 처음에는 단계 이름을 먼저 물음 · 반응만 · 화제 전환이라는 행동으로 지었는데, 반응은 좋았지만 먼저 묻지는 않은 날처럼 행동끼리 엇갈리면 둘 곳이 애매해서 관심 수준으로 바꿨다(2026-08-27). 행동은 저장되는 값이 아니라 단계를 옮기는 신호다.

| 신호 | 읽는 법 |
| --- | --- |
| 유저가 나중에 자기 쪽에서 먼저 물었다 | 올리는 신호, 가장 강하다 |
| 그 자리에서 반응은 했지만 거기서 끝났다 | 유지 신호 |
| 짧게 받고 화제가 바뀌었다, 답이 오지 않았다 | 내리는 신호 |
...
| 어떻게 되면 | 어디로 |
| --- | --- |
| 유저가 자기 쪽에서 먼저 물었다 | 한 번으로 많음까지 올린다 |
...
| 추가 정보 관심도 많음 / 약속 기다림 | 항목에 딸린 나머지 필드를 묶어 추가 정보라고 부른다. 명령형으로 쌓던 것이 여기로 들어온다 |
...
**칸별 개수는 초기값을 잡아 두고 조절한다(2026-08-27).** 셋 중 앞의 둘은 규칙만으로 돌아가는데 개수 상한은 값이 있어야 돈다. 칸마다 따로 두는 이유는 한 칸이 자리를 다 차지하는 것을 막기 위해서다. 지난 일기와 알게 된 사실은 쌓이는 속도가 빨라서 상한이 하나면 그 둘이 자리를 채우고, 진행 중인 일과 예정된 일은 건수가 적어 밀린다. 초기값은 진행 중인 일 3건, 예정된 일 3건, 사실 5건, 지난 일기 2건으로 판단해 잡았다. 지금 쌓인 항목이 73건에 태그 110장이라 한 태그에 걸리는 건수가 아직 작아 어느 값이든 당장은 물리지 않고, 값은 상수 모듈에 두어 실제 프롬프트 길이를 보면서 조절한다.
...
**꺼낸 기록을 남긴다(2026-08-27 확정).** memory_items에 마지막으로 꺼낸 시각과 꺼낸 횟수 컬럼을 둔다. 태그 검색으로 골라 프롬프트에 넣을 때 코드가 갱신하고, 항상 들어가는 캐릭터 쪽 사실에는 매 턴 값이 올라 신호가 되지 않으므로 찍지 않는다. 읽는 코드가 없는 값을 두지 않는 원칙과 어긋나 보이지만, 이 둘은 판단의 결과가 아니라 지나가면 다시 만들 수 없는 원시 기록이라 예외로 두고 저장부터 시작한다. 이 기록의 쓸 곳은 흐려짐이다(방향만 확정 2026-08-27). 키가 붙는 항목이 아주 많아지면 관리 비용이 커져서 잘 안 쓰는 기억을 요약으로 응축해 흐린 기억으로 만들어야 하는데, 이 구조는 대화 원문을 쌓지 않고 키를 찾아 계속 고쳐 쓰는 방식이라 오래됐다는 것이 안 쓰인다는 뜻이 아니고, 그래서 기존 챗봇처럼 시간을 기준으로 삼을 수 없다. 기준은 꺼낸 기록으로 잡는다. 오래 안 꺼내고 덜 꺼낸 항목부터 영역 단위 요약으로 응축하고, 지우지는 않는다. 발동은 캐릭터당 항목 수가 임계를 넘을 때이고, 가족이나 오랜 지인처럼 안 꺼냈다고 잊으면 이상한 기억은 예외 목록으로 지킨다. 임계값과 세부 규칙은 항목 수가 실제로 커질 때 별도로 설계한다.
...
| 생활/식사 | 점심은 지점 근처에서 혼자 | 매일 나오는 밥 얘기를 그때마다 지어낸다 |
...
생활/식사는 운영 데이터 대조(2026-08-26)에서 추가했다. 점심을 혼자 먹는다는 같은 사실이 접힐 자리 없이 세 번 적혀 있었고, 각본이 매일 점심 블록을 만드는데 읽을 값이 없어 대화가 즉흥으로 메운 값이었다.

주변 인물은 뼈대 키가 아니라 별도 칸이지만 **생성 시 최소 3명은 채운 채로 시작한다(2026-08-27 확정).** 가족, 직장, 오래된 친구 각 하나. 비어 있으면 월 리듬이 이벤트를 친구 만남처럼 익명으로 만든다. 대화가 늘어지거나 같은 인물만 되풀이되면 그때 수를 늘린다.
...
이관하면서 새로 채워야 하는 값이 아홉이다. 성별, 형편, 생활/술, 생활/잠, 생활/식사, 연애/현재, 연애/이력, 성향/MBTI, 성향/애착유형. 지금 바이블에 자리 자체가 없어서 대화에서 나올 때마다 즉흥이던 값들이다.
...
| 상대의 지금 상태 | 답장마다 도는 판정 호출 | 매 턴 판정, 바뀔 때만 저장, 밤에 비움 | 항상 |
| 최근 대화 | 코드 | 매 턴 | 항상 |
| 지금 | 코드가 계산 | 매 턴 | 항상 |
...
> **V2에서 일부 대체(2026-08-27).** 운영과 측정 문단의 유저 선호 서술은 맨 끝 V2 절이 대신한다.
...
**1. 첫 호출이 사람과 진행 중인 일을 만든다.** 안 변하는 것 표와 변하는 것 표, 주변 인물 최소 3명, 진행 중인 일 2~3개를 한 응답으로 낸다. 나눠 부르면 이름과 고향과 직업과 주거와 통근과 가족과 인물이 서로를 못 봐서, 대구 출신인데 부모님이 서울에 있고 통근 삼십 분인데 사는 곳은 반대편인 사람이 나온다. 진행 중인 일을 같은 응답에 두는 것도 같은 이유다. 승진 심사는 직업과, 동생 결혼은 가족 구성과 맞물려야 한다. 출력 스키마가 곧 칸 목록이라 통문장을 거치지 않는다.
...
> **V2에서 대체(2026-08-27).** 캐릭터는 코드가 정하는 값 없이 유저가 직접 적은 설정으로 생성한다. 맨 끝 V2 절 참조.
...
> **V2에서 일부 대체(2026-08-27).** 주변 인물은 기억의 person 항목으로 들어가고 필드는 추가 정보와 키·태그로 옮겼다. 읽는 쪽 구분(월 리듬·프롬프트·코드)은 그대로다. 맨 끝 V2 절 참조.
...
값을 여기 모으는 이유는 두 가지다. 같은 성격의 값이 흩어져 있으면 하나를 고칠 때 나머지와 어긋나고, 근거 없이 들어간 값이 문서 안에서는 근거 있는 값처럼 읽힌다. 이 값들은 구현 때 표와 1:1로 맞춘 상수 모듈 하나로 옮긴다(2026-08-27, 「구현 계획」에 등재).
...
| 3~25분 | 자는 시간에 온 첫 연락의 답장 텀 | 구현 잔재 · 폐기 확정 · 잠 예외도 표의 칸을 쓴다(첫 연락은 틈틈이·개인, 그 뒤는 즉답) | 완료 |
...
| 12:05~12:50 | 무응답 2일째에 아침 대신 보내는 시각 범위 | 판단 · 점심을 먹기 시작할 무렵 | ○ |
| 4시간 | 근황 선톡을 보내는 무응답 임계, 자리비움 선톡을 보낼 수 있는 마지막 경계, 아침 선톡이 유저가 먼저 연락했는지 재는 창 | 판단 · 세 자리가 같은 값을 쓴다 | ○ |
...
| 시작 10분 전 ~ 3분 후 | 미리 아는 일정에 나갈 때 자리비움 선톡을 보내는 시간 창 | 판단 · 닥친 일은 시작부터 12분 안에 알리고 바로 조용, 직전 블록도 답장 불가면 2분 전 ~ 12분 후 | ○ |
| 12분 | 일정이 끝난 뒤 자리비움 선톡을 보내는 시간 창 | 폐기 · 구간 끝의 깨우기가 쌓인 메시지 몰아 답장과 복귀 인사를 한 자리에서 처리한다 | 완료 |
| 하루 3회 | 자리비움 선톡의 하루 상한 · 나갈 때 알리는 것만 세고 복귀 인사는 뺀다 | 판단 · 하루 각본을 만들 때도 같은 상한을 지킨다 | ○ |
...
| 40턴 | 프롬프트에 원문 그대로 넣는 최근 대화 턴 수 | 판단 · 세는 단위를 행에서 턴으로 옮겼다(2026-09-01). 행으로 세면 남는 대화 길이가 유저가 말을 끊어 보내는 습관에 딸려 간다 — 한 번에 길게 쓰는 사람과 짧게 여러 통 보내는 사람이 같은 40에서 서로 다른 길이를 본다. 한 사람이 연달아 보낸 말을 한 턴으로 세면 누구에게나 스무 번 왕복이 남는다. 개수 40은 옮기기 전 값을 그대로 뒀고, 실제 프롬프트 길이를 보면서 조절한다 | |
...
| 3개 | 하루 각본을 만들 때 프롬프트에 넣는 진행 중인 일의 상한 · 최근에 손댄 것부터 넣는다 | 판단 · 태그로 검색해 넣는 상한과 같은 값으로 잡았다(2026-08-29) | ○ |
| 8개 | 답장을 만들기 전 짧은 호출이 한 번에 고르는 주제 태그 수 | 판단 · 초기값으로 잡았고, 저장 항목별 상한이 뒤에서 한 번 더 줄이므로 이 값은 검색을 얼마나 넓게 시작할지만 정한다(2026-08-29) | ○ |
| 8개 | 새벽 정리가 일기 한 편에 붙이는 주제 태그 수 | 판단 · 초기값으로 잡았다. 일기는 하루 한 편씩 쌓이므로 상한이 없으면 한 편이 태그 어휘를 통째로 늘리고, 늘어난 이름은 태그 고르기 프롬프트와 글자 대조에 매일 얹힌다(2026-08-29) | ○ |
| 진행 3 · 예정 3 · 사실 5 · 일기 2 · 인물 3 | 태그로 검색한 항목을 저장 항목마다 프롬프트에 넣는 상한 | 판단 · 초기값으로 잡았고 상수 모듈에 두어 실제 프롬프트 길이를 보면서 조절한다(2026-08-27). 인물 3은 구현하며 진행 중인 일과 같은 값으로 잡았다(2026-08-28) | ○ |
| 미정 | 기억 응축을 시작하는 캐릭터당 항목 수 | 미정 · 잘 안 꺼내는 기억을 요약으로 응축하는 기준인데, 지금 73건이라 당장 필요 없어 항목 수가 실제로 커질 때 정한다(2026-08-27) | ○ |
...
- 남은 확인 하나: 우진이 지금 실제로 들고 있는 누적 항목과 대조해 뼈대에서 빠진 키가 더 있는지 본다. 절반은 2026-08-25에 했다. 씨앗 쪽은 로컬 코드로 대조가 되는데, 지금 씨앗이 못 채우는 키가 12개다. 생년월일(나이대만 있다), 성별, MBTI, 애착유형, 형편, 생활의 술과 잠과 식사, 연애의 현재와 이력, 가족 구성, 주거의 통근. 이 중 성별과 애착유형은 코드가 정하고 나머지는 이관 3번 단계에서 대화 기록에서 근거를 찾아 채운다.
- 나머지 절반은 운영 DB를 봐야 한다. `relationships.state_json`의 `char_facts`를 꺼내 뼈대 두 표의 키에 하나씩 붙여보고, 어느 키에도 안 붙는 사실이 모이는 자리가 있으면 그게 빠진 키다. 붙는 것들은 이관 4번 단계에서 접히므로 이 확인의 대상이 아니다. → 했음(2026-08-26). 누적 사실 54줄을 전부 붙여봤고 모이는 자리는 식사 하나다. 점심을 혼자 먹는다는 같은 사실이 6주 사이 세 번 다시 적혔는데, 각본이 매일 점심 블록을 만드는데 읽을 값이 없어 대화가 즉흥으로 메우고 접힐 자리 없이 되풀이되던 값이었다. **생활/식사를 변하는 것 표에 추가했다.** 씨앗이 못 채우는 자리라 위 목록에 넣었고, 대화 기록에 값이 있으므로 이관 3번 단계에서 그 근거로 채운다. 한 줄씩만 안 붙는 사실은 셋(대학 전공, 입행 면접 경험, 사무실에서 집중이 잘 되는 일 습관)인데 모이지 않아 키를 만들지 않고 대화가 짓는 정체성 키로 받는다. 유년 기억 두 줄도 안 붙지만 갱신되는 값이 아니라 쌓이는 값이라 「빈자리는 만들지 않는다」 결정을 유지한다. 나머지 44줄은 전부 기존 키나 다른 저장 항목에 붙는다. 재기록이 가장 큰 두 묶음인 영화 관람 다섯 줄과 승진 심사 다섯 줄은 각각 진행 중인 일 항목 하나로 접힌다. 줄별 판정 기록은 커밋하지 않는 쪽 문서에 있다.
...
1. 1단계의 남은 확인 하나 — 우진이 들고 있는 누적 항목과 뼈대 목록을 대조해 빠진 키를 찾는다. → 했음(2026-08-26). 결과는 위 1단계 항목에. 생활/식사 키 하나가 추가됐다.
...
**숫자 기준값은 상수 모듈 하나에 모은다(2026-08-27).** 선톡 창과 틱 간격, 답장 텀 범위, 검색 개수 상한처럼 숫자로 관리하는 값을 「고정된 기준값」 표와 1:1로 맞춘 상수 모듈로 모으고, 코드 곳곳의 리터럴을 이 모듈 참조로 바꾼다. labels.ts가 닫힌 목록의 이름을 한곳에 모은 것과 같은 방식이라 표의 값을 조정할 때 고칠 자리가 한곳이 되고, 나중에 관리 화면을 만들면 값을 DB나 설정으로 옮길 때도 이 모듈만 갈아 끼우면 된다.
...
3. 바이블에 자리가 없던 값 아홉은 대화 기록에서 근거를 찾아 채운다. 지어내면 이미 한 말과 어긋난다. 근거가 없으면 비워두고 대화에서 나올 때 쌓는다.
...
**3·4단계 실제(2026-08-28 배포)** — 8번 세션이 프롬프트 조립까지 새 구조로 함께 옮겨서, 이중 기록 기간 없이 쓰기와 읽기가 같은 배포로 넘어갔다. 되돌릴 자리는 배포 전 DB 백업과 직전 커밋 재배포다. 그래서 10번 세션은 읽기를 옮기는 자리가 아니라, 새 구조로 읽은 대화가 실제로 맞게 나오는지 확인하는 자리다.

### 작업 세션 나누기 (2026-08-26, V2 재편 2026-08-28)

구현은 세션 하나에 단계 하나로 나누고, 단계마다 이슈와 브랜치를 하나씩 둔다. 나누는 이유는 둘이다. 단계마다 쓰는 모델이 다르고, 사람이 결과를 확인하고 넘어가야 하는 자리가 중간에 있다. 1~4번은 V2 확정 전에 이미 끝난 세션이라 줄을 그대로 두고 결과는 CLAUDE.md 상태에서 본다. 5번부터는 「V2」의 「구현 순서」대로 스키마 변경과 데이터 이관을 앞에 두고 다시 짰다.

모델은 되돌리기 비용으로 가른다. 판단이 틀렸을 때 앞 작업까지 다시 해야 하는 단계는 Fable로 가고, 이 문서가 정해 놓은 것을 코드로 옮기기만 하는 단계는 Opus로 간다. 뒤쪽은 분량이 많은 대신 판단할 것이 적어서 fast 모드를 켜면 빠르다.

| 세션 | 하는 일 | 이 문서에서 볼 곳 | 모델 |
| --- | --- | --- | --- |
| 1 | 운영 DB의 누적 사실을 뼈대 키에 대조해 빠진 키 찾기 | 「진행 순서」 1단계의 남은 확인 | Fable |
| 2 | 스키마 마이그레이션, ADR 작성, 스키마만 먼저 배포 | 「스키마」, 「배포 순서」 1 | Opus |
| 3 | 이관 스크립트 작성, 사본에서 검수, 운영에 반영 | 「지금 있는 캐릭터를 옮기는 절차」, 「배포 순서」 2 | Fable |
| 4 | 새 모듈 셋과 잔손질 | 「구현 때 같이 하는 잔손질 셋」, 「답장 텀은 표 한 장에서 나온다」, modules.md의 「지금 코드에서 바뀌는 것」 | Opus |
| 5 | V2 스키마 마이그레이션 — memory_items는 세 항목과 주인의 여섯 조합에 전용 컬럼 여섯, relationships는 관계 컬럼 여덟, origin 값은 creation·conversation으로, scheduled_sends는 scheduled_messages로 개명, thresholds.ts 신설, 저장 함수는 conversation 행에만 쓰도록 수정. 스키마만 배포 | 「V2」의 「기억은 세 항목」·「관계는 컬럼 여덟 항목으로 저장한다」, erd.md의 「지금 구조에서 바뀌는 것」, modules.md의 「지금 코드에서 바뀌는 것」 | Opus |
| 6 | 캐릭터 생성을 유저 입력으로 — 입력 형식 확정, 생성 호출 두 번(정체성·주변 인물·진행 중인 일·관계 첫 값 / 삶의 흐름) 구현, 입력과 결과 원본은 genesis_json에 보관 | 「V2」의 「캐릭터는 유저가 만든다」, modules.md의 「봇 프로세스 — 온보딩 경로」 | Fable |
| 7 | V2 데이터 이관 — cast_members를 person 행으로 이동, 관계 컬럼 초기값은 새로 적는 설정에 생성 호출을 돌려 채우고 지난 일기·대화와 다른 부분은 만나 온 이야기로 흡수, 프롬프트 고정값을 대체할 키(생활/잠·생활/매주 루틴·생년월일) 채우기, 사본 검수 후 운영 반영 | 「V2」의 「구현 순서」, 「배포 순서」 2 | Fable |
| 8 | 프롬프트 조립과 새벽 정리 다시 쓰기 — 관계 항목은 relationships에서 읽어 항상 주입, 기억 정리 호출 출력에 관계 갱신 포함, 프롬프트에 남은 캐릭터 고유값 다섯 자리를 이관한 키를 읽도록 교체 | 「프롬프트에 쌓는 순서」, 「실제 한 밤을 통과시켜 보면」, 「규칙층은 하는 말로 쓴다」 | Fable |
| 9 | 쓰기를 새 칸으로 옮기고 며칠 관찰 | 「배포 순서」 3 | Opus |
| 10 | 새 구조로 읽은 대화 확인 — 확인할 목록은 CLAUDE.md 「배포·확인 대기」, 「개편 전후 비교」의 확인 방법 열, 「채점표」에서 가져온다. 운영 데이터로 보는 것(관찰 도구를 기준선과 비교, 8/30 이후 새벽 정리 결과에 채점표 1~7 적용, 인물 항목이 검색되는 빈도로 #102 결정)과 대화로만 볼 수 있는 것(채점표 8·9의 열 장면씩, 아직 한 번도 발현되지 않은 경로 다섯을 일부러 만드는 시나리오, 비교표에서 대화가 필요한 줄)으로 나눠 하고, 확인한 줄은 그 자리에서 비교표 확인 열에 적는다. 시나리오는 docs/session-10-plan.md, 이슈 #262 | 「채점표」, 「개편 전후 비교」 | Fable |
| 11 | 옛 경로 삭제 — 이슈 #156의 목록에 옛 바이블 생성 경로(createDaepyoCharacter·DAEPYO_BIBLE·DAEPYO_CAST·addCastMember), render-map.ts의 옛 형식 파싱, saveUserPreferences와 user_preferences를 더해 지운다. 10번과 독립이라 순서를 기다리지 않는다. 목록 가운데 attention_override·capture_marks는 스키마 정의에서 이미 빠져 운영 DB에 남은 표만 지우면 되고, createDaepyoCharacter는 평가 실행기(src/eval/run.ts)가 캐릭터 고정 재료로 쓰고 있어 그 자리를 먼저 바꿔야 지울 수 있다. character.ts 머리 주석의 대표 캐릭터 문장도 함께 고친다 | 「배포 순서」 5 | Opus |
| 12 | 마무리 — 10번이 채우지 못한 확인 열을 판단해 채우고, CLAUDE.md 상태 절의 재설계·V2 두 항목과 README 문구를 개편이 끝난 상태로 정리한 뒤 이슈 #13을 닫는다. 개편 후 열 세 줄은 9/4에 먼저 고쳤다(#261) | 「배포 순서」 6, 「개편 전후 비교」 | Fable |

6번을 7번보다 앞에 둔 것은 관계 컬럼 초기값을 채울 때 생성 호출의 관계 첫 값 출력을 그대로 쓰기 위해서다. 8번에 넣은 캐릭터 고유값 다섯 자리는 이슈 #38에서 정체성 키가 없어 남겨 둔 곳이다. src/context.ts SLEEP의 기상·취침 시각과 근무 중 전제, src/day-plan.ts의 근무일 시간표, 아침 선톡 앵커의 직장인 전제(src/nightly.ts·src/dispatch.ts), identityFacts의 나이대, 그리고 규칙층 예시 대사의 존댓말·반말 혼재까지 다섯이다. 앞 넷이 읽을 값은 7번이 채워 두는 생활/잠·생활/매주 루틴 키와 생년월일이다.

세션을 열 때 쓰는 프롬프트는 번호만 바꿔서 쓴다. 범위를 지정하지 않으면 문서 전체를 보고 여러 단계를 한꺼번에 손대게 된다.

```
random-ai-companion 캐릭터 시스템 개편의 N번 세션을 시작한다.
erd.md, modules.md, time-and-memory.md, CLAUDE.md를 먼저 읽고
time-and-memory.md 「작업 세션 나누기」 표의 N번 줄에 적힌 범위만 한다.
이슈 생성부터 브랜치 삭제까지는 같은 절에 적힌 순서를 따른다.
```

중간에 사람이 봐야 멈추는 자리는 셋이다. 7번은 이관 후보와 새로 적는 설정을 확인하고 넘어가야 하고, 10번은 실제로 대화해 봐야 알 수 있으며, 12번은 개편 전후를 비교하는 판단이라 대신할 수 없다. 9번과 10번 사이에는 며칠이 필요하다. 새 칸에 쌓이는 것이 맞는지 보려면 새벽 정리가 몇 번은 돌아야 한다.

**10번을 여는 조건 (2026-08-30 정리, 2026-09-04 확인)** — 읽기 경로를 바꾸는 작업이 08-30까지 이어졌다. 마지막 변경이 배포되고 그 코드로 새벽 정리가 2~3회 돈 뒤에 연다. 이슈 #145가 아직 배포 전이고, CLAUDE.md 「배포·확인 대기」의 읽기 경로 넷(#138·#139·#140·#144)도 확인이 닫혀야 한다. 그전에 열면 답이 이상하게 나와도 어느 변경 때문인지 가릴 수 없다. 세 조건은 9/4에 다 채워졌다. #145는 8/30에 배포됐고, 읽기 경로 넷은 확인이 끝나 CLAUDE.md에서 빠졌으며, 마지막 배포(9/2)의 코드로 새벽 정리가 두 번 돌았다. 다만 8/30 뒤에도 답장 경로를 고친 배포가 9/2까지 이어져서, 10번은 8/30의 여덟 목록이 아니라 CLAUDE.md 「배포·확인 대기」에 남은 목록을 본다(#261).

**이슈 생성부터 브랜치 삭제까지**

단계마다 아래 순서를 따른다.

1. 이슈 생성 — 제목은 `기억 구조 재설계 N단계: 하는 일`으로 짓고, 본문에는 표의 그 줄과 참조할 절을 옮겨 적는다.
2. 브랜치 생성 — main에서 파고 이름은 `타입/이슈번호-영문요약`으로 짓는다. 타입은 문서만 고치면 docs, 코드를 고치면 feat·fix·refactor 중에서 고른다.
3. 커밋 — Conventional Commits 한글로 쓰고 제목 끝에 `(#이슈번호)`를 붙인다.
4. PR과 머지 — 본문 끝에 `Closes #이슈번호`를 넣는다. 커밋 메시지에 적은 번호는 링크만 걸고 이슈를 닫지 않아서, 닫는 일은 PR 본문이 한다.
5. 브랜치 삭제 — `gh pr merge <번호> --merge --delete-branch`가 머지와 삭제를 함께 처리한다.

머지까지 세션 안에서 끝내도 된다고 정해 뒀으니 단계마다 다시 묻지 않는다. 다만 7·10·12번은 사람이 결과를 확인한 뒤에 PR을 올린다. 이슈 #13은 개편 전체를 묶는 이슈라서 12번 세션이 끝날 때 닫는다.

### 세션별 결과 (2026-08-26 ~ 09-04)

끝난 세션이 무엇을 남겼는지 세션마다 한 줄로 적는다. 다음 세션이 앞 세션의 결과를 그대로 이어받아야 해서, 만든 것과 남긴 확인을 함께 적는다. CLAUDE.md 상태 줄에 쌓이던 것을 2026-08-29에 여기로 옮겼다.

- **1번 세션 완료(08-26, 이슈 #14, 브랜치 docs/14-skeleton-key-audit)**: 운영 DB 누적 사실 54줄을 뼈대 키에 대조 — 빠진 키는 생활/식사 하나(혼밥 사실 3회 재기록, 각본의 점심 블록이 읽을 값 부재), 변하는 것 표에 추가. 한 줄짜리 미부착 셋(전공·입행 면접·일 습관)은 대화가 짓는 키로 수용, 유년 기억은 빈자리 없음 결정 유지. 줄별 판정은 `docs/skeleton-key-audit-2026-08-26.md`(gitignored, 실데이터 포함). 1단계 남은 확인이 닫혀 남은 설계 없음 — 다음은 2번 세션(스키마 마이그레이션, Opus)
- **2번 세션 완료(08-26, 이슈 #18, 브랜치 feat/18-schema-migration, PR #19)**: 스키마만 v1으로 옮겨 배포했다. 새 테이블 6개(memory_items·tags·areas·today_notes·day_actuals·pending_replies) 추가, 쓰던 테이블 13개의 컬럼 이름·저장 값 교체, 닫힌 목록 값은 영어 식별자 + CHECK. 테이블 정의를 `TABLES` 표 한곳에 모아 새 DB 생성과 이관이 같은 정의를 쓰고, 부팅 때 `user_version`<1 + 옛 컬럼이 있으면 트랜잭션 하나로 이관 후 1로 올린다(SQLite가 CHECK·UNIQUE를 나중에 못 붙여 테이블 재생성 방식). 판단 근거는 `docs/adr/0001-memory-schema-v1-migration.md`(docs 무시 규칙에 adr만 예외). 운영 사본 리허설 → 배포 전 DB 백업(`/home/ubuntu/db-backups/companion-2026-08-26-pre-v1.db*`) → 재빌드 배포, 운영 확인 완료(user_version 1, integrity ok, fk 0, 옛 테이블 행 수 보존, 새 테이블 비어 있음). 아직 새 테이블을 읽는 코드는 없다. 설계와 다르게 둔 것 셋: `user_profile.age_band` 유지(프롬프트가 읽는 중, 나이대→생년 계산 불가) / 새 프로필 컬럼 NULL 허용 / `schedules.origin`은 전 행 `conversation`, `user_knows`는 전 행 `unknown`으로 백필(3번 세션이 교정). `relationships.state_json`은 `legacy_state_json`으로 남겨 뒀다 — 다음은 3번 세션(데이터 이관, 사람 확인 필요)
- **3번 세션 완료(08-27, 이슈 #22, 브랜치 feat/22-data-migration, PR #23)**: v1 데이터를 운영 DB에 이관했다. 사람 확인을 마친 이관 후보(`docs/migration/candidates.json`, gitignored)를 `src/tools/migrate-v1-data.ts` apply로 적재하고, 미래 데이터(월 리듬 이벤트·컨디션 시드)는 운영 컨테이너 안 opus 1콜로 재생성(8/28~9/30, 이벤트 8·시드 34). 판단 지점 둘만 멈춰 처리: ① diff-legacy — 8/26 스냅샷 이후 운영에 쌓인 13건 중 9건 반영(기존 항목에 병합 3·신규 6), 나머지 4건은 이미 있는 항목과 open_loops 처분표가 담당해 제외 ② regen 플랜 — 제약 8개는 전부 충족했으나 추석 연휴(9/24~26)에 평일 업무 시드가 들어가 있어, 연휴 시드 3일과 가족 통화 이벤트 날짜(9/26→9/25), 시드 사유 1건을 손으로 교정한 뒤 적용. 최종 검증: memory_items 73(identity 36 / ongoing 4 / relationship 5 / user_fact 28), tags 110, areas 11, cast 13, 시드 7/1~9/30 하루 1건, 미래 일정 9건, user_version 2·integrity ok·fk 0, 봇 로그 error 패턴 0. 배포 전 백업 `/home/ubuntu/db-backups/companion-2026-08-27-pre-data.db`. open_loops 처분·리허설 기록 전문은 `docs/migration/REVIEW.md`(gitignored). 아직 새 테이블을 읽는 코드는 없다 — 다음은 4번 세션(새 모듈 셋과 잔손질, Opus)
- **4번 세션 완료(08-27, 이슈 #25, 브랜치 refactor/25-reply-timing-modules, PR #26)**: 새 모듈 넷을 만들고 답장 순서를 바꿨다. `src/labels.ts`(닫힌 목록 값 = 영어 식별자, 한국어 이름은 한곳) / `src/reply-timing.ts`(답장 텀 표 한 장 + 예외 둘, 붙잡기 판정 1콜, 35분 상한·12분 붙잡기 제한 폐기) / `src/pending.ts`(만든 답장을 행으로 남겨 정한 시각 발송, 재시작 이어받기, 추가 발화 시 폐기) / `src/memory.ts`(키로 저장·태그로 검색, 읽는 쪽 연결은 다음 세션). **답장 순서 = 텀 결정 → 생성 → 대기 → 발송** — 대기 90초 초과면 도착 예정 시각 한 줄을 코드가 실시간 꼬리에 적어 준다(RESPONSE_TIMING 상수는 삭제). 함께 정리: 아침 선톡 게이트를 논리일 05:00 → 직전 유저 발화 4시간 창 / `attention_override` 테이블·`capture.ts` 삭제 / 스키마 v3(pending_replies.kind·areas.note + plan_json 두 태그 식별자 치환). 검증은 타입체크 + 새 DB 스모크(스키마·기억 저장/검색·붙잡기 기록·꼬리 줄·대기 답장 발송/재무장/폐기·텀 표 각 칸). **남은 공백 둘**: ① 대화 중 사실 포착이 비어 있다 — `pending_replies.note_to_save` 배관만 깔고 지금은 null, 답장 프롬프트가 남길 한 줄을 만드는 건 5번 세션 ② 붙잡기 판정 콜은 로컬에 유효한 키가 없어 실제 응답 미검증(실패 시 일정 유지 폴백까지만 확인). 다음은 5번 세션(V2 스키마 마이그레이션, Opus)
- **5번 세션 완료(08-28, 이슈 #43, 브랜치 feat/43-v2-schema-migration, PR #44)**: V2 기억 구조의 스키마를 v4로 옮겨 배포했다. memory_items = 저장 항목 셋(fact·ongoing·person, 옛 identity·user_fact는 fact로 합침) × 주인 둘, extra_json 대신 항목별 전용 컬럼 여섯(person=relation·contact_mode·region·last_mentioned_at / ongoing=end_condition / 캐릭터 쪽=interest)에 CHECK, 꺼낸 기록 둘(last_retrieved_at·retrieval_count) 추가, origin=creation·conversation을 UNIQUE 키에 포함. **저장 함수는 언제나 conversation 행에만 쓴다** — 부르는 쪽이 origin을 넘길 자리를 없애고 생성 배치용 insertCreationMemory·saveCreationMemory를 따로 뒀다(정체성 불변이 프롬프트 지시문에서 쓰기 경로로 내려옴). relationships에 관계 여덟 항목 컬럼 아홉 + updated_at 추가(값 채우기는 7번 세션, 그때까지 legacy_state_json 유지). scheduled_sends → scheduled_messages 개명(db.ts·nightly.ts·analyze.ts). thresholds.ts 신설(「고정된 기준값」과 1:1). 옛 memory_items는 memory_items_legacy로 남김(73건, 7번 세션이 이관 확인 후 삭제), 저장 항목 relationship 5건과 딸린 태그 7개는 버림. 판단 근거는 `docs/adr/0005-memory-schema-v4-migration.md`. 검증 = 새 DB 스모크 61건 + 운영 사본 v2→v4 리허설 52건 + 옛 백업 v0→v1→v4 전 구간 33건 통과 → 배포 전 백업 → 재빌드 배포 → 운영 확인(user_version 4, integrity ok, fk 0, 기억 68건=fact 64·ongoing 4, 출처 conversation 45·creation 23, 태그 103, 떠 있는 태그 0, 손대지 않은 표 행 수 보존). 4번 세션이 5번 몫으로 적어 둔 「대화 중 사실 포착(pending_replies.note_to_save)」은 세션 재편으로 8번 세션(프롬프트 조립·새벽 정리 다시 쓰기)에 들어간다. **남긴 확인 둘**: SEARCH_LIMIT.person = 3은 「고정된 기준값」 표에 없는 값이라 표에 줄이 필요하다 / thresholds.ts를 참조하는 곳은 memory.ts뿐이고 나머지 모듈 리터럴은 아직 그대로다. 다음은 6번 세션(유저 입력 캐릭터 생성 두 콜, Fable)
- **6번 세션 완료(08-28, 이슈 #47, 브랜치 feat/47-user-input-character-generation, PR #48)**: 유저 입력 캐릭터 생성 두 콜을 구현했다. 입력 = 선택지 둘(성별 남·여 / 나이대 20대 초반~40대 후반 9칸 — 목록 자체가 성인 가드)과 서술형 셋(성격·관계·바라는 모습, 각 1000자 상한, 비우면 생성이 앞뒤 맞게 채움). 첫 호출 `generateGenesis`(opus 1콜, 6000토큰) = 정체성(고정 키 25 + 취미/<이름> 최소 3)·주변 인물 3~4명(가족·직장·오랜 친구 갈래 최소 한 명씩)·진행 중인 일 2~3개(끝나는 조건 별도)·관계 첫 값 여섯·첫 인사. 출력이 칸 목록과 어긋나면(`genesisProblem` — 빠진 키·겹친 키·목록 밖 키·그늘 known·키 규칙 위반 등) 문제를 붙여 한 번 재시도 후 실패는 throw. 저장 `persistGenesis`는 트랜잭션 하나 — genesis_json=`{v:2,input,output}`(옛 바이블 JSON과 v 표식으로 구분, 이 원본은 대화·새벽 정리가 읽지 않음), 기억은 origin=creation 행(fact=정체성·person=인물(전용 컬럼 4, last_mentioned_at은 null)·ongoing=진행(end_condition), interest 전부 medium, 인물 이름=태그), 관계 첫 값은 말투만 코드가 polite로 고정(결은 speech_note)하고 rapport·cautions는 비운 채 시작(db.ts `saveRelationshipFirstValues`). 두 번째 호출 = 기존 `ensureArcs` 재사용 — 인물 재료를 문자열 인자로 받게 바꾸고(character.ts `arcMaterial`이 첫 호출 출력을 문장 목록으로), 옛 바이블 경로는 nightly.ts `bibleArcMaterial`이 같은 자리를 채움(refreshArcs는 그대로 = 8번 세션 몫). 유저 프로필은 `getUserProfileFull`로 읽어 부르는 이름은 호칭·첫 인사에, 하는 일·지역은 접점 하나만("유저를 닮은 사람이 아니라 한 군데 겹치는 낯선 사람"). 실행 도구 `src/tools/create-character.ts`(7번 세션이 기존 캐릭터의 관계 첫 값을 채울 때 씀). 검증 = 타입체크 + 스크래치 DB 스모크 41건(입력·출력 검증 각 케이스, 원본 보관, 전용 컬럼·태그·영역, 관계 컬럼과 비운 두 값, creation 불변 — saveMemory가 conversation 행에만 쓰는 것, integrity·fk). **남긴 확인 둘**: 실제 생성 응답은 로컬 키 무효(401)로 미검증 — CLI가 프롬프트 조립·호출 도달까지는 확인, 운영 환경 몫(4번 세션과 같은 조건) / 생성 직후 월 리듬·오늘 각본 첫 실행은 미연결 — life-plan·day-plan이 아직 바이블을 받는 구조라 대화가 먼저 오면 만드는 lazy 경로가 안전망, 봇 /start 연결과 함께 8번 세션. 다음은 7번 세션(V2 데이터 이관, Fable, 사람 확인 후 PR)
- **7번 세션 완료(08-28, 이슈 #50, 브랜치 feat/50-v2-data-migration, PR #51)**: V2 데이터를 운영 DB에 이관했다. 도구 `src/tools/migrate-v2-data.ts` 세 명령 — genesis(생성 호출로 관계 초안만, DB 미기록)·check(후보 검증: cast_members 이름 정확 일치·키 규칙·유저 쪽 값 제약·legacy 전 행 이관 확인·재실행 방지)·apply(트랜잭션 하나), 대상 DB는 DB_PATH 환경변수 필수. 관계 초안 셋(stage·history·feelings)은 운영 컨테이너 안 genesis 호출(opus 1콜) 출력을 기본으로 실제 대화 이력과 병합해 사람 확인 후 반영 — 실제와 다른 부분은 history에 흡수, 기간 표현("한 달 반")은 낡는 문구라 stage에서 제외(관계 갱신 배선은 8번 세션 몫). 반영 = person 13건(캐릭터 쪽 8·유저 쪽 5, 전용 컬럼 4 채움) + relationships 9컬럼 + memory_items_legacy 삭제(73건 중 68건 이관 확인, relationship 5건은 관계 컬럼이 대체). keepKeys 셋(기본/생년월일·생활/잠·생활/매주 루틴)은 creation 행에 값 있음 확인 — 8번 세션이 프롬프트 고정값 다섯 자리를 이 키 읽기로 교체한다. 검증 = 운영 사본 리허설 → 배포 전 백업(`/home/ubuntu/db-backups/companion-2026-08-28-pre-v2data.db`) → apply → 운영 확인(memory_items 81=fact 64·ongoing 4·person 13, tags 133, 고아 태그 0, user_version 4, integrity ok, fk 0, 봇 로그 에러 0). `legacy_state_json`은 현 봇 코드가 읽어 유지 — 8번 세션이 읽기를 관계 컬럼으로 전환한 뒤 삭제. 다음은 8번 세션(프롬프트 조립·새벽 정리 다시 쓰기, Fable)
- **8번 세션 완료(08-28, 이슈 #52, 브랜치 feat/52-prompt-and-nightly-rewrite, PR #53, 커밋 5개)**: 프롬프트 조립·새벽 정리·선톡 문안을 전부 V2 기억·관계 구조로 다시 썼다. ① 프롬프트 조립(eb93f23) — buildSystemBlocks가 바이블 대신 memory_items(creation+conversation)·관계 8컬럼·유저 프로필을 읽는 3층으로, 규칙층은 하는 말 위주로 재작성 + [메모] 규칙 신설, opts.situation(선톡 상황 문단을 꼬리 맨 끝에)·검색 기억(searchMemories, 갱신 날짜 포함) 추가, 말투는 저장값(relationships.speech_level) 우선·currentSpeechLevel 폴백 ② 하루 각본·월 리듬(0a9ad56) — day-plan·life-plan의 바이블 의존을 정체성 키 읽기(생활/잠·생활/매주 루틴 등)로 교체 ③ 새벽 정리(9f5ad3a) — gather/apply를 V2 기준으로(기억 정리 출력=키·태그 붙은 memory_items 행 + 관계 갱신, 기존 키 목록 동봉, person은 last_mentioned_at 갱신), 아침·안부 문안 3층 전환 ④ 봇 연결(b9d879c) — /start 온보딩=유저 입력 캐릭터 생성(생성 직후 월 리듬·오늘 각본·첫 인사까지), [메모] 배관(parseReplyTags→발송 성공 시 saveTodayNote), 말투 래칫(반말 판정 시 casual 저장, 존댓말 회귀 없음), pending 대기 지연이 시프트 시계와 epoch를 섞어 항상 0이 되던 버그 수정 ⑤ 마무리(이번 커밋) — followup·presence 문안 넷(근황·굿나잇·자리비움·복귀)도 3층+상황 문단으로 전환해 **선톡 문안 여섯 곳 전부가 대화와 같은 3층을 탄다**(앞 두 층 캐시 공유, 스모크에서 답장 프롬프트와 문자열 완전 동일 확인). legacy 읽기 grep 전수 — legacy_state_json·getRelationshipState·open_loops·COMPACT 둘·speechGuard의 런타임 호출자 0, 남은 자리는 정의부(db.ts·context.ts)와 개발·이관 도구(render-map·render-user·migrate-v1/v2)뿐이라 삭제는 11번 세션 몫. 검증 = 타입체크 + 스크래치 DB 스모크 17건(3층 조립·기억 저장/검색·creation 불변·[메모] 분리와 꼬리 반영·말투 래칫=판정이 뒤집혀도 저장값 유지·문안 조립·nightly gather/apply·중복 실행 skip·integrity/fk). **남긴 확인**: LLM 실호출은 로컬 키 무효(401)라 미검증 — 조립·호출 도달까지 확인, 실응답·운영 배포는 9번 세션 몫.
- **9번 세션 완료(08-28, 이슈 #56, 브랜치 chore/56-write-switch-observe, PR #57)**: 8번 세션 코드를 운영에 배포하고 새 구조에 쌓이는 것을 볼 자리를 만들었다. ① 봇 밖에서 새벽 정리를 실행하는 스케줄러 지시서를 새 입출력 형식으로 다시 썼다 — 없어진 필드(바이블·아는 인물·누적 사실·말투)를 읽고 옛 추출 형식을 내보내도록 적혀 있어서, 그대로 두면 배포 다음 새벽 실행 결과를 저장할 수 없었다. 새 지시서는 수집 22개 필드, 추출 세 묶음(기억·관계·일정)과 키·태그 규칙, 영어 식별자 두 태그, 침묵 단계 checkin, 아침·안부 발송 창 계산을 봇 안 프롬프트와 같은 내용으로 담고, 캐릭터 설정을 문서에 적어 두던 것은 걷어냈다(수집 결과가 정체성·인물·진행 중인 일·관계를 전부 넘긴다). 이 파일은 repo 밖에 있어 커밋 대상이 아니다. ② 관찰 도구 `src/tools/check-writes.ts` — 기억(항목·주인·출처별 건수와 최근 행)·태그·오늘 메모·각본과 달라진 기록·하루 준비·관계 여덟 항목·발송 대기·대화량과 모델 사용을 한 번에 읽는다(`--days N`·`--json`). ③ 배포 = DB 백업 → 동기화 → 재빌드, 부팅 로그 정상. **실호출 첫 검증** — 3층 프롬프트 15,855자로 1회차 캐시 쓰기 15,510·2회차 캐시 읽기 15,510이 찍혀, 로컬 키 401로 6·8번 세션이 미검증으로 남겨 둔 항목이 닫혔다. PR #55의 한 글자 태그 수정도 같이 나갔다. **배포 시점 기준선**: 기억 81건(사실 64·진행 중인 일 4·주변 인물 13), 태그 133·끊긴 태그 0, 오늘 메모 0, 각본과 달라진 기록 0, 검색된 적 있는 기억 0, 태그 없는 기억 1, 관계 여덟 항목 전부 채움, 시드 남은 날 34. **며칠 관찰 대상 넷** — 오늘 메모가 쌓이는가([메모] 배관), 검색 횟수가 오르는가(태그 검색), 주변 인물의 마지막 등장이 갱신되는가(새벽 정리), 태그 없는 기억이 늘어나는가. **배포 순서 3·4단계는 합쳐졌다** — 8번 세션이 읽기까지 옮겨서 이중 기록 기간이 없다(time-and-memory.md 「배포 순서」에 기록). 다음은 10번 세션(새 구조로 읽은 대화 확인, Fable)이고, 그 사이에 새벽 정리가 몇 번은 돌아야 한다
- **9번 관찰의 중간 결과(08-30, 운영 사본 기준 08-30 00:34까지)**: 관찰 대상 넷 중 둘이 아직 0이다. 오늘 메모 0건(`today_notes`는 지우는 코드가 없어 누적인데 한 줄도 없고, 08-29 하루에만 답장이 24번 나갔다), 각본과 달라진 기록 0건. 기억 검색은 돌고 있다(83건 중 14건이 꺼내졌고 누적 24회). 주변 인물의 마지막 등장은 8/22가 최신이라 갱신 흔적이 없는데, 그날 대화에 인물 이야기가 없었을 수도 있어 단정하긴 이르다. 최신 상태는 운영에서 `src/tools/check-writes.ts`로 다시 본다
- **10번 세션의 운영 데이터 판정(09-04, 이슈 #262)**: 새 구조로 쌓인 것을 운영 DB에서 읽어 볼 것 넷을 전부 OK로 판정했다. 오늘 메모는 답장에 딸려 3건, 검색된 기억 37행 297회, 인물 마지막 등장 갱신 1건, 태그 없는 기억 1→2. 채점표의 운영 데이터 항목(A1~A4)은 닫았고 대화 시나리오(B1~B5)는 사람이 대화한 뒤 판정한다. 판정표는 docs/session-10-plan.md(gitignored)에 있다. 확인 중에 찾은 문제 넷은 따로 고쳤다 — 새벽 정리가 유저 사실의 앞 값을 지우는 것(#264, PR #268), 자리 비움 예고 문안에 자리를 비운다는 말이 빠지는 것(#265, PR #266), 새벽 정리가 이미 있는 일정을 다시 넣는 것(#267, PR #271), 발송이 몇 분 끊기면 만들어 둔 답장과 선톡을 버리는 것(#269, PR #270). 새벽 정리의 기본 경로는 봇 밖 스케줄러 지시서라 #268의 프롬프트 규칙은 그 지시서에도 넣어야 운영에 적용된다.
- **11번 세션 완료(09-04, 이슈 #156, 브랜치 chore/156-remove-legacy-paths, PR #272)**: V2 구조로 대신할 자리가 생긴 표 5개(cast_members·attention_override·capture_marks·user_preferences·memory_items_legacy)와 컬럼 3개(relationships.legacy_state_json·relationships.last_contact_at·user_profile.age_band)를 스키마 v6 마이그레이션으로 지우고, 그것을 읽고 쓰던 함수·상수 12개와 타입 3개, 개발 화면 파일 2개(render-map·render-user)를 삭제했다. age_band는 birth_year에서 계산하는 함수로, 평가 실행기의 캐릭터 재료는 모델을 부르지 않는 고정 재료(src/eval/fixture-character.ts)로 옮긴 뒤 옛 생성 경로를 지웠다. 마이그레이션은 한 트랜잭션 안에서 돌고 끝에서 외래 키 검사가 어긋나면 전체를 되돌린다. 검사 4개를 더했다(빈 DB가 v6으로 서는지, v5 DB가 값을 보존하며 올라가는지, 지운 이름이 src에 없는지, 고정 재료가 생성 검사를 통과하는지). 이관 도구 두 개(migrate-v1-data·migrate-v2-data)는 ADR 0005가 기록으로 남긴다고 정해 지우지 않았고, 지우려면 새 ADR이 필요하다. 배포는 아직이며 배포 전에 운영 DB를 백업해야 한다.
...
| 캐릭터 기본 설정 | MBTI 없이 만들어져 대화에서 굳었다 | 유저가 /start에서 적은 입력 다섯으로 모델이 정체성·주변 인물·진행 중인 일·관계 첫 값을 한 번에 짓고, 유저를 대하는 방식과 애착 성향은 정체성의 태도 영역에 생성 행으로 둬 새벽 정리가 고치지 못하게 한다(ADR 0010) | 생성 직후 태도 두 행까지 값이 다 차 있는지, 며칠 대화한 뒤에도 태도 행이 그대로인지 |
...
| 자리비움 선톡 | 하루 네 번까지 알렸고, 나갈 때와 돌아올 때를 함께 세서 실제로는 두 일정에서 상한에 닿았다 | 나갈 때 알리는 것만 세어 하루 세 번까지, 각본도 그만큼만 만든다 | 자리를 비우는 일정이 하루 세 개를 넘지 않는지 |
| 규칙층 | 52줄 중 33줄이 금지형이라 말투 지시가 묻혔다 | 상대를 어떻게 대하는가(사람 층)와 표현(말 층)으로 나눠 하는 말로 다시 쓰고, 생활 블록은 한 자리로 모았다. 캐릭터마다 다른 태도는 규칙층에 두지 않고 정체성 값으로 뺐다 | 담백하되 무심하지 않게가 실제로 지켜지는지, 말투 한 줄을 고칠 때 태도 규칙을 함께 읽지 않아도 되는지 |
| 캐릭터와 유저의 관계 | 관계 JSON 한 덩어리에 여섯 가지가 섞였고, 우리끼리 표현과 조심할 것은 자리만 있고 비어 있었다 | 일곱 항목을 컬럼으로 나눠 항상 넣고 말투 판정 값을 따로 둔다. 지금 어떤 사이·서로 부르는 말·말투 판정은 답장 자리에서 바로 갱신하고, 나머지 다섯은 새벽 정리가 다시 쓴다 | 낮에 사이가 달라진 날 저녁 답장이 바로 그 사이로 나오는지, 유저의 틀을 심판하는 답이 줄어드는지 |
...
## V2 — 유저가 만드는 캐릭터와 기억 통합 (확정 2026-08-27)

여기까지의 설계는 저장소에 v1 태그로 남기고, 방향을 바꾼 결정을 이 절에 모았다. 테이블과 컬럼의 전체 모양은 [erd.md](erd.md)가, 어느 코드가 언제 읽고 쓰는지는 [modules.md](modules.md)가 단일 소스고, 이 절은 결정과 이유만 담는다.

### 캐릭터는 유저가 만든다

랜덤 매칭을 없애고, 유저가 선택지와 서술형 입력으로 원하는 캐릭터를 적어 내면 생성 호출이 그 입력으로 한 사람을 만든다. 성격과 유저와의 관계 설정, 바라는 모습까지 자유롭게 적을 수 있다. 코드가 정하던 네 값(성별·나이 범위·MBTI·애착유형)은 유저 입력이 대신하므로 없앤다. 입력 화면은 나중에 만들고, 생성 호출이 이 입력 형식을 받는 것부터 구현한다. 유저가 적어 낸 입력과 생성 결과 원본은 characters.genesis_json에 함께 보관해서 무엇을 받아 무엇을 만들었는지 나중에 확인할 수 있다.

범위는 현대 한국에서 일상을 사는 캐릭터로 한정한다. 하루 각본과 월 리듬, 답장 텀 같은 삶의 엔진이 전부 현실의 시간표를 전제해서, 판타지나 역사물 설정을 받으면 엔진이 만드는 삶과 설정이 어긋난다.

생성 호출은 두 번으로 나눈다(확정 2026-08-27). 첫 호출이 정체성 항목 전부와 주변 인물, 진행 중인 일, 유저와의 관계 첫 값을 만들고, 두 번째 호출이 올해·이번 계절·이번 달·이번 주의 흐름을 한 줄씩 쓴다. 이모지 사용은 정체성 항목에서 뺐다(확정 2026-08-27). 캐릭터마다 값을 두지 않고 시스템 프롬프트의 공통 규칙으로 관리한다.

### 과거와 현재의 경계

유저와 캐릭터가 과거를 공유하는 설정(전 직장 동료였다, 같은 학교를 다녔다)은 자유다. 현재를 공유하는 설정(지금 같은 회사 옆자리다)은 캐릭터의 하루가 유저의 실제 하루와 매번 맞아야 해서 금세 어긋난다. 그래서 생성 호출이 현재 공유 설정을 받으면 과거형으로 바꾼 안(옆자리였는데 최근에 팀을 옮겼다)을 제시하고 유저가 확정한다. 이 규칙은 기획으로만 적어 두고 지금 구현하지는 않는다.

### 기억은 세 항목

기억(memory_items)의 저장 항목을 사실(fact)·진행 중인 일(ongoing)·주변 인물(person) 셋으로 정리하고, 각각을 캐릭터 쪽과 유저 쪽으로 나눈다. 여섯 조합이 전부 유효하고, 어느 쪽 것인지는 모델이 저장할 때 내용을 보고 정한다. 주변 인물을 따로 담던 cast_members 테이블은 person 항목이 대신한다.

세부 결정은 다섯이다.

- **출처 값은 creation과 conversation으로 짓는다.** 씨앗을 뜻하던 seed는 컨디션 시드(day_seeds)와 이름이 겹쳐서 바꿨다. 저장 함수는 origin을 확인하는 분기 없이 언제나 conversation 행에만 써서, 같은 키에 두 행이 나란히 놓이고 프롬프트에는 둘 다 들어간다(2026-08-27 확정, UNIQUE 키에 origin 포함). 생성 때 정한 큰 정체성이 대화로 바뀌지 않는다는 원칙이 지시문이 아니라 이 쓰기 규칙 하나로 지켜진다.
- **관심도는 캐릭터 쪽 세 항목 공통 컬럼으로 남긴다(2026-08-27 정정).** 읽는 코드가 없다고 지우기로 했다가, 「관심도는 꺼내기만 조절한다」의 설계가 항목마다 이 필드를 전제하고 있어 되살렸다. 진행 중인 일만이 아니라 사실과 주변 인물까지 캐릭터 쪽 행 전체가 갖고, 유저 쪽 행은 비워 둔다. 값은 3단계 영어 식별자이고 이름은 erd.md가 정한다.
- **user_knows는 캐릭터 쪽 행에서만 의미가 있다.** 유저 쪽 행은 유저 자신의 이야기라 known으로 고정한다.
- **항목별 추가 정보는 JSON 한 칸이 아니라 전용 컬럼으로 둔다(2026-08-27 확정).** person은 어떤 사이·만나는 방식·사는 곳·마지막 등장 넷, ongoing은 끝나는 조건 하나, fact는 없다. 필드가 전부 미리 정해져 있어 자유 칸이 필요 없고, 마지막 등장은 코드가 인물을 고를 때 정렬에 쓰는 값이라 컬럼이어야 하며, 목록 밖 필드를 쓰기 코드가 버리던 검사를 스키마가 대신한다. 새 필드가 필요해지면 컬럼을 더하는 설계 변경으로 다룬다.
- **감정은 기억에 저장하지 않는다.** 그날그날의 감정은 일기가 담고, 유저를 향한 지속되는 마음은 relationships의 마음 상태가 담는다.

person 행의 값은 그 사람이 요즘 어떻게 지내는지의 요약이고, 그 사람과 얽힌 일화는 이름 태그로 일기·일정과 연결된다. 같은 쪽에서 같은 이름은 같은 사람으로 보아 영역이 달라도 저장 코드가 하나로 합치고, 실제로 다른 사람이면 회사 민수처럼 이름을 늘려 가른다(동명이인 규칙 그대로). 유저 쪽 일정은 schedules의 owner=user 자리를 그대로 쓴다.

### 관계는 컬럼 일곱 항목으로 저장한다

캐릭터와 유저의 관계는 두 사람 사이의 상태라 누구 것인지 나눌 수 없고 항목도 미리 정해져 있어서, 키가 대화에서 태어나는 기억과 달리 relationships 테이블의 컬럼으로 둔다.

| 항목 | 컬럼 | 초기값 | 갱신 |
| --- | --- | --- | --- |
| 사이 정의 | stage | 생성 배치가 유저의 관계 설정에서 | 새벽 정리 |
| 지금 말투 | speech_level · speech_note | 존댓말에서 시작, 결은 생성 배치가 | 값은 답장 파이프라인, 결은 새벽 정리 |
| 서로 부르는 말 | address_terms | 유저 프로필의 부르는 이름에서 | 새벽 정리 |
| 잘 통하는 것 | rapport | 비워 두고 시작 | 새벽 정리 |
| 조심할 것 | cautions | 비워 두고 시작 | 새벽 정리 |
| 만나 온 이야기 | history | 생성 배치 | 새벽 정리 |
| 마음 상태 | feelings | 생성 배치 | 새벽 정리, 최근 일기에 뚜렷한 근거가 있을 때만 |

- 프롬프트에는 일곱 항목이 항상 전부 들어간다.
- 캐릭터가 유저를 어떻게 대하는지는 이 표가 아니라 memory_items의 `태도` 영역에 `creation` 행으로 넣는다(2026-08-29 변경). 이 표의 항목은 새벽 정리가 매일 다시 쓰기 때문에, 유저가 캐릭터를 만들며 적어 낸 태도를 여기에 두면 며칠 대화에 밀려 흐려진다. 생성 때 정한 값은 저장 함수가 고치지 않아 그 자리라야 유지된다.
- 관계 갱신은 새벽 정리에서 기억을 정리하는 호출의 출력에 같이 들어 있어서, 모델 호출 수(평소 4번)가 늘지 않는다.
- 말을 놓은 날과 먼저 꺼낸 쪽은 필드로 두지 않는다. 사이가 달라진 사건은 새벽 정리가 만나 온 이야기에 날짜와 함께 적는다.
- 변화를 날짜별로 남기는 로그 테이블은 두지 않는다. 그날그날의 기록은 일기가 담당한다.
- 만난 지 며칠인지와 연락 빈도는 만난 날과 대화 기록에서 계산되므로 저장하지 않는다.
- 관계 칸에 두기로 했던 우리끼리 생긴 표현은 잘 통하는 것이 담는다.

### 유저 선호는 세 갈래

유저의 선호를 캐릭터에게 비밀로 하는 원칙을 버린다. 유저가 좋아하는 것을 알고 맞춰 주는 것 자체는 관계에서 자연스럽고, 문제는 근거 없이 비위를 맞추는 쪽이라 아부 금지는 stance 규칙이 그대로 지킨다.

선호는 쓰임에 따라 세 갈래로 갈린다.

- **대화용.** 이 유저와 잘 통하는 것은 relationships의 잘 통하는 것에 담겨 항상 프롬프트에 들어간다.
- **측정용.** 어떤 대화가 유저를 붙드는지는 저장된 선호가 아니라 운영 로그(대화 기록·반응)를 집계해서 본다.
- **유저 단위.** 매칭이 없어지면서 매칭용 선호도 없어졌고, user_preferences는 캐릭터와 무관한 유저 자체의 선호를 담을 자리로 남긴다. 캐릭터를 여럿 두는 구조가 생겼을 때 채우고, 그때의 재료는 대화 원문과 카드 반응, 관계가 얼마나 이어졌는지의 기록이다. 읽는 코드가 없는 값을 미리 쌓지 않는다는 규칙도 그대로다.

### 유저 페르소나

유저가 자기 역할이나 설정을 만들어 그 인물로 대화하는 기능은 이번 범위에 넣지 않고, README에 확장 가능성으로만 적는다. 유저 프로필은 실제 유저의 사실을 담는 자리로 유지한다.

### 확정 뒤에 덧붙인 것 (2026-08-27 ~ 08-28)

위 절들을 확정한 뒤 세부를 채우거나 범위를 조정한 기록이다. CLAUDE.md 상태 줄에 쌓이던 것을 2026-08-29에 여기로 옮겼다.

- **확정 요약(08-27)**: 랜덤 매칭 폐지 — 유저가 선택지+서술형 입력으로 캐릭터를 만들고(UI는 나중, 생성 콜이 입력 형식을 먼저 받음), 입력·결과 원본은 characters.genesis_json에 보관, 범위는 현대 일상 캐릭터 한정(삶 엔진이 현실 시간표 전제). 기억은 memory_items 3항목(fact·ongoing·person)×owner(char·user) 여섯 조합 — cast_members는 person으로 흡수, origin은 seed·accrued→creation·conversation 개명 + creation 행은 저장 함수가 수정 거부(큰 정체성 불변을 쓰기 규칙으로), interest_level 삭제, user_knows는 캐릭터 쪽 행만 의미, extra_json은 항목별 화이트리스트(person=어떤 사이·만나는 방식·사는 곳·마지막 등장 / ongoing=끝나는 조건 / fact=비움), 감정 미저장(그날 감정=일기, 지속 마음=relationships.feelings). 관계는 relationships 컬럼 여덟 항목(stage / speech_level·speech_note / address_terms / texture / rapport / cautions / history / feelings) — 전부 항상 주입, 초기값은 생성 배치(rapport·cautions 비움), 갱신은 말투 값·last_contact_at=답장 파이프라인 / 나머지=새벽 정리(기억 정리 호출 출력에 포함, 호출 수 평소 4번 불변), 로그 테이블 없음(일기가 담당), 말 놓은 날·먼저 꺼낸 쪽 필드 폐지(사건은 history에 날짜와 함께), 우리끼리 표현은 rapport가 흡수. 유저 선호 비공개 원칙 폐기(아부 금지 stance는 유지) — 대화용=rapport / 측정용=운영 로그 집계 / user_preferences는 캐릭터와 무관한 유저 단위 선호 자리로 재정의(지금은 자리만, 채움은 다중 캐릭터 때 산 기록에서). 과거 공유 설정 자유·현재 공유 설정은 생성 콜이 과거형 변환안 제시→유저 확정(기획만, 미구현). 유저 페르소나는 README 확장 가능성으로만. 문서 반영: erd.md·modules.md·time-and-memory.md 「V2」 절·ADR 0002~0004. 다음: 설계 허브 HTML(docs/data-map.html) V2 재구성 → 구현 세션 재편(5번 세션부터 V2 스키마·이관 우선) → README는 구현 뒤 마지막.
- **V2 세부 문답 확정(08-27)**: memory_items 추가 정보는 JSON 대신 전용 컬럼 6개(relation·contact_mode·region·last_mentioned_at=person 전용 / end_condition=ongoing 전용 / interest=캐릭터 쪽 전용, 해당 없는 행은 CHECK로 거부 — plan_json 태그를 CHECK가 못 지켜 labels.ts 3중 방어가 필요했던 선례가 근거) / 관심도 값=관심 수준 high·medium·low(행동 셋은 단계를 옮기는 신호, 초기 medium, 신호 집계는 코드·단계 이동은 새벽 정리) / UNIQUE에 origin 포함, 저장 함수는 항상 conversation 행에만 upsert(creation 행은 생성 배치가 한 번 쓰고 불변, 읽을 때는 둘 다) / 주변 인물 생성 최소 3명(루즈하면 증원) / 각본 여가 블록은 제목까지 구체로 적어 항상 주입값 수렴(컨택트 반복) 차단 / 흐려짐=꺼낸 기록 기준 요약 응축, 발동 임계는 미정으로 기준값 표 등재 / 기준값 상수는 thresholds.ts로 모음(modules.md 반영됨) / user_profile 추가 필드 없음
- **허브 V2 검수·보강(08-27)**: 병렬 세션이 마친 허브 V2 재구성을 카드 아홉 장 전수 검수해 공백 두 건 반영. ① 유저의 관심 수준이 허브 전체에 부재 → 05 카드(데이터 한 건 표 값 + 추가 정보 툴팁: 많음·보통·적음, 초기 보통, 화제로 먼저 꺼내는 빈도만 조절, 관심 많다고 지어내지 않음, 유저 쪽 행 없음) / 06 카드 3번(값이 검색 데이터와 함께 프롬프트에 들어감) / 07 카드 2단계(꺼낸 횟수·답 여부·유저가 먼저 꺼냈는지는 코드가 세어 넘기고 기억 정리 호출이 단계 이동 — 먼저 물으면 한 번에 많음, 답 두 번 연속 없으면 내림, 반응만이면 유지, 실시간 판정 없음) ② 01 카드 정체성 툴팁을 묶음 13줄 → 키별 27행으로(성별 추가, MBTI·애착유형은 V2 저장 키가 아니라 제외, 직업 3키·주거 3키·말투 5키·생활 5키·연애 2키, 값은 이관 실데이터), 06 카드 1번 정체성 문단도 키 단위 서술로 동기화(13가지 카운트 폐기). 잔여 불일치 1건은 보고만: 01 카드 범례 '호출 두 번' vs modules.md character.ts '1번' — 생성 호출 수는 설계 결정 필요
- **생성 호출 수·이모지 확정(08-27)**: 생성 호출은 두 번(첫 호출=정체성·주변 인물·진행 중인 일·관계 첫 값 / 두 번째=삶의 흐름) — 카드 01 범례가 맞고 modules.md character.ts를 2번으로 정정. 이모지 사용은 정체성 항목에서 제외하고 시스템 프롬프트 공통 규칙으로 관리 — 카드 01 정체성 툴팁 27→26행, 두 결정은 time-and-memory.md 「V2」 절에 기록(v1 절·뼈대 표는 기록 보존 원칙대로 미수정).
- **테이블 이름·규칙층 정리(08-27, 이슈 #34·#36)**: `scheduled_sends` → **`scheduled_messages`**(담기는 건 발송 기록이 아니라 미리 만든 선톡 문안 + `pending_replies`와 짝) — erd.md·modules.md는 새 이름, 코드 10곳(src/db.ts 9·src/nightly.ts 1)은 마이그레이션이 필요해 V2 스키마 세션에서 함께 개명. erd.md 기억 예시 표에 origin 열 추가(같은 키에 creation·conversation 두 행이 나란히 놓이는 예시). 공통 규칙층에서 성격이 정할 항목 6줄 삭제(자기 개방 정도·미러링·위트·담백함·취향 꺼내기), 정체성 일관성 2줄 병합, FACT_CARE 중복 조항 제거, 물음표 규칙을 RULES 2줄 → OUTPUT_FORMAT 1줄로 옮겨 압축 + OUTPUT_FORMAT_COMPACT에도 주입(선톡 문안에는 물음표 규칙이 없었음). 물음표 규칙 자체는 유지 — `fixQuestion`이 의문 전용 어미·끝머리 의문사만 보수적으로 잡아서 "밥 먹었어" 같은 흔한 반말 질문을 못 붙인다.
- **구현 세션 재편 완료(08-28, 이슈 #41, PR #42)**: time-and-memory.md 「작업 세션 나누기」 표를 V2 기준 12세션으로 다시 짰다(끝난 1~4번 줄은 그대로). 5=V2 스키마 마이그레이션(Opus) / 6=유저 입력 캐릭터 생성 두 콜(Fable) / 7=V2 데이터 이관(Fable) / 8=프롬프트 조립·새벽 정리 다시 쓰기 + 이슈 #38 잔여 고유값 다섯 자리(Fable) / 9=쓰기 전환 후 며칠 관찰(Opus) / 10=읽기 전환(Fable) / 11=옛 경로 삭제(Opus) / 12=개편 전후 비교 확인 열 채우기(Fable). 6번을 7번 앞에 둔 이유 = 관계 컬럼 초기값을 생성 호출의 관계 첫 값 출력으로 채우기 위해. 사람 확인 후 PR = 7·10·12번, 이슈 #13은 12번 세션이 끝날 때 닫는다
- **프롬프트 캐릭터 고유값 제거(08-28, 이슈 #38, PR #39)**: 프롬프트 문자열에 대표 캐릭터의 직업·지명·취미·인물 이름이 예시로 박혀 있어 다른 캐릭터를 만들어도 생성이 같은 삶으로 끌려가던 것을 걷어냈다. 인물 데이터가 이미 같은 프롬프트에 들어가 있어 예시를 지워도 참고할 정보가 줄지 않는 자리만 골랐다 — nightly 추출(캐릭터 실명·직업·인물 이름 예시 삭제 + 일정 날짜가 직업 상식과 맞는지 보는 규칙이 예시에만 기대고 있어서 인물의 직업을 프롬프트 머리에 주입, 이 규칙이 처음으로 판정할 근거를 갖게 됨) / life-plan 월 리듬(특정 지역·사건 이름 → 인물 데이터 참조) / day-plan 각본(직무·출력 형식 예시의 활동 이름을 일반 활동으로 + 실제 활동은 인물의 직업·생활·취향에서 뽑으라는 지시 추가, 주말·저녁 여가 예시도 취향 참조로) / context(호칭 예시, 자리 비움·붙잡기 예시 대사, 일찍 자는 사유 예시 삭제 — 예시 대사 자리는 어떤 결로 말하라는 서술로 대체) / character 생성 시스템 프롬프트(나이대 고정 삭제). **손대지 않은 자리 넷**은 값을 담을 정체성 키가 아직 없어 지금 지우면 생성이 매일 다른 값을 지어낸다: context.ts SLEEP의 기상·취침 시각과 근무 중 전제 / day-plan.ts 근무일 각본 시간표(자차 출근 포함) / 아침 선톡 앵커의 직장인 전제(nightly.ts 문안·앵커 탐색 정규식, dispatch.ts) / identityFacts의 age_band — 앞 셋은 생활/잠·생활/매주 루틴 키, 마지막은 생년월일을 읽도록 바꾸는 세션에서 함께 처리한다. 규칙층 예시 대사의 존댓말·반말 혼재(PRESENCE_NARRATION·CATEGORY_RULE vs DEVOTION)도 같은 세션 몫. src/tools/render-map·render-user의 화면 문구에도 캐릭터 이름이 박혀 있으나 프롬프트가 아닌 개발 도구라 범위 밖

### 구현 순서

「작업 세션 나누기」 표는 V2 기준으로 다시 짰다(2026-08-28). 5번부터 V2 스키마 변경·캐릭터 생성·데이터 이관이 앞에 오고, 프롬프트 조립과 새벽 정리 다시 쓰기는 그 뒤다. 운영 중인 캐릭터는 관계 컬럼의 초기값을 새로 정한 설정으로 채울 수 있고, 지난 일기·대화와 차이가 나는 부분은 만나 온 이야기에 사이가 변해 온 과정으로 서술해 흡수한다.

## 날짜별 설계 기록 (2026-08-19 ~ 08-26)

설계를 확정해 온 순서를 날짜별로 남긴다. 확정한 내용 자체는 위의 각 절이 원본이고, 여기는 무엇을 언제 어떤 이유로 바꿨는지를 되짚을 때 본다. CLAUDE.md 상태 줄에 쌓이던 것을 2026-08-29에 여기로 옮겼다.

- **착수(08-19)**: 프롬프트 조립과 기억의 write/read/update 정책을 다시 설계한다. 설계 원본은 repo 루트 `time-and-memory.md` — 키·태그 체계, 13칸 저장 구조와 칸별 주인, 저장 세 묶음, 프롬프트 쌓는 순서(캐시 경계 둘), 세 층 생성(타고나는 성향은 케미 축에서 도출해 1층), 유저 기본 프로필 다섯까지 확정. 리드미용 데이터 지도 도식은 `docs/readme-data-map` 브랜치(assets/data-map.svg·html, 미푸시).
- **설계 마감(08-21)**: 남은 설계 셋 확정 — 생성 순서(코드 재료 → 사람 한 콜 → 진행 중인 일·예정된 일 → 아크 → 월 리듬 → 각본. 순서가 "아크는 결만"을 강제한다) / 주변 인물 필드(월 리듬용 갈래·만나는 결·어디 + 프롬프트용 관계·지금·유저가 아는가 + 코드용 누구 쪽·마지막 등장, 이 칸은 이름 하나가 키인 예외) / 바이블 내려보내기는 절차를 만드는 대신 바이블 자체를 없앰(생성 출력 스키마 = 칸 목록). 구현 계획(스키마·이관·배포 5단계 이중 기록)까지 time-and-memory.md에 기록.
- **순서 재조정(08-21 밤)**: 바로 이관하지 않고 2단계(하루 각본·오늘 칸 / 월 리듬·시드 / 판정 한곳=답장 텀·잠·조정·선제 문안 / 우리 사이 속·유저 선호 처분)를 마저 설계한 뒤 한 번에 구현·이관 — 밤 정리·프롬프트 조립을 두 번 갈아엎지 않기 위해. 그 뒤 배포·실사용 → 채점 하네스. 칸 사이 연결은 태그가 담당(연결 테이블 없음, 이름=태그)(`src/tools/eval.ts`, 채점표는 time-and-memory.md 「채점표」 절). 현 구조는 그대로 두고 병행 개발. 확인된 개선 대상 3종: 규칙층이 금지형 위주라 말투 지시가 희석됨 / 접은 일정이 각본에 남아 실제로 한 일처럼 주입됨(`attention_override`가 `until_ts`만 보관) / 경과 시간 줄이 실시간 꼬리에 없어 모델이 추측함.
- **2단계 ① 확정(08-21 밤)**: 작업 순서는 사용자 결정(월 리듬·컨디션 → 하루 각본·오늘 → 답장 텀 → 우리 사이·유저 선호). 월 리듬은 저장소 없음(산출 셋=이벤트→예정된 일 / 시드 / 의향 착수 판정→진행 중인 일, 한 콜) / 예정된 일에 필드 5 추가(누구와=이름 태그, 영역, 유저가 아는가, 출처=대화·리듬·진행 중인 일, 부모=맥락 사슬) — 두 자리 키 두 번째 예외 / 시드 4값 유지 / 아크 갱신 재료에 진행 중인 일·예정된 일 추가, 같은 밤엔 아크 먼저. 동명이인은 같은 쪽 안에서만 문제 — 이름 늘려 가르고(회사 민수) 태그는 둘 다 유지. 이관 8단계 추가: 기억은 옮기고 삶 재료(미래 시드·유저 모르는 리듬 이벤트)는 재생성, 산 기록은 불변.
- **2단계 ② 확정(08-24)**: 하루 각본·오늘 칸 모양 — 오늘 칸=계획(각본)·실제·오늘 메모 셋 / 각본 블록 5필드(시각·활동·답장 여건·활동 성격은 생성이 값으로 적음·출처=예정된 일/루틴 가리킴 — 원본 규칙의 각본 쪽 답) / 생성 재료 6종(시드·어제 일기·예정된 일·매주 루틴·진행 중인 일·아크) / 실제는 어긋난 블록만 3필드(하려던 것·어떻게 됐나·왜), 접기·미루기 결정은 즉시 실제에 기록(until_ts만 남기던 attention_override 대체), 대화로 드러난 어긋남은 오늘 메모 경유. 설계 허브는 `docs/data-map.html`(gitignored, 08-25 스크래치패드에서 이동) — 예시가 대표 캐릭터 실데이터라 로컬 전용, 공개·커밋 시 예시 교체 필수.
- **개편 전후 비교 항목 신설(08-25)**: 문서 본문에서 "지금은 이런데 새 구조에서는" 식 서술을 걷어내고 time-and-memory.md 「개편 전후 비교」 절에 표로 모았다(정체성 저장·활동 성격 태그·취소한 일정·경과 시간·진행 중인 일·아크·예정된 일 필드·즉시 기억·유저 프로필 아홉 줄). 배포 순서에 6단계를 추가 — 5단계에서 옛 경로를 지우면 비교 대상이 사라지므로, 며칠 써 본 뒤 확인 열을 반드시 채운다.
- **2단계 ③ 일부 확정(08-25)**: 답장 텀 파생 표를 설계로 명문화 — 답이 나가기까지 = 유저 말이 다 도착할 때까지 기다리기(20~40초, 유저별 학습, **답장 텀에 포함하지 않음**) + 답장 텀(두 태그 표: 즉답 0~2분 짧은 쪽 몰림·0초 허용 / 틈틈이 개인 20s~2.5m·사회 30s~4m·공적 1~8m / 불가 일정이 끝날 때, 개인·사회 일정은 시작 10분 안 예외). **구현에 있던 35분 상한은 폐기** — 세 시간짜리 회의면 세 시간 뒤가 맞고, 대신 대기 중인 답장을 저장해 재시작 뒤에도 이어가게 만든다(현 복구는 최근 3h만 봄). 표를 따르지 않는 예외 둘(붙잡아 취소·미룬 상태 / 자는 시간에 온 연락) = 답장 텀 0. 잠 정책 단일 판정(새벽엔 깨워도 일어나 유저와 있는다), 붙잡기 판정(개인 취소·사회는 만나기로 한 상대에게 양해·공적 불가)과 실제 기록까지 같은 절에. **가운데 태그 이름 짬짬이 → 틈틈이**(문서 전부 반영, 코드 문자열 19곳은 구현 때 함께).
- **2단계 ③ 마감(08-25)**: 선제 발화 여섯 곳(아침·재연결·근황·굿나잇·예고·복귀)도 대화와 같은 3층을 쌓고 맨 끝 상황 문단·출력 형식만 다르게 — 앞 두 층이 같아야 캐시가 붙는다. 손으로 잇던 셋(정체성 JSON 주입·SPEECH_TEXTURE_COMPACT·미리 계산한 openLoops) 폐기. 규칙층은 52줄 중 33줄이 금지형이라 말투 지시가 묻힘 → 하는 말로 다시 쓰고 금지는 재발 근거 있는 것만 남긴다(줄 추가 시 근거 대화 명시 의무). RESPONSE_TIMING 5줄 삭제(코드가 태그 값을 안 쓴 지 오래 + 텀은 표가 정함) — 대신 이 답장이 언제쯤 나가는지를 코드가 꼬리에 적어준다. 설계 허브에 04 답장 텀 카드 추가.
- **2단계 ④ 확정(08-25, 2단계 전체 마감)**: 우리 사이 칸 = 서로 부르는 말 / 지금 말투(존댓말·반말을 값으로 저장)와 말 놓은 날·먼저 꺼낸 쪽 / 관계의 결(라벨 아닌 짧은 서술) / 우리끼리 생긴 표현 / 조심할 것 다섯. 뒤 둘은 RelationshipState에 필드·프롬프트 자리가 있는데 밤 정리 추출 스키마에 없어 **한 번도 채워진 적 없음** → 이관 때 추출 출력에 추가하고 지난 대화에서 초기값 채운다(태도 규칙 1번이 재료 없이 돌던 상태). 만난 날수·연락 빈도는 계산값이라 미저장. **유저 선호 추출은 중단**(읽는 코드 없음 + 케미 축→MBTI로 축이 바뀌어 지금 값은 재사용 불가), 테이블·기존 값은 보존하고 매칭은 나중에 산 기록(대화 원문·카드 반응·관계 지속)에서 다시 뽑는다.
- **설계 허브 여덟 카드 완성(08-25)**: 05 먼저 말 걸기(여섯 종류·침묵 백오프·발송 직전 재확인) / 06 프롬프트 쌓는 순서(네 층·재사용 표시 둘·꺼내는 양 셋) / 07 새벽 정리(재료→칸 이동→일기→아크·월 리듬→내일 각본·문안→한 번에 반영, 하루 경계 새벽 5시) / 08 대화 한 번이 지나가는 길(도착→텀 먼저 결정→조립→생성→다듬어 발송→오늘 메모, 모델 호출 1회). 01 카드도 우리 사이 다섯·유저 선호 중단에 맞춰 수정. **남은 설계는 1단계 잔여 하나뿐** — 운영 DB의 `relationships.state_json` → `char_facts`를 뼈대 두 표 키에 대조해 빠진 키 찾기(로컬 절반은 08-25 완료, 씨앗이 못 채우는 키 11개 확인). 그 뒤 구현 착수.
- **허브 05~08 피드백 1차 반영(08-25)**: 말투를 판정만 하고 버리던 것을 값 저장으로 변경(반말 사이가 된 뒤 유저가 존댓말을 섞으면 최근 발화 판정이 되돌아가고, 말 놓은 날짜만 남기면 지금 상태를 알 수 없음) / 04 답장 텀 표의 막대·회색 줄 제거(가득 찬 길이 8분에 근거가 없었음 — 눈금 규칙 없는 시각 요소는 폐기) + 붙잡을 수 있는 10분 설명을 표 각주에서 「유저가 붙잡으면」 쪽으로 옮겨 다시 씀 / 05 근황은 간격이 늘지 않고 두 번째에서 멈추는 것으로 정정, 각본 전환점 판단 주체가 모델임을 범례·본문에 반영 / time-and-memory.md에 「고정된 기준값」 절 신설 — 답장 텀·선톡·하루와 기록 3표에 값마다 출처(측정/판단/구현 잔재)와 결정 필요 여부 표시, 붙잡기 시간이 코드 12분·문서 10분으로 갈린 것도 여기 기록.
- **허브 피드백 2차(08-25)**: 붙잡기 시간 제한 폐기 — 일정 시작 후 10분(코드 12분) 안에만 조정 가능하던 규칙을 없애고, 답장 불가 블록에 온 메시지가 답을 기다리는 내용인지를 짧은 판정 한 콜로 가른다(개인=취소·사회=양해·공적=불가, 경과 시간 무관). 답장 텀에서 유일하게 모델을 부르는 자리라 04·08 카드의 범례와 호출 수도 수정 / 04 툴팁 예시 정정(회의는 틈틈이가 아니라 불가 — 예시를 회식으로) / 12분 상수 제거를 구현 잔손질 셋에 추가 / 즉답 0~2분은 설계상 짧은 쪽 몰림인데 코드가 균등(rand)이라 구현 때 skewLow로 맞춤. README는 허브 HTML로 교체·링크할 가능성이 있어 이번 정리에서 손대지 않음.
- **허브 피드백 3차(08-25)**: 선제 발화 용어를 **선톡**으로 통일(문서·허브 05 카드 전체). 종류를 여섯에서 다섯으로 재편 — 미리 만드는 둘(아침 선톡 / 안부 선톡=구 재연결) + 대화 중 셋(자리비움 선톡=구 예고+복귀 통합 / 근황 선톡 / 밤 인사 선톡=구 굿나잇, 이름 잠정). **아침 선톡의 "근거 있을 때만"을 폐기** — 어제 대화했으면 항상 보내고 챙길 것이 있으면 얹는다. 무응답 1일=아침 한 번 더 / 2일=아침 거르고 점심 한 통(시각 미정) / 3~13일=조용, 단 유저가 말해 둔 유저 일정이 있는 날 아침만 예외 한 통 / 14일=저녁 안부 선톡. 근황은 조건 셋(2시간+각본 전환점+연속 2회 taper)을 **4시간 무응답 하나**로 축소해 하루 한 통, 자리비움은 유저 발화 4시간 이내 게이트 유지(조건 없이 보내면 조용한 유저에게 알림만 쌓임) — 4시간이 대화 중 선톡 전체의 마지막 경계. 붙잡기 문단은 경과 시간 서술을 빼고 "개인·사회의 답장 불가 중 붙잡는 말이면 취소·미룸, 평범한 답장이면 그대로 유지"로 다시 씀. 판정 콜은 공적에서 미호출, 유저가 시간 두고 여러 번 보내면 묶음마다 한 번. 고정된 기준값 표도 선톡 이름으로 갱신(100~139분·2회 폐기, 점심 시각은 미정 표시)
- **허브 피드백 4차(08-25)**: 밤 인사 선톡 조건을 사용자 의도대로 정정 — 새벽 2~5시 창이 아니라 **밤 10시 이후 대화에서 유저가 자겠다는 말 없이 1시간 무응답**이면 발송, 미리 예약하지 않고 15분 틱마다 조건 재확인(유저가 새벽에 답하면 1시간 재계산, 유저가 잘 거라고 말하면 그 답장이 마지막 말이라 미발송). 자리비움 선톡 **하루 4회 상한 폐기**(일정 수가 곧 상한 + 4시간 게이트가 이미 대화 중일 때로 한정, 선톡 전체 6통 캡에서도 제외). 아침 선톡 툴팁에서 90분 유예 설명 제거(전송 실패 대비 장치라 카드에 둘 내용이 아님, 기준값 표에만 유지). 근황 선톡은 목적을 풀어 다시 씀(재촉하지 않고 자기 근황만 한 통 → 유저가 다시 말 걸 자리). 05 카드 foot 두 문단을 '층' 없이 재작성, aside 제목 '답이 없으면 물러난다'→'유저가 답이 없을 때', 확인 항목 '다른 종류가 겹치는가'→'다른 선톡 예약과 겹치는가'
- **허브 피드백 5차(08-25)**: 밤 인사 선톡 기준을 밤 10시→**자정**으로 물림(자정에 자는 사람이 10시부터 1시간 딴 일 하다 잘 자라는 인사를 받으면 그날 대화가 닫힘 — 기준값 표 00:00~05:00). 근황 선톡 뒤 무응답이면 그날 아무것도 더 보내지 않고 다음 날 아침 선톡으로 이어짐을 명시. 05 카드 foot 두 문단→한 문장(선톡도 대화 답장과 같은 내용을 담고 맨 끝 한 문단으로 종류를 알림, 쌓는 순서는 06 카드로 넘김), 캐시·비용 서술은 06 카드 aside에만 유지. 06 aside에 마지막 문단이 여섯 가지인 이유(자리비움만 나갈 때·돌아왔을 때 분리) 추가
- **허브 피드백 6차(08-25)**: 데이터 지도 카드를 프롬프트 카드에 합쳐 여덟 → 일곱 카드(01 캐릭터 생성 / 02 하루 각본 / 03 답장 텀 / 04 선톡 / 05 프롬프트 조합 순서 / 06 새벽 정리 / 07 대화 한 번) — 데이터 지도 왼쪽 열이 프롬프트 순서를 그대로 그리고 있어 같은 그림이 두 번 나왔다. 저장 항목 설명·예시는 각 묶음 툴팁으로 흡수, 두 곁상자(삶을 만드는 데이터·운영과 측정)는 보존. 카드 제목 '프롬프트 쌓는 순서'→'프롬프트 조합 순서', 부제는 「답장 생성 프롬프트를 어떻게 구성하는가」, 열 제목 '쌓는 차례'→'데이터 순서'(부제에 상단·하단 원칙). '층' 어휘 폐기 → 잘 바뀌지 않는 / 하루 동안 같은 / 지금 얘기로 찾아낸 / 방금 생긴 데이터, 각 묶음 small은 실제 저장 항목 이름만. 05 foot의 06번 카드 안내 삭제, 카드 사이 참조·내비게이션 전부 새 번호로
- **허브 피드백 7차(08-25)**: 합쳐온 설명이 저장 방식을 말하고 있어 05 카드 툴팁 전부를 "프롬프트에 무엇을 어떻게 꺼내 넣는가"로 재작성(씨앗=항목 이름과 값을 짝지어 넣음 / 유저 프로필=빈 항목 제외 / 최근 며칠 일기=항상, 그보다 오래된 것은 3번에서 주제 일치 시 — 기억 셀 지도의 읽는 조건을 그대로 문장화). **칸 이름 '우리 사이' → '캐릭터와 유저의 관계'**(문서·허브 전체). 묶음 이름 셋째·넷째 교체(지금 얘기로 찾아낸 → 대화 주제에 맞춰 꺼내는 / 방금 생긴 → 답장할 때마다 달라지는). 캐릭터의 삶을 만드는 데이터 곁상자 삭제(01 카드에서 이미 설명 + 프롬프트에 안 들어감), 운영과 측정 곁상자는 07 카드로 이동. 85%는 청구서가 아니라 로그의 캐시 재사용 토큰 수 × 0.1 단가 계산값임을 카드·문서 양쪽에 명시(첫 답장은 캐시 쓰기로 더 냄, 출력 요금 불변). time-and-memory.md 「프롬프트에 쌓는 순서」의 꼬리 설명에서 오늘 각본을 빼고 두 번째 묶음으로 정정
- **허브 피드백 8차(08-25)**: **05 기억 저장 구조 카드 신설**(선톡과 프롬프트 조합 순서 사이, 뒤 카드 한 칸씩 밀려 여덟 장 — 06 프롬프트 조합 순서·07 새벽 정리·08 대화 한 번). 내용은 time-and-memory.md 「항목 키와 주제 태그」 이하 절 — 기록 한 줄의 모양(저장 항목·키·내용·태그·딸린 값) / 저장은 키로 자리 찾기·읽기는 태그로 모으기 / 밤 4단계(쪼개기→저장 항목·키 붙이기(기존 키 목록 동봉)→덮을지 판정→태그) / 이름 태그로 줄끼리 엮기(연결 테이블 없음) / 지시문 미저장·사실만 / 임베딩·위계 보류 이유. 06 카드 손질 넷: 유저 프로필=값 있는 것만, 태도 규칙을 stance 원문(프레임 존중·안정형·아부 금지)에 맞춤, 오늘 각본 설명은 02 카드 가리키기로 축약, 진행 중인 일은 끝나는 조건부터 서술. **최근 일기 = 3일치**(구현값 `getRecentDiaries(id, 3)`, 기준값 표에 구현 잔재·결정 필요로 등재, 셀 지도 읽는 조건도 갱신). 06 카드 어휘 '꺼낸다'→'가져온다' 통일. **툴팁 hover→클릭 토글**(바깥 클릭·Esc 닫힘, 열리면 표시가 '닫기'로) — 스크롤 시 사라지던 문제
- **허브 피드백 9차(08-25)**: **05 카드 전면 재구성** — 카드 범위를 「대화에 필요한 데이터를 저장하고 주제별로 검색하는 방법」으로 확정하고 저장 항목 10가지를 카드에 명시(검색으로 골라 넣는 4=알게 된 유저 사실·주변 인물·진행 중인 일·지난 일기 / 항상 넣는 6=정체성·관계·유저 프로필·예정된 일·오늘·오늘 메모 / 최근 대화·지금 시각은 매 턴 계산값이라 이 구조 밖, 유저 성향은 운영 전용이라 미포함). 왼쪽=저장하는 데이터 구조 표(라벨 왼쪽 정렬)+두 동작(저장=키로 업데이트할 데이터 찾기 / 검색=태그로 관련 데이터 가져오기), 밤 4단계 처리는 오른쪽 곁상자→왼쪽 본문 이동. 저장 후를 '줄'이라 부르던 표현 폐기(데이터로), 연결 서술을 긍정형으로(이름 태그 하나로 인물·일정·일기 함께 검색), .cut 구분선의 문장 오용·기존 구조 대비 수치(71줄/43줄) 제거. **배치 이름을 새벽 정리로 통일**(밤 정리·새벽 하루 정리 혼용 정리, 실행 시각이 새벽이고 밤은 아직 대화 중이라 — 허브·time-and-memory.md 전체 반영, 코드와 이 파일의 밤 정리 표기는 구현 때 일괄 변경). **01 카드에 9월 달력 신설** — 운영 DB에서 월 리듬 실생성값(이벤트 6개·컨디션 시드 30일)을 읽기 전용으로 가져와 달력으로 그림. 기력=단일 색 점 진하기 3단계, 이벤트·그날 기분은 글자, 날짜 클릭 시 시드 4값(기력·기상·기분·이유) 툴팁. 카드 순서는 05 유지(01~04=캐릭터의 삶과 행동, 05~08=데이터 처리 — 06이 05의 검색 개념을 이어받아 앞이 맞다)
- **허브 피드백 10차(08-25)**: 05 카드를 저장 관리 방식 기준으로 재편 — 상단에 저장 이유(원문 통째 누적 대신 주제 단위 저장·업데이트로 저장량 최소화·검색 용이) 신설, 오른쪽 상자를 「키와 태그로 관리」(정체성·유저 사실·진행 중인 일 / 주변 인물=이름이 키 / 관계·유저 프로필=키 미리 고정) vs 「태그로만 관리」(지난 일기 / 예정된 일=날짜로 찾는 목록이라 키 없음 / 사람 이름 태그 설명 포함)로 교체, 항상/검색 구분은 프롬프트 기준이라 06 카드로 넘김. 핵심 흐름(검색은 전부 태그, 키는 저장 시 업데이트할 데이터 찾기 전용)을 표 밑에 명시. 용어 둘 변경: 딸린 값→**추가 정보**(코드 필드는 아직 없음, 구현 때 명명), 키 조각→**단어**(time-and-memory.md 전체 동기화, 문장 조각 의미는 유지). 키/태그 회색·자주 범례 폐지(다른 카드의 행위자 색과 충돌, 두 동작 다 k-ac로), 툴팁 있는 요소만 커서 포인터(.tip-host보다 뒤에 선언된 .aside·.step-b의 cursor:default가 이기던 특이도 버그 수정)
