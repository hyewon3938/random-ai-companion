...
### 작업 세션 나누기 (2026-08-26)

구현은 세션 하나에 단계 하나로 나누고, 단계마다 이슈와 브랜치를 하나씩 둔다. 나누는 이유는 둘이다. 단계마다 쓰는 모델이 다르고, 사람이 결과를 확인하고 넘어가야 하는 자리가 중간에 있다.
...
| 5 | 프롬프트 조립과 새벽 정리 다시 쓰기 | 「프롬프트에 쌓는 순서」, 「실제 한 밤을 통과시켜 보면」 | Fable |
| 6 | 쓰기를 새 칸으로 옮기고 며칠 관찰 | 「배포 순서」 3 | Opus |
| 7 | 읽기를 새 칸으로 옮기고 대화 확인 | 「배포 순서」 4 | Fable |
| 8 | 옛 경로 삭제 | 「배포 순서」 5 | Opus |
| 9 | 개편 전후 비교의 확인 열 채우기 | 「배포 순서」 6, 「개편 전후 비교」 | Fable |
...
중간에 사람이 봐야 멈추는 자리는 셋이다. 3번은 이관 후보를 확인하고 넘어가야 하고, 7번은 실제로 대화해 봐야 알 수 있으며, 9번은 개편 전후를 비교하는 판단이라 대신할 수 없다. 6번과 7번 사이에는 며칠이 필요하다. 새 칸에 쌓이는 것이 맞는지 보려면 새벽 정리가 몇 번은 돌아야 한다.
...
머지까지 세션 안에서 끝내도 된다고 정해 뒀으니 단계마다 다시 묻지 않는다. 다만 3·7·9번은 사람이 결과를 확인한 뒤에 PR을 올린다. 이슈 #13은 개편 전체를 묶는 이슈라서 9번 세션이 끝날 때 닫는다.
...
「작업 세션 나누기」 표는 V2 전 기준이라, 5번 세션부터는 V2 스키마 변경과 데이터 이관을 앞에 두는 순서로 다시 짠다. 운영 중인 캐릭터는 관계 컬럼의 초기값을 새로 정한 설정으로 채울 수 있고, 지난 일기·대화와 차이가 나는 부분은 만나 온 이야기에 사이가 변해 온 과정으로 서술해 흡수한다.
