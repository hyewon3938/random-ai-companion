...
| 선톡 | 여섯 곳이 각자 프롬프트를 조립해 누적 사실·관계·아크·각본 없이 쓰였다 | 대화와 같은 층을 쌓고 맨 끝 상황 문단만 다르다 | 아침 선톡이 어제 대화를 이어받는지 | 9/4 OK. 확인한 아침 선톡이 모두 어제 대화를 이어받았다 |
| 아침 선톡 | 어제 대화에서 이어질 이야기나 유저의 오늘 일정 같은 근거가 있는 날만 보냈다 | 어제 대화했으면 항상 보내고, 답이 없으면 아침 한 번·점심 한 번으로 줄이다 멈춘다 | 아침 인사가 매일 오는지, 조용해진 유저에게는 멈추는지 | 9/4 OK. 확인한 기간에 매일 나갔고, 조용해진 유저에게 멈추는지는 못 봤다 |
| 근황 선톡 | 2시간 무응답에 각본 전환점이 겹칠 때 보내고 연속 두 번째에서 멈췄다 | 4시간 답이 없으면 한 통 보내고 그날은 멈춘다 | 낮에 오는 선톡이 하루 한 통으로 줄었는지 | 9/4 OK. 하루 한 통 안에서 나갔다. 그 뒤 캐릭터의 마지막 말에서도 4시간을 재도록 조건을 바꿨다(#274) |
| 밤 인사 선톡 | 새벽 2~5시에 대화가 끊긴 지 한 시간이 지났을 때만 보냈다 | 자정이 지난 대화에서 한 시간 답이 없으면 보낸다 | 자정 넘어 대화가 끊긴 날 새벽 1시쯤 인사가 오는지 | – |
| 자리비움 선톡 | 하루 네 번까지 알렸고, 나갈 때와 돌아올 때를 함께 세서 실제로는 두 일정에서 상한에 닿았다 | 나갈 때 알리는 것만 세어 하루 세 번까지, 각본도 그만큼만 만든다 | 자리를 비우는 일정이 하루 세 개를 넘지 않는지 | 9/4 OK. 나갈 때 알린 것이 상한 안이었다. 그 뒤 상한을 하루 2개, 관계 초반 1개로 줄였다(#335) |
