...
기존 구조를 그대로 둔 채 백지에서 다시 설계한 결과이고, 코드를 이 설계로 옮기는 작업은 2026-09-04에 옛 경로를 지우면서(#156) 끝났다. 진행 기록은 이슈 [#13](https://github.com/hyewon3938/random-ai-companion/issues/13)에 있다.
...
| 1,000토큰 | 선톡 문안 호출의 출력 상한 · 생각 과정에 쓰는 토큰도 이 안에 든다 | 측정 · 정상으로 끝난 의도 선톡이 생각과 문안에 쓴 토큰이 많아야 460 정도라 두 배 넘게 잡았다(2026-09-19) | |
...
**재는 것**: 유저가 캐릭터에게 다시 말 걸고 싶어지는가. 마음은 직접 잴 수 없어서 실제로 다시 말을 건 행동을 센다.

**재는 법**: 매주 월요일 슬랙에 올라오는 주간 관계 요약의 유저 반응 줄을 쓴다. 재료는 새벽 정리 산출물 대신 한 주(월~일, 새벽 5시 경계)의 대화 기록이고, 코드가 그 기록에서 유저가 말한 날 수, 틈새 한 줄을 뺀 선톡 가운데 같은 날 6시간 안에 유저 답이 온 수, 캐릭터 말 뒤 6시간 안에 유저가 다시 말한 턴 수와 그 간격의 중앙값을 센다. 6시간 창은 반응 점수 표본의 규칙을 그대로 써서, 이 항목을 위해 새로 설계한 측정은 없다.

**통과선**: 9/4 운영 DB에서 센 7일 값인 선톡 26통 가운데 답 20통, 답장 뒤 6시간 안에 다시 말한 99건의 간격 중앙값 8분보다 나빠지지 않으면 통과다. 그날은 직접 조회로 세어 창이 주간 요약과 같다는 보장이 없으므로, 첫 주간 요약과 나란히 놓고 크게 다르면 첫 주 값을 기준선으로 삼는다.
...
구현은 세션 12개로 나눠 세션마다 이슈와 브랜치를 하나씩 두고 진행했다. 9번까지와 11번은 9/4까지 끝났고, 10번은 9/4에 운영 데이터 판정을 마친 뒤 9/10에 이슈 #262를 닫았으며, 12번의 문서 정리는 9/20에 V3 구현 7(이슈 #479)에서 했다. 세션마다 만든 것과 남긴 확인은 아래 「세션별 결과」에 적었다.

### 세션별 결과 (2026-08-26 ~ 09-20)
...
- **12번 세션의 문서 정리(09-20, V3 구현 7, 이슈 #479)**: 아래 「개편 전후 비교」에 확인 열을 만들어 9/4 운영 데이터로 판정한 네 줄을 옮기고, 그 뒤에 설계가 바뀐 두 줄에는 바꾼 이슈 번호를 적었다. 나머지 줄은 따로 판정하지 않았고, V2 묶음 이슈 #13은 9/10에 먼저 닫혔다. 채점표에는 유저가 다시 말을 거는지 재는 10번을 더했다.

## 개편 전후 비교 (2026-08-25)

설계 허브와 문서에서 "지금은 이런데 새 구조에서는 이렇게 된다"는 서술을 걷어내면서, 그 내용을 여기 한곳에 모았다. 문서 본문은 완성된 상태만 쓰고, 무엇이 어떻게 나아졌는지는 이 표로 남긴다.

개편 후 열은 2026-08-25에 확정한 설계다. 확인 열에는 9/4 운영 데이터로 판정한 네 줄만 적었고, 그 뒤에 설계가 바뀐 줄은 바꾼 이슈 번호를 함께 적었다. 나머지 줄은 따로 판정하지 않았으며, 나중에 확인할 때는 어떻게 확인하나 열의 방법을 쓴다.

| 항목 | 개편 전 | 개편 후 | 어떻게 확인하나 | 확인 |
| --- | --- | --- | --- | --- |
...
| 선톡 | 여섯 곳이 각자 프롬프트를 조립해 누적 사실·관계·아크·각본 없이 쓰였다 | 대화와 같은 층을 쌓고 맨 끝 상황 문단만 다르다 | 아침 선톡이 어제 대화를 이어받는지 | 9/4 OK. 9/3·9/4 아침 선톡 2통이 모두 어제 대화를 이어받았다 |
| 아침 선톡 | 어제 대화에서 이어질 이야기나 유저의 오늘 일정 같은 근거가 있는 날만 보냈다 | 어제 대화했으면 항상 보내고, 답이 없으면 아침 한 번·점심 한 번으로 줄이다 멈춘다 | 아침 인사가 매일 오는지, 조용해진 유저에게는 멈추는지 | 9/4 OK. 8/29부터 9/4까지 매일 나갔고, 조용해진 유저에게 멈추는지는 못 봤다 |
| 근황 선톡 | 2시간 무응답에 각본 전환점이 겹칠 때 보내고 연속 두 번째에서 멈췄다 | 4시간 답이 없으면 한 통 보내고 그날은 멈춘다 | 낮에 오는 선톡이 하루 한 통으로 줄었는지 | 9/4 OK. 9/2에 1통 나갔다. 그 뒤 캐릭터의 마지막 말에서도 4시간을 재도록 조건을 바꿨다(#274) |
| 밤 인사 선톡 | 새벽 2~5시에 대화가 끊긴 지 한 시간이 지났을 때만 보냈다 | 자정이 지난 대화에서 한 시간 답이 없으면 보낸다 | 자정 넘어 대화가 끊긴 날 새벽 1시쯤 인사가 오는지 | – |
| 자리비움 선톡 | 하루 네 번까지 알렸고, 나갈 때와 돌아올 때를 함께 세서 실제로는 두 일정에서 상한에 닿았다 | 나갈 때 알리는 것만 세어 하루 세 번까지, 각본도 그만큼만 만든다 | 자리를 비우는 일정이 하루 세 개를 넘지 않는지 | 9/4 OK. 9/2에 나갈 때 알린 것이 3건으로 상한 안이었다. 그 뒤 상한을 하루 2개, 관계 초반 1개로 줄였다(#335) |
...
위 두 줄은 2단계 3번에서 나온 것이라 표에 바로 더했다. V3에서 달라진 동작은 이 표에 더하지 않고 v3-readme.md에 V2와 다른 점으로 적는다.
...
구현 세션은 V2 기준으로 다시 짰다(2026-08-28). 5번부터 V2 스키마 변경·캐릭터 생성·데이터 이관이 앞에 오고, 프롬프트 조립과 새벽 정리 다시 쓰기는 그 뒤다. 운영 중인 캐릭터는 관계 컬럼의 초기값을 새로 정한 설정으로 채울 수 있고, 지난 일기·대화와 차이가 나는 부분은 만나 온 이야기에 사이가 변해 온 과정으로 서술해 흡수한다.
