...
**바뀌는 코드는 넷이다.** 아침·점심 선톡의 조건은 새벽 정리의 문안 준비와 dispatch, 근황 선톡과 밤 인사 선톡은 followup, 자리비움 선톡은 presence, 자리를 비우는 일정 수 상한은 하루 각본을 만드는 day-plan이다. 무응답 일수로 그날 무엇을 보낼지 정하는 판정은 침묵 백오프를 맡은 관제탑에 함수 하나로 두고, 문안을 준비할 때와 보내기 직전에 같은 함수를 부른다.
