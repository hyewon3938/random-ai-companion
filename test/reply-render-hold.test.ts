...
test("판정 문안은 물은 말에서 유저 표시를 떼고 개인 일정이면 취소할지를 답장이 정한다는 뜻을 붙인다", () => {
  ...
  assert.deepEqual(text.split("\n"), [
    ...
    "*판정* yes — 바로 답하고, 일정을 취소할지 하던 일로 돌아갈지는 답장 모델이 정한다",
  ]);
});

test("사회 일정은 미룰지를 답장이 정한다는 뜻, 붙잡지 않으면 그대로 둔다는 뜻이 붙는다", () => {
  const base = row({
    id: 7,
    purpose: "hold",
    output_hash: putBlob("yes"),
  ...
  assert.ok(
    social.endsWith(
      "*판정* yes — 바로 답하고, 양해를 구해 미룰지 하던 일로 돌아갈지는 답장 모델이 정한다",
    ),
  );
