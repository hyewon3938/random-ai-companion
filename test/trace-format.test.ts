// 슬랙 게시 문안의 표기 도우미(trace/format.ts)와 트레이스 표에 쌓는 두 함수(trace.ts)를 검사한다 — 슬랙은 부르지 않는다.
//
// 표기 도우미는 넣은 글자와 나온 글자만 견준다. recordTraceChunks는 긴 본문이 상한 단위로
// 몇 행이 되고 머리에 번호가 붙는지, 코드 울타리로 감싸는지를 트레이스 표 행에서 읽어 본다.
...
test("슬랙 값이 없으면 트레이스 표에 아무것도 쌓지 않는다", () => {
