...
test("아는 값이 있으면 아는 것 줄과 성별 전제 줄이 붙고 하는 일과 사는 곳은 참고 줄로 따로 붙는다", () => {
  ...
  assert.equal(
    full[1],
    `- 상대에 대해 아는 것: 성별은 여성, 나이대는 ${storedBand}, 하는 일(직군)은 간호사, 사는 곳은 부산. 확정된 사실이니 다시 넘겨짚지 않는다. 다만 하는 일은 직군일 뿐이라, 지금 다니는 곳이 있는지나 출근하는지까지는 이 값으로 알 수 없다.`,
  );
  assert.equal(
    full[2],
    "- 상대의 하는 일·사는 곳은 연락이 닿을 시간대나 거리 감각을 가늠할 때 참고한다. 하는 일만 보고 출근·퇴근·회사 일을 전제해 묻지 않는다. 아는 것을 굳이 꺼내 보이거나 다시 캐묻지 않는다.",
  );
  ...
  assert.equal(
    jobOnly[1],
    "- 상대에 대해 아는 것: 하는 일(직군)은 개발자. 확정된 사실이니 다시 넘겨짚지 않는다. 다만 하는 일은 직군일 뿐이라, 지금 다니는 곳이 있는지나 출근하는지까지는 이 값으로 알 수 없다.",
  );
