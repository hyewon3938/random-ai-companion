// 몰아 답장 표시와 답장이 멈춘 자리가 트레이스 표에 쌓이는지 검사한다 — 모델도 텔레그램도 부르지 않는다.
//
...
// dropWakeRows가 구간 끝 행마다 남기고 약속 행은 건드리지 않는 것, 예외로 끝난 자리가
...
const { db, insertOutboxRow, waitingWakeRow } = await import("../src/db.js");
...
const insertWake = (p: {
  ...
  const id = insertOutboxRow({
    ...
    payload: {
      ...
      ...(p.kind === "promise"
        ? { promise: "집 가서 연락", userMsgAt: "2026-09-10 21:38:00" }
        : {}),
    },
...
test("표시를 거두면 거둔 행마다 쌓이고 약속 행은 건드리지 않는다", () => {
  ...
  // 유저가 말을 건 구간(첫 발화 시각 있음)과 말을 걸지 않은 구간을 하나씩 건다
  const wake = insertWake({
    chatId: chat,
    kind: "block_end",
    userFirstAt: "2026-09-10 21:38:00",
  });
  const ret = insertWake({ chatId: chat, kind: "block_end", activity: "회의" });
  ...
  assert.equal(
    (
      db.prepare(`SELECT status FROM outbox WHERE id = ?`).get(
        promise,
      ) as { status: string }
    ).status,
    "waiting",
...
test("걸려 있는 표시를 행 번호와 함께 돌려주고 울리는 행은 빼고 센다", () => {
  ...
  const ret = insertWake({ chatId: chat, kind: "block_end" });
  const found = waitingWakeRow(chat);
  assert.equal(found?.id, ret);
  assert.equal(found?.kind, "block_end");
  assert.ok(found?.payload_json.includes("차로 퇴근"));
