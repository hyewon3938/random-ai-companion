...
- [ ] **진행 중인 일을 각본대로 옮기기 (09-04 머지·배포 대기, 이슈 #276, PR #279)** — 유저가 아는 캐릭터 쪽 일만 행 번호를 달고 각본에 들어가고, 새벽 정리가 어제 각본의 그 블록을 보고 값을 한 걸음 옮긴다. 끝나면 사실로 옮긴다. **배포 전 필수**: #274와 같이 9/5 새벽 정리 확인 뒤 올린다. 봇 밖 새벽 정리 지시서는 이미 고쳤다. 볼 것: 오늘 각본의 책 블록에 source ongoing과 번호가 붙는지, 다음 새벽 트레이스에 진행 중인 일 글이 붙고 값이 옮겨지는지, 완독 뒤 사실로 옮겨져 각본에서 빠지는지
- [ ] **캐릭터가 말한 일정 시각 (09-04 머지·배포 대기, 이슈 #277·#278, PR #280·#282)** — 답장이 이번에 정해 말한 자기 일정의 시각을 오늘 메모로 남기고, 새벽 정리가 이미 저장된 일정 줄의 시각을 그 값으로 고친다. **배포 전 필수**: 외부 스케줄러 지시서에 schedule_updates 규칙을 넣어야 기본 경로에서 그 항목이 나온다(문안은 gitignore된 docs/woojin-nightly-patch-278.md). 볼 것: 캐릭터가 시각을 말한 날 `today_notes`에 그 값이 적히는지, 새벽 정리 트레이스에 일정 시각 고침이 남는지, 다음 날 각본에 같은 시각으로 들어가는지
- [ ] **근황 선톡의 침묵 조건 (09-04 머지·배포 대기, 이슈 #274, PR #275)** — 유저와 캐릭터 둘 다 마지막 말이 네 시간 지나야 나간다. **배포 전 필수**: 9/5 새벽 정리가 새 스키마에서 정상으로 돈 것을 확인한 뒤 올린다. 볼 것: 자리 비움 복귀 인사 뒤 네 시간 안에 근황이 안 나가는지
- [ ] **옛 경로 삭제와 호출 관측값 (09-04 배포, 이슈 #156·#218, PR #272·#273)** — v6·v7 마이그레이션이 돌고 외래 키 검사 0건까지 확인했다. 볼 것: 답장 호출 행에 중단 사유와 블록 종류가 적히는지, 예고를 접은 날 `trace_events`에 접은 사유 행이 남는지, 9/5 새벽 정리가 새 스키마에서 정상으로 도는지
...
- **출력 규칙 단일 소스**: 캐릭터가 내보내는 모든 글에 공통으로 적용할 규칙(태도·대화 규칙·표기·말의 결)은 `src/prompts/reply.ts`의 고정 문안에서만 관리한다 — PERSON·SPEECH·OUTPUT_FORMAT·FACT_CARE·NOTE_RULE. 캐릭터마다 다른 값으로 두면 생성 결과에 따라 규칙이 흔들리므로 정체성 항목에 넣지 않는다. 선톡 문안 프롬프트 7곳(nightly 아침·안부, followup 근황·굿나잇·달래기, presence 자리비움·복귀)도 전부 `buildSystemBlocks`(3층+상황 문단)를 타므로 같은 규칙이 자동으로 들어간다. 큰 결을 바꿀 일이 생기면 이 블록들을 고친다. 다만 답장이 내보내는 **형식**(JSON 객체)은 규칙층이 아니라 `src/reply-signal.ts`가 갖는다 — 선톡 문안 6곳은 같은 3층을 쓰되 자기 형식으로 답하므로, 규칙층에 넣으면 두 형식이 부딪힌다.
