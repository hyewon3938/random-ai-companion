...
- **작업 단위**: `/build` 절차대로 이슈부터 만들어 PR 머지까지 한 세션에서 끝낸다. 머지 전에 사람이 확인해야 하는 세션은 `LOCAL-SESSIONS.md` 대기열의 머지 전 확인 칸에 적혀 있다. 머지한 자리에서 `v3-readme.md`에 지금 동작과 V2와 다른 점, 이슈 번호를 한 줄 붙이고, 리드미에 안 갈 작업이면 「안 넣는 것」에 붙인다. 끝난 작업은 이 파일에 적지 않고 닫힌 이슈로 찾는다.
