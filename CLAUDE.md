...
- [x] 스캐폴딩: grammY 봇 + SQLite 스키마 + Anthropic 연동 + Docker (2026-07-06)
- [ ] 토큰 수령 후 로컬 기동 확인 (`yarn dev`) → 첫 매칭·대화 확인
- [ ] 밤 일기 크론 구현 (`src/index.ts`의 TODO — 23:30 KST)
- [ ] 아침 안부 크론 구현 (08:30\~09:10 지터, 근거 있을 때만)
- [ ] 교체 플로우 (비가역 확인 + "어떤 점이 아쉬웠어?" → user_preferences)
- [ ] 분할 발화·미러링 (대화 리듬)
- [ ] OCI VM 배포 (docker compose, 기존 서비스와 별개 컨테이너)
- [ ] 프로브 플래그 (교체 제안 / 침묵일)
