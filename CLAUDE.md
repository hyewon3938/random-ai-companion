...
- [ ] **선톡과 답장을 연락 하나로 묶기 (09-11 설계, 설계 원본은 repo 루트 `outgoing.md`)** — 내보내는 연락 전부를 계기 발생 → 시각 정하기 → 메시지 작성 → 내보내기 네 단계로 지나가게 하고, 예약 표 한 장에 연락 한 건을 한 행으로 적는다. 종류마다 다른 것은 함수 3개(연락 계기·발송 판정·메시지 작성)와 값 2개(하루 상한 여부·유저가 말을 더 보낼 때 버릴지)로만 둔다. 적용 순서 8단계와 세션 9개 표는 그 문서에 있고, ADR 2건(표 하나로 묶기, 계기 조건과 발송 조건 갈라내기)이 세션 3·6에 딸린다. 이슈는 세션마다 따로 만든다. 세션 1은 #430으로 09-13에 머지했고 09-14 01:29에 배포했다. 다음은 깨우기·약속 핸들러를 동작 그대로 떼어내는 세션 2이고 09-15에 연다
