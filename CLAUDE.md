...
- [ ] **캐릭터가 말한 일정 시각 (09-05 배포, 이슈 #277·#278, PR #280·#282)** — 답장이 이번에 정해 말한 자기 일정의 시각을 오늘 메모로 남기고, 새벽 정리가 이미 저장된 일정 줄의 시각을 그 값으로 고친다. 9/5 각본은 대화에서 나온 두 시 반을 일기를 거쳐 받았지만 일정 42번 줄의 time_hint는 아직 오후다. 볼 것: 캐릭터가 시각을 말한 날 `today_notes`에 그 값이 적히는지, 새벽 정리 트레이스에 일정 시각 고침이 남는지, 다음 날 각본에 같은 시각으로 들어가는지
