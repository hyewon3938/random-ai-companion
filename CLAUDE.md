...
- [ ] **리팩토링 뒤 테스트 없는 파일에 검사 붙이기 (09-06, 이슈 #305)** — 영역별 리팩토링 6단계는 9/6에 다 끝났다(areas.md 순서 절). 어느 단계도 건드리지 않아 테스트가 없는 파일 8개(recall·tag-pick·user-profile·life-plan·dispatch·feedback·nightly-trace·relationship-update)에 파일 하나당 테스트 하나를 붙인다. 모델을 부르지 않는 자리부터 한다.
