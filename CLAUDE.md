...
- [ ] **로그에만 남는 관측값을 DB에 적기 (이슈 #218, 9/4 착수)** — 생각 과정 블록·중단 사유는 `llm_calls`에, 예고를 접은 기록은 트레이스에. 배포 한 번에 관측이 끊기는 것을 막는다
- [ ] 프로브 플래그 (교체 제안 / 침묵일)
...
- [ ] **답장에서 생각 과정을 끌지 정하기 (08-30 관측)** — `src/llm.ts`는 끄는 호출만 값을 싣는데 sonnet은 값이 없으면 생각을 켜고, 그 텍스트는 안 돌아오면서 토큰만 출력으로 나간다. 상한 1,200을 다 쓰고 144자만 돌아온 날이 있어 상한을 2,000으로 올렸다(이슈 #214). 9/1 답장 4건은 전부 `blocks=thinking:1,text:1`에 출력 167~369토큰·글자 59~124자다. 끌지는 #218이 값을 DB에 남긴 뒤 분포를 며칠 보고 정한다
...
- [ ] **옛 경로 삭제 배포 (09-04, 이슈 #156, PR #272)** — 배포 전 필수: 운영 DB 백업. 첫 부팅의 v6 마이그레이션이 표 5개·컬럼 3개를 지워 되돌릴 수 없다. 배포 뒤 마이그레이션이 한 번 돌고 외래 키 검사가 깨끗한지 본다
