...
나와 같은 시간을 살고 먼저 물어봐주는 AI 대화 상대 PoC (만남은 카드 한 장으로 시작). 컨셉·가설·설계 총론은 [README.md](README.md), 타겟·어필 포인트(일상 침투·상상으로 완성·역체감)는 [docs/positioning.md](docs/positioning.md), 설계 논리·층은 [docs/architecture.md](docs/architecture.md), 캐릭터 속성 체계는 [docs/character-design.md](docs/character-design.md), 수집·생성 데이터 카탈로그(유저/캐릭터 입장·상호작용·신호)는 [docs/data-model.md](docs/data-model.md)가 단일 소스. 이 저장소 밖의 기획 사료 위치는 `LOCAL-CONTEXT.md`(gitignored, 로컬 전용) 참조 — 새 세션은 그 파일부터 읽을 것.
...
- [x] **밤 정리(구 밤 일기) — 이중 경로 확정** (07-11): 기본 = **scheduled task `woojin-nightly`(매일 05:06 KST, 구독·API 비용 0)** — ssh로 `src/tools/nightly-read.ts`(수집)→자체 생성→`nightly-write.ts`(적용). 폴백 = 봇 내 05:40 크론(API, opus) — 일기 중복 체크로 이중 실행 방지. 산출 = 일기 응고(각본 vs 실제·상대 감정·내부 온도) + 유저기억·누적정체성·일정·인물 추출 + 새 날 각본 + 선톡 문안
- [x] **선톡 = 밤 정리(문안 준비) + 발송 디스패처(10분 틱, LLM 콜 없음)** — scheduled_sends 하루 1통, 근거 있을 때만, 유저가 그날 먼저 연락했으면 미발송. 무응답 시 익일 후속은 밤 정리 판단에 위임 (07-11)
- [ ] ~~교체 플로우~~ → **PoC 미구현 결정**(첫 캐릭터 정착 시나리오). 설계는 docs/architecture.md "이별과 교체" 절에 문서화 완료 (07-11)
...
[Telegram] ←→ src/bot.ts (grammY long polling, 유저 분할입력 디바운스(유저별 텀 학습 20~40s) + 분할 발화 + 각본기반 답장지연 blockDelayMs(밤대화 즉답·초반 밀착 EARLY_DAYS=60·자다 깸·불가는 일정 끝날 때쯤) + 물음표 보정 fixQuestion + 부팅 시 놓친 답장 복구 recoverMissedReplies=워터마크 중복방지+최근 3h만(자정 경계 버그 회피) + 발송 종류 로그 kind=reply|recover|morning|followup)
                ...
                ├─ src/dispatch.ts — 아침 안부 디스패처(10분 틱, LLM 0): 밤에 준비된 문안을 창 안 발송, 유저가 그날 먼저 연락 시 skip
                ├─ src/followup.ts — 침묵 팔로업(15분 틱, 크론 0-3,8-23시): 낮=유저 무응답 ~2h+각본 전환점이면 근황(연속 무응답 taper), 밤=유저가 잔다 말 없이 2h+ 잠수하면 굿나잇 1회. 경과시간은 Date.now 기준(getKstNow().getTime()은 +9h라 오산)
                └─ src/presence.ts — 자리 비움 예고(10분 틱, 크론 7-23시, 관계 초반): 곧 긴 불가(운동·샤워·외출, ≥25분)로 들어가기 직전/연속 불가 경계에서 "이제 러닝 갈게요 답 늦어요"/"막 왔어요 씻고 올게요" 선톡(블록당 1회, 하루 4회 상한). 공적 불가(회의·시험)는 "끝나고 연락" 톤. **개인·사회 불가는 유저가 붙잡으면 respond가 [남음] 태그 읽어 attention_override 세팅→접거나 미루고 돌아옴(그 블록 즉답), 공적은 못 접음.** override 중엔 예고 스킵. 막연한 침묵→'알고 하는 기다림'
[SQLite] src/db.ts — characters / relationships(관계+유저기억+누적정체성, 캐릭터별) / user_preferences(매칭 전용, 캐릭터에 주입 금지) / diary_entries / day_plans(하루 각본) / day_seeds(매일 컨디션 시드: 기력·기상·기분·이유) / schedules(일정 슬롯, who=char|user) / cast_members(관계도, who=char|user) / arcs(연·계절·월·주 흐름=서술) / scheduled_sends(아침 안부 문안) / recovery_marks(복구 중복 방지 워터마크) / attention_override(유저가 붙잡아 개인·사회 일정 접거나 미룬 상태, until_ts까지 즉답) / messages(원시 로그 = 측정 재료, meta.kind로 발송 종류=reply|recover|morning|followup|presence)
[모델] 실시간 대화=sonnet / 일기·추출·각본·월 리듬·아크·캐릭터 생성=opus(MODEL_DEEP) / 밤 정리 기본 경로는 scheduled task(구독)라 API 미사용
```
