...
- [x] **말투 자연화 — 말의 결 규칙 (08-19, 이슈 #9)** — 모델 기본 버릇(직전 발화 명사구 되받기·문어체 은유·상담사식 감정 라벨링 공감)을 억제하는 `SPEECH_TEXTURE` 블록을 공통 규칙층(불변층)에 신설, buildSystemBlocks를 타지 않는 선제 발화 문안 프롬프트 6곳(followup·presence·nightly)에는 압축판 `SPEECH_TEXTURE_COMPACT` 주입. 사색적 취향은 유지하고 표현만 입말로(내용/표현 분리), 무심함 회귀 방지 가드 포함 — humanizing-log #18
