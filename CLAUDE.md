...
- [ ] 밤 일기 = **Claude 앱 scheduled task(루틴)로 비동기 처리** (확정 07-06): 밤 루틴이 ssh로 VM DB에서 오늘 대화·상태를 읽어 일기 작성(Opus) + 관계 상태 패치 + **다음날 안부 문안까지 미리 작성**해 scheduled_sends에 등록. 실시간 대화만 API(sonnet). 루틴 SKILL 작성 + VM 읽기/쓰기 헬퍼 스크립트 필요. poc-설계 §6\~7에 이 방식 전파할 것
- [ ] 발송 디스패처 (VM 크론 15분 틱 — **LLM 콜 없음**, 밤에 준비된 문안 발송만. 창 08:00\~22:30, 무응답 시 익일 오전 랜덤 후속 1회)
