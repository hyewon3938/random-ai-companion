...
- [ ] **말투 예시와 신경 쓰는 티 (09-04 착수, 이슈 #281·#285, PR #283)** — 목표 말투 예시 12쌍을 프롬프트에 넣고 어미 반복·추측형 말끝 겹침·하루 은유·질문의 말 되풀이를 평가 항목으로 잰다. 9/5에 안정형 stance를 빼고(ADR-0012) 기다렸다는 말·가벼운 질투·시키는 말끝을 규칙층에 넣었으며 예시도 그 결로 다시 썼다. 예시 문안은 사람이 함께 다듬는 자리라 PR은 확인 뒤에 머지한다. 머지 전에 `eval` 라벨의 통과율을 본다. 캐릭터 정체성 값에 남은 안정형 문구는 사용자가 정한다
