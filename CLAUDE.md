...
- [ ] **말투 예시와 어미 반복 규칙 (09-04 착수, 이슈 #281, PR __PR__)** — 목표 말투 예시 12쌍을 프롬프트에 넣고, 말풍선 끝 어미 반복·추측형 말끝 겹침·하루 은유·질문의 말 되풀이를 평가 항목으로 재기 시작했다. 예시 문안은 사람이 함께 다듬는 자리라 PR은 확인 뒤에 머지한다. 머지 전에 `eval` 라벨의 통과율을 본다
- [ ] 프로브 플래그 (교체 제안 / 침묵일)
