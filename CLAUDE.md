...
> 로그에만 남는 표시(`blocks=`·`[presence] ... 접음`)는 컨테이너를 다시 만들면 지워지므로 배포 절차 0단계에서 로그를 파일로 뜬다. PR #273 배포 뒤에는 같은 값이 DB에 쌓인다.

- [ ] **옛 경로 삭제와 호출 관측값 배포 (09-04, 이슈 #156·#218, PR #272·#273)** — 배포 전 필수: 운영 DB 백업. 첫 부팅의 v6 마이그레이션이 표 5개·컬럼 3개를 지워 되돌릴 수 없고, v7은 `llm_calls`에 칸 둘을 더한다. 배포 뒤 마이그레이션이 돌고 외래 키 검사가 깨끗한지, 답장 호출 행에 중단 사유와 블록 종류가 적히는지, 예고를 접은 날 `trace_events`에 접은 사유 행이 남는지 본다
- [ ] **9/4에 고친 넷 (이슈 #264·#265·#267·#269)** — 배포 전 필수: 새벽 정리 외부 지시서에 #268의 기억 합치기 규칙을 넣는다(초안은 docs/woojin-nightly-patch.md). 볼 것: 유저 말이 없을 때의 자리 비움 예고에도 자리를 비운다는 말이 있는지, 새벽 정리가 유저 사실의 앞 값을 지우지 않고 합치는지, 결과 문장에 건너뛴 일정 건수가 붙는지, 발송이 끊겼을 때 답장이 1·2·5·10분에 다시 나가고 선톡이 다음 틱에 다시 나가는지. 중복 일정은 `src/tools/dedupe-schedules.ts`를 dry-run으로 보고 지우는 것은 사용자가 정한다
