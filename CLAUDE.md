...
- [ ] **캐릭터가 말한 일정 시각 (09-04 머지·배포 대기, 이슈 #277·#278, PR #280·#282)** — 답장이 이번에 정해 말한 자기 일정의 시각을 오늘 메모로 남기고, 새벽 정리가 이미 저장된 일정 줄의 시각을 그 값으로 고친다. 외부 스케줄러 지시서에는 같은 규칙을 9/4에 넣었고, 배포 전까지는 지시서가 그 항목을 만들어도 봇이 버린다. 볼 것: 캐릭터가 시각을 말한 날 `today_notes`에 그 값이 적히는지, 새벽 정리 트레이스에 일정 시각 고침이 남는지, 다음 날 각본에 같은 시각으로 들어가는지
