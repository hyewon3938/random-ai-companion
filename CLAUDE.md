...
- [ ] **자리비움 예고 문안에 자리를 비운다는 말이 빠지는 것 (09-04, 이슈 #265)** — 유저가 방금 남긴 말이 없을 때는 상황 문단에 반드시 남긴다는 줄이 안 들어가서, 예고가 지난 대화에 답만 하고 나갔다. 예고가 접힌 사유가 트레이스에 없는 것은 #218에서 함께 옮긴다
...
- [ ] **오늘 메모가 한 건도 안 쌓이던 것 (09-02, 이슈 #257·#259)** — 대화 중에 남길 것이 나온 날 `today_notes`에 행이 생기는지 본다. 함께 볼 것은 갓 시작한 대화다 — 기록이 두 턴뿐인 평가 케이스에서 답이 객체로 안 오는 회차가 55회 중 1회에서 7~9회로 늘어서, 첫 몇 턴에서 형식 깨짐 재생성이 잦아지는지 본다
- [ ] **밤에 자다 깨서 답하는 텀 (09-02, 이슈 #255)** — 자는 시간에 말을 걸었을 때 첫 답장이 2분 30초 안에 오는지, 이어지는 답장이 즉답 구간으로 붙는지 본다
- [ ] **자리 비움 예고 두 가지 (09-02, 이슈 #251·#252)** — 답을 미룬 예고에도 무슨 일로 자리를 비우는지 들어 있는지 하루 본다. 반대로 답을 미룬다는 말이 사라지지는 않는지도 함께 본다. 발송이 다시 실패하면 슬랙 문안 스레드에 발송 포기 행이 붙는지 본다. 9/4 정찰에서 9/2 예고 3건 중 1건이 자리를 비운다는 말 없이 나갔다(이슈 #265)
