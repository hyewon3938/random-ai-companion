...
1. **메시지 수신.** 대화 기록에 저장하고, 유저가 말을 이어서 보내는 중일 수 있어 바로 답하지 않고 잠시 기다린다. 기다리는 시간은 그 유저가 메시지를 연달아 보낼 때의 간격을 모아 20초에서 40초 사이로 정하기 때문에, 길게 이어 치는 사람에게는 길게 짧게 끊어 치는 사람에게는 짧게 기다린다.
2. **답장 텀 결정.** 기다리는 시간이 끝나면 지금 시각이 각본의 어느 일정인지 보고 두 태그로 텀을 정한다.
   - 답장할 수 있는 일정이면 답장을 바로 만들어 두고 정해진 텀만큼 기다렸다가 보낸다.
   - 답장이 불가한 일정이면 답장을 만들지 않고, 그 일정이 끝날 때 깨어나도록 표시만 걸어둔다. 일정이 끝나면 그동안 쌓인 메시지를 한 번에 읽고 몰아서 답한다.
3. **주제 태그 고르기.** 저장해 둔 태그 목록을 짧은 모델 호출로 훑어 이번 발화와 관련된 것을 고른다. 목록에 없는 낱말을 지어내 답하는 경우가 있어 저장된 목록과 대조해 없는 것은 버린다. 관련된 태그가 하나도 없으면 주제로 가져오는 데이터 없이 관계와 주변 인물, 진행 중인 일, 최근 일기만으로 답하고, 그 대화에서 새로 알게 된 것은 새벽 정리 때 태그와 함께 저장되어 다음부터 검색에 걸린다.
...
6. **발송.** 2번에서 정한 텀만큼 기다렸다가 말풍선으로 쪼개 타이핑 시간을 두고 순차 전송한다. 전송이 성공한 뒤에 어디까지 보냈는지 기록한다. 기다리는 동안 유저가 말을 더 보내면 1번처럼 아직 할 말이 남은 것이라, 만들어 둔 답장을 버리고 2번부터 다시 한다.
7. **기록 게시.** 발송이 끝나면 이 답장에 들어간 프롬프트와 모델이 쓴 답, 텀이 정해진 경로를 슬랙 채널에 올린다. 답장의 품질을 확인하고 개선하기 위한 기록이고, 어떻게 활용하는지는 다음 절에서 설명한다.
