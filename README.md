...
캐릭터가 유저와 서로의 일상을 자연스럽게 주고받으려면, 유저에 대해 알게 된 것을 계속 기억하고 자기가 한 말도 앞뒤가 맞게 이어 가야 한다. 그래서 대화에서 나온 주제와 유저의 성향, 유저가 지나가듯 말한 일정을 골라 기억하는 것부터 설계했다.

대화 원문을 통째로 쌓아 두고 필요할 때마다 찾아 쓰는 방법도 있지만, 그러면 프롬프트가 대화량만큼 길어지고 답장 한 건을 만들 때마다 중요한 대목을 다시 골라내야 한다. 답장은 유저가 방금 보낸 짧은 메시지 하나에 답하는 일이라, 그 메시지에서 나온 주제에 걸리는 기억을 우선해 꺼내 넣는 편이 더 정확하다고 생각했다. 지금은 최근 대화 40건을 그대로 넣고, 거기에 주제로 찾은 기억을 더해 프롬프트를 만든다.

그래서 대화 내용을 주제별로 한 건씩 나눠 키를 붙여 저장하고, 같은 주제가 다시 나오면 새로 쌓지 않고 그 건의 내용을 업데이트하는 구조로 만들었다. 무엇을 남길지는 매일 새벽에 모델이 그날 대화와 주요 데이터를 다시 읽고 정한다.

기억은 잘 바뀌지 않는 사실, 진행 중인 일, 주변 인물 3가지로 나누고 각각을 캐릭터 쪽인지 유저 쪽인지로 다시 나눠, 모두 6가지 자리에 저장한다. 기억 한 건은 저장 항목과 주인, 업데이트할 자리를 찾는 키, 내용, 검색에 쓰는 태그로 이뤄진다. 아래는 실제로 저장되는 모양을 보여주는 예시 3건이다.
