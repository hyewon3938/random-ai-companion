...
1. [캐릭터 생성](#캐릭터-생성)
2. [하루 각본과 답장 텀](#하루-각본과-답장-텀)
3. [먼저 거는 연락](#먼저-거는-연락)
4. [기억 저장 구조](#기억-저장-구조)
5. [프롬프트 조합과 관리](#프롬프트-조합과-관리)
6. [유저의 한마디부터 답장까지](#유저의-한마디부터-답장까지)
7. [답변 품질 관측과 개선](#답변-품질-관측과-개선)
...
캐릭터가 만들어진 그날부터 몇 시에 일어나 무엇을 하고 언제 어디에 있는지를 시간대별로 정한다. 캐릭터는 자기 하루가 미리 정해져 있다는 것을 모른 채 그 하루를 자신의 삶으로 알고 살아가서, 만들어진 순간부터 오늘을 살고 있는 상태가 된다.
...
캐릭터의 하루는 대화가 시작되기 전에 만들어두기 때문에 유저가 언제 묻든 같은 하루가 나온다. 아침이면 그날 컨디션대로 일어나고, 어제 회식을 했으면 오늘 피곤해하고, 유저가 읽지 않는 사이에도 그 하루가 이어진다. 계획과 다르게 지낸 시간은 코드가 실제 기록으로 따로 남기고, 삶의 흐름도 한 번 만들고 끝내지 않고 매주 월요일과 매달 1일에 최근 일기를 반영해 이어 쓴다.
