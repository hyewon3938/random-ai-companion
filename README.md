...
이 문서는 **"AI가 사람처럼 느껴지려면 무엇이 필요한가"** 를 문제로 놓고, 그것을 어떤 판단으로 풀었는지 정리한 기록이다. 기획·설계·구현·배포·운영을 혼자 했고, 배포한 뒤로는 직접 유저가 되어 매일 대화하며 고치고 있다.
...
| 저장 | SQLite (better-sqlite3, WAL) | 지금 유저 규모에서 별도 DB 서버를 띄울 이유가 없다. WAL로 배치와 대화의 동시 접근을 견딘다 |
