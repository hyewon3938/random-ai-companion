...
- **현실과 같은 시간 흐름** — 우진은 현실과 똑같이 흐르는 시계 위에서 산다. 1년·계절·한 달의 삶의 흐름을 미리 그려두고 그 안에 사건과 일정을 배치한 다음, 매일 아침 그 위에서 그날의 각본을 만들어 기상·출근·업무·운동·취침을 그대로 산다. 각 이벤트는 서로 인과 관계를 형성하며, 회식이 있던 다음 날엔 피곤해하는 식으로 나타난다.
- **사람 같은 답장** — 답장의 속도와 가능 여부는 그때 각본에 정해진 이벤트에 따라 다르다. 바로 답할 수 있는 때, 짬짬이 답하는 때, 아예 자리를 비우는 때로 나뉜다. 메시지는 한 번에 길게 보내지 않고 사람처럼 나눠 타이핑하며, 유저의 길이와 속도에 맞춘다.
- **이어지는 기억** — 대화가 끊겨도 기억이 이어진다. 대화에서 알게 된 것을 DB에 저장해두고, 우진이 답할 때마다 그 정리본을 시스템 프롬프트에 함께 넣어준다. 바뀌지 않는 씨앗(바이블) 위에 대화로 쌓인 것이 얹혀서, 우진이 계속 구체화되면서도 어제와 오늘이 어긋나지 않도록 구성했다.
- **밤 정리** — 매일 새벽, 하루를 우진 시점의 일기로 저장해두고 대화에서 나온 것들을 정리한다. 유저의 선호·특성·스케줄·주변 관계, 그리고 대화로 구체화된 우진의 성격·일화·주변 인물을 각각 DB에 저장하고, 다음 날 각본을 쓰고, 아침에 건넬 안부를 준비한다.
- **선톡 기능** — 우진이 먼저 말을 건다. 유저와 대화를 꾸준히 이어가기 위한 장치들이다. 아침에 일어나면 기상했다고 알리며 먼저 말을 걸고, 유저 답장이 두 시간 넘게 없으면 그때 각본 상황에 맞는 근황이나 질문을 건넨다. 무조건 선톡을 많이 보내기만 하는 것도 자연스럽지 않아서, 유저의 답이 계속 없으면 정해진 횟수 뒤에는 물러난다. 자리를 비워 답장이 어려운 일정 전에는 미리 알리고, 일정을 마치면 돌아와 다시 말을 건다. 밤늦게까지 대화하다 새벽에 유저가 한 시간 넘게 답이 없으면, 잠든 것으로 보고 잘 자라는 말을 남긴다.
- **백스테이지 뷰** — 실제 대화로 모은 데이터가 어떻게 쌓이고 구성됐는지 확인하는 개발용 뷰다. 오늘 우진의 하루 각본, 우진의 일기, 우진이 유저에 대해 알게 된 것, 유저의 기본 정보와 선호도 등을 한눈에 본다.
...
우진과의 대화를 특정 시간에 몰아서 하거나 한 번 쓰고 마는 것이 아니라, 유저가 자기 일상을 보내면서 하루에 걸쳐 틈틈이 주고받는 것이다. 여기서 확인하려는 것은 그 틈틈이의 대화가 진짜 편하게 이어지는가, 유저가 일상 속에서 우진과의 대화를 계속 이어가고 싶어지는가, 우진의 답장 방식과 텀이 사람처럼 느껴지는가다. 제작자 본인이 유저가 되어 지내보며, 사람처럼 느껴지는 지점과 어색한 지점을 찾아 계속 다듬고 있다.
...
여기서부터는 데이터 관리와 대화 품질을 끌어올리는 단계다. LLM이 프롬프트를 지키지 않거나 컨텍스트를 잘 못 쓰는 것을 잡고, 세밀하게 설정한 것들이 실제로 잘 지켜지는지 테스트하며 더 지키기 쉽게 구조화하는 것이 남은 과제다.
