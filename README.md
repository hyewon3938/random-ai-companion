...
매일 대화를 주고받는 서비스라 원문을 통째로 쌓아 두면 양이 계속 늘어나고, 그중 무엇이 지금 필요한 이야기인지 답장할 때마다 다시 찾아야 한다. 하루치 대화를 요약해 두는 방법도 그날 있었던 일만 남기기 때문에, 둘이 주고받은 표현이나 사소한 이야기처럼 사람같이 느껴지게 하는 디테일은 기억하지 못한다. 그래서 유저가 보낸 메시지 한 건에서 주제를 골라 그 주제의 기억을 꺼내고, 꺼낸 기억과 이어지는 맥락으로 답하게 했다.

대화에서 나온 주제를 골라 한 건씩 키를 붙여 저장하고, 다음 대화에서 같은 주제에 새로 나온 내용이 있으면 그 건의 값을 업데이트한다. 이 방식으로 매일 새벽에 모델이 그날 대화와 주요 데이터를 읽어 기억을 업데이트한다.

기억은 잘 바뀌지 않는 사실, 진행 중인 일, 주변 인물 3가지로 나누고 각각을 캐릭터 쪽인지 유저 쪽인지로 한 번 더 갈라, 총 6가지로 나뉘어 저장된다. 아래는 실제로 저장되는 모양을 보여주는 예시 3건이다.
