...
5. [유저의 한마디부터 답장까지](#5-유저의-한마디부터-답장까지)
6. [답변 품질 관측과 개선](#6-답변-품질-관측과-개선)
7. [시스템 구조와 기술 선택](#7-시스템-구조와-기술-선택)
8. [지금 상태와 남은 것](#8-지금-상태와-남은-것)
...
캐릭터와 유저가 서로의 일상을 주고받으려면 캐릭터가 유저에 대해 알게 된 것을 계속 들고 있어야 하고, 자기가 지금까지 한 말과 어긋나지 않는 사람으로 남아야 한다. 그래서 대화에서 나온 주제와 유저의 성향, 유저가 지나가듯 말한 일정을 골라 기억하는 것을 먼저 잡았다.

대화 원문을 통째로 쌓아 두고 필요할 때마다 거기서 찾는 방법도 있지만, 그러면 프롬프트가 대화량만큼 길어지고 답장 한 건을 만들 때마다 중요한 대목을 다시 골라내야 한다. 답장은 유저가 방금 보낸 짧은 메시지 하나에 답하는 일이라, 그 메시지에서 나온 주제에 걸리는 기억만 꺼내 넣는 편이 더 정확하겠다고 생각했다. 최근에 오간 말은 그대로 넣되 그보다 앞의 이야기는 정리된 기억으로 대신한다.

그래서 대화 내용을 주제별로 한 건씩 나눠 키를 붙여 저장하고, 같은 주제가 다시 나오면 새로 쌓지 않고 그 건의 내용을 업데이트하도록 구성했다. 무엇을 어떻게 남길지는 하루가 끝난 뒤 모델이 그날 대화를 다시 읽으며 정한다.

기억은 잘 바뀌지 않는 사실, 진행 중인 일, 주변 인물 3가지로 나누고 각각을 캐릭터 쪽인지 유저 쪽인지로 다시 나눠, 모두 6가지 자리에 저장한다. 기억 한 건은 이 저장 항목과 주인, 업데이트할 자리를 찾는 키, 내용, 검색에 쓰는 태그로 이뤄진다. 아래는 실제로 저장되는 모양을 보여주는 예시 3건이다.

| 저장 항목 | 주인 | 키 | 내용 | 태그 |
|---|---|---|---|---|
| 진행 중인 일 | 캐릭터 | 영화/오디세이 | 유저가 추천한 영화를 보려고 취소표를 알아보는 중 | 영화 · 오디세이 · 놀란 · 상영관 |
| 사실 | 유저 | 커피/취향 | 아메리카노만 마시고 단 커피는 안 마신다 | 커피 · 아메리카노 · 카페 |
| 주변 인물 | 유저 | 민석 | 대학 동기, 요즘 주말마다 같이 러닝을 한다 | 민석 · 러닝 · 대학 |

### 키

새로 저장할 내용과 같은 키를 가진 기억이 그 저장 항목 안에 이미 있으면 내용만 바꾸고, 없으면 새로 만든다. 한 저장 항목 안에서 같은 키는 하나뿐이라, 같은 이야기가 몇 번 나와도 기억은 한 건으로 유지된다.

키는 의미 범위가 넓은 단어를 앞에, 좁은 단어를 뒤에 놓고 슬래시로 이어 두 단어로 짓는다. 영화/오디세이, 가족/어머니, 회사/승진심사처럼 쓴다. 한 단어는 범위가 넓어 다른 기억과 겹치고 세 단어부터는 순서가 애매해져 같은 사실에 다른 키가 생기기 때문에, 두 단어로 고정했다. 주변 인물은 사람 이름 하나가 그대로 키가 된다.

키는 내용이 바뀌어도 그대로인 말로만 짓고, 바뀌는 사실은 그 키가 가리키는 내용 쪽에 넣는다. 키를 붙이는 모델에게는 지금 쓰고 있는 키 목록을 함께 줘서, 같은 이야기면 새로 짓지 않고 있는 키를 그대로 쓰게 한다. 표기가 조금 달라 업데이트할 자리를 못 찾고 같은 이야기가 두 건으로 갈라지는 것을 막는 장치다.

캐릭터를 만들 때 정한 이름·직업·고향은 같은 키로 새 내용이 와도 저장 코드가 업데이트를 거절한다. 프롬프트에 지시문으로 적어 두는 대신 저장 규칙으로 막아서, 대화 도중에 캐릭터가 다른 사람이 되지 않게 했다.

### 태그

답장을 만들기 전에 모델이 유저가 방금 한 말에서 주제를 뽑으면, 코드가 저장 항목을 가리지 않고 같은 태그가 붙은 기억을 모아 프롬프트에 넣는다. 키는 한 건에 하나지만 태그는 여러 개라서, 대화가 어느 주제로 흘러도 그중 하나만 맞으면 그 기억이 검색된다.

태그는 키를 이루는 두 단어가 그대로 들어가고, 내용에서 관련된 주제어를 더 뽑아 붙인다. 지난 일기와 예정된 일정에도 같은 어휘로 태그를 달아둬서, 민석이라는 이름 하나로 그 사람에 대한 기억과 함께 잡힌 일정, 그 사람이 나온 일기가 같이 검색된다.

기억을 저장하는 시점은 대화 중이 아니라 하루가 끝난 뒤다. 답장을 만든 모델이 오늘 다시 꺼낼 만한 말을 표시하면 그날 안에는 메모로 들고 있다가, 새벽 정리가 그 메모와 하루 대화를 다시 읽어 이 구조로 저장한다.
