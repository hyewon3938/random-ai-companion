...
- **절대 틀리면 안 되는 사실은 위치로 지킨다.** 층이 늘어날수록 LLM이 중간 정보를 잘못 읽는 일이 생겼다. 바뀌지 않는 사실(캐릭터의 이름·나이·직업)은 프롬프트 맨 위에, 매번 바뀌는 사실(지금 시각·현재 하는 일·말투)은 답장 직전에 읽는 맨 끝에 둔다. 특히 말투는 프롬프트 지시만으로는 존댓말로 회귀해서, 최근 발화 표본을 코드가 판정해 반말/존댓말을 값으로 주입한다.
...
| 종류 | 언제 | LLM |
|---|---|---|
| 아침 안부 | 그날 각본의 기상·출근 시각에 연동된 창 안, 유저가 그날 먼저 연락했으면 스킵 | 밤에 미리 작성 |
...
- **물러날 줄도 안다.** 운영 집계를 내보니 유저 발화가 없는 날에도 선톡이 평균 2통씩 나가고 있었다. 사람은 답 없는 상대에게 매일 같은 텐션으로 연락하지 않는다. 그래서 무응답 3일째부터는 모든 선제 연락을 멈추고 문안 생성 자체를 건너뛰다가, 2주째 되는 날 저녁에 근황을 묻는 한 통만 보낸다. 그래도 답이 없으면 유저가 돌아올 때까지 침묵하고, 답이 오면 그 즉시 평소로 돌아온다.
- **먼저 말을 걸지 말지는 한 곳이 정한다.** 채널이 다섯이 되자 보낼 수 있는가의 판단이 채널마다 흩어졌고, 캐릭터가 보낸 아침 안부를 '최근 대화'로 세어 예고가 예고를 부르는 구멍도 생겼다. 발송 허가(침묵 단계·하루 총량·동시 발송 방지)를 정책 모듈 하나로 모으고, 채널에는 각자의 트리거만 남겼다.
...
- **캐릭터에게 시간을 먼저 만들어줬다.** 3층 구조다. 아크(연·계절·월·주 흐름) → 월 리듬(한 달치 이벤트와 매일의 컨디션 시드) → 하루 각본(시간 블록). 컨디션을 매일 독립된 주사위로 굴리면 어제와 오늘이 이어지지 않는다. 그래서 한 달치를 미리 깔아 회식 다음 날은 피곤한 식의 인과가 성립하게 했고, 실제로 산 하루가 그 위를 덮어쓴다. 아크도 고정이 아니다 — 매주·매달의 경계에서 최근 일기를 반영해 단절 없이 이어 쓴다.
...
| `proactive-policy.ts` | 선제 연락 허가 판단. 무응답 기간으로 침묵 단계(평상·조용·재연결·침묵)를 계산한다 |
...
3. 컨텍스트 조립. 바뀌는 빈도 순서로 세 층을 쌓는다 — 캐릭터가 사는 한 그대로인 불변층(바이블·태도와 대화 규칙), 하루 단위로 굳는 일간층(유저에 대해 아는 것·주변 인물·일정·최근 일기), 매 응답마다 바뀌는 꼬리(오늘 각본의 현재 위치·지금 시각·말투). 아직 오지 않은 블록 중 캐릭터가 미리 알 수 없는 것은 가린다. 층 경계는 프롬프트 캐시 경계와 같다. 캐시가 살아 있는 동안 앞 두 층은 기본 요금의 십분의 일로 처리되는데, 실측으로 응답 한 번의 입력 약 1만 8천 토큰 중 1만 6천 토큰이 여기에 해당한다.
...
- **밤 정리의 이중 경로.** 기본 경로는 외부 스케줄러가 VM의 수집·적용 도구를 호출해 처리하고, 그게 돌지 않았을 때만 봇 안의 05:40 크론이 API로 대신 한다. 수집과 적용을 나눠 두 경로가 같은 쓰기 코드를 쓰게 했고, 일기가 이미 있으면 건너뛰어 이중 실행을 막는다. 적용은 트랜잭션 하나로 묶어 중간에 실패해도 반쪽짜리 상태가 남지 않고, 며칠 밀렸으면 결번 날짜를 오래된 것부터 소급해 채운다.
- **전송 실패는 흔적을 남긴다.** 선톡이 전송에 실패하면 시도 횟수와 마지막 에러를 행에 남기고, 발송 창이 지나도 90분의 유예 안에서는 늦게라도 보낸다. 정상적으로 건너뛴 것과 실패해서 못 보낸 것이 로그에서 구분된다.
- **연결을 데워둔다.** VM에서 새 HTTPS 연결 수립이 간헐적으로 실패해 선톡만 골라 유실된 적이 있다. 가장 가벼운 API 호출로 살아 있는 연결을 주기적으로 유지하고, 발송이 그 연결을 재사용하게 했다.
- **중복 방지.** 하루 한 통 가드, 일기 존재 확인, 포착 워터마크. 크론이 겹쳐 돌아도 같은 일을 두 번 하지 않는다.
- **로그.** 에러 로그에 토큰이 섞여 나가지 않도록 마스킹한다. LLM 호출량과 캐시 적중은 날짜별로 DB에 누적해, 로그를 뒤지지 않아도 비용 상태를 질의 한 줄로 본다.
...
- **선톡만 조용히 유실.** 운영 26일 중 11일의 아침 안부가 발송 창을 놓치고 버려졌는데, 로그에는 창을 놓쳤다는 사유뿐이었다. 파고들어 보니 경로 장애가 아니라 새 연결 수립의 간헐 실패였다. 롱폴링이 유일한 연결을 계속 점유하니 몇 시간 만에 나가는 선톡만 매번 새 연결을 맺었고, 대화 중 답장은 방금 쓴 연결을 재사용해 멀쩡했던 것이다. 연결 보온과 재시도, 유예 발송으로 고치고 실패가 흔적을 남기게 했다.
- **매일 오는 안부 → 물러나는 선톡.** 며칠째 답이 없어도 안부와 예고가 계속 나가는 게 비용 낭비이자 사람 같지 않은 지점이었다. 무응답 기간에 따라 단계적으로 조용해지는 백오프를 넣고, 발송 허가 판단을 정책 모듈 하나로 모았다.
- **프롬프트의 42%가 구조 오버헤드.** 관계 기록을 들여쓰기 JSON으로 통째로 넣고 있었다. 압축 서술로 바꾸고, 매번 바뀌는 값(시각)을 프롬프트 끝으로 옮겨 층 경계에 캐시를 걸었다. 응답 한 번의 입력 대부분이 캐시 요금으로 떨어졌다.
- **분 단위 시각 오인.** 12시 반인데 곧 12시라고 말하는 식으로, 모델이 숫자 시각에서 분을 흘려 읽었다. 정확히 읽으라는 지시로는 부족해서, 낮 12시 반쯤 같은 말 표현을 코드가 계산해 숫자와 함께 주입했다. 시각 산수를 모델에게 시키지 않는 것이 원칙이 됐다.
...
동작하는 것: 시간 흐름(아크·월 리듬·하루 각본), 각본 기반 답장 행동, 이어지는 기억, 밤 정리 배치, 다섯 종류의 선톡과 무응답 백오프, 3층 프롬프트 캐싱, 개발용 백스테이지 뷰와 신호 집계 스크립트.

### 더 검증이 필요한 것

- **규모.** 지금은 디테일이 촘촘한데, 유저가 늘면 매 턴 다시 실어 보내는 맥락이 비용의 병목이 된다. 관계가 오래될수록 유저 한 명당 비용이 우상향하는 구조이기도 하다. 코드 기반으로 한 번 검토한 내용은 [규모와 비용, 그리고 기억 설계](scaling.md)에 정리했다. 이 중 고정 맥락 캐시는 구현이 끝났다 — 프롬프트를 바뀌는 빈도 순 세 층으로 재배열하고 캐시 경계를 걸어, 실측으로 입력의 90%가 기본 요금의 십분의 일에 처리된다. 남은 방향은 요약 기반 기억, 모델 분리, 그리고 규칙으로 굳는 판단을 LLM에서 코드로 옮기는 것이다.
