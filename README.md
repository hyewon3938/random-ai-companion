...
키는 내용이 바뀌어도 그대로인 말로만 짓고, 바뀌는 사실은 그 키가 가리키는 내용 쪽에 넣는다. 같은 사실을 여러 키로 나눠 저장하는 것을 막으려고, 키를 붙이는 모델에게 지금 쓰고 있는 키 목록을 함께 넘겨 같은 기억이면 있는 키를 그대로 쓰게 했다.
...
답장을 만들 때 유저가 보낸 메시지에서 관련 주제를 뽑아, 그 주제들을 태그로 검색해 관련 기억을 가져온다. 태그는 기억 구조 밖에도 붙이는데, 캐릭터의 일정과 캐릭터가 매일 남기는 일기에 같은 어휘로 달아둬서 박민석이라는 이름 하나로 그 사람에 대한 기억과 함께 잡힌 일정, 그 사람이 나온 일기가 같이 검색된다.

키는 한 건에 하나지만 태그는 여러 개라, 대화가 어느 주제로 흘러도 그중 하나만 맞으면 그 기억이 검색된다. 태그에는 내용과 관련된 말을 넉넉히 붙이고 사람 이름은 반드시 넣는다.
