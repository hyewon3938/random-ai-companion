...
캐릭터는 자기 삶을 사는 중이라 유저가 연락한다고 언제나 즉답하지 않고, 그날 하루 일정을 기준으로 답장한다. 캐릭터를 만들 때 함께 생성해둔 하루 각본을 보고 지금 무엇을 하는 시간인지에 따라 답장 텀을 정한다.
...
각본의 시간대마다 태그를 두 종류 붙였다. 지금 메신저를 볼 수 있는지를 나누는 답장 여건(즉답 / 틈틈이 / 불가)과, 유저가 붙잡을 때 접을 수 있는 일인지를 나누는 활동 성격(개인 / 사회 / 공적)이다. 두 태그는 서로 영향을 주지 않아서 회의는 불가·공적, 러닝은 불가·개인, 카페에서 혼자 있는 시간은 즉답·개인이 된다.

답장을 어떻게 할지 일정마다 지정하면 일정 종류가 늘수록 관리하기 어려워져서, 두 태그의 조합 하나만 보고 정하게 했다. 새로 생긴 일정도 태그 두 개만 붙으면 답장 텀이 나오고, 텀을 손볼 일이 생겨도 아래 표 한 장만 고치면 된다.
...
- **텀은 코드가 계산한다.** 두 태그가 만나는 칸에서 구간을 고르고 그 안에서 매번 다른 값을 뽑는다. 규칙으로 정리되는 판단까지 LLM에 맡기면 매 턴 비용이 들고, 그렇게 해도 잘 지켜지지 않는다.
- **답장할 수 없는 시간에는 답장을 만들지 않는다.** 회의나 잠처럼 폰을 볼 수 없는 시간에 온 메시지는 그때 답을 만들어 쥐고 있지 않고, 그 일정이 끝나는 시각에 깨어나 쌓인 메시지를 한 번에 읽고 답한다. 만드는 시점과 보내는 시점이 같아서 일정을 막 끝내고 이제 봤다는 말이 실제와 맞는다.
- **유저가 붙잡으면 일정을 조정한다.** 답장이 어려운 시간에 유저가 붙잡는 말을 하면, 지금 하는 일 한 줄과 유저의 마지막 말만 넘기는 짧은 호출로 붙잡는 말인지만 가른다. 붙잡혔으면 개인 일정은 취소하고 사회 일정은 양해를 구해 미룬 것으로 그날 실제 기록에 적고, 그때부터 그 시간은 즉답으로 바뀐다. 공적 일정은 취소할 수 없고, 밤에 자다가 연락이 와도 깬다.
- **유저 쪽 리듬에도 맞춘다.** 유저가 말을 나눠 보내면 다 올 때까지 기다렸다 한 번에 답한다. 이 대기 시간은 그 유저가 메시지를 이어 보낼 때의 간격을 모아 그중 80%가 들어오는 값에 1.4배 여유를 둬 20초에서 40초 사이로 정한다. 사람마다 타이핑 속도가 다르므로 고정값으로는 말을 끊거나 너무 오래 기다린다.
- **캐릭터의 발화도 나눠서 보낸다.** 한 메시지에 길게 담아 보내면 사람이 보낸 말처럼 읽히지 않아서, 할 말을 최대 6개 말풍선으로 쪼갠 뒤 글자 수에 비례한 타이핑 시간을 두고 순차 전송한다.

### 선톡

캐릭터가 먼저 거는 연락을 선톡이라 부른다. 유저가 먼저 찾을 때만 반응하면 관계보다 도구에 가깝고, 그렇다고 매일 같은 시각에 알림이 오면 사람이 아니라 알람처럼 느껴져서, 말을 걸 거리가 있을 때만 그날 각본에서 시각을 가져와 보낸다. 걸 거리는 각본과 저장해둔 기억에서 나오는데, 각본에는 캐릭터가 언제 무엇을 하는지가 있고 기억에는 유저와 나눈 이야기 중 다시 꺼낼 만한 것이 들어 있다.

선톡은 5종류로 나눠 트리거를 다르게 뒀다. 종류마다 판단이 얼마나 필요한지가 달라서 LLM을 부르는 시점도 다르다.
...
- **연락 시각을 그날 각본에서 가져온다.** 아침 안부는 발송 시각을 고정하지 않고 그날 각본의 기상·출근 시각에 창을 맞춰서, 늦잠 잔 날은 늦게 나가고 일찍 일어난 날은 일찍 나간다. 자리 비움 예고도 그날 일정에서 나오므로 매일 다른 시각에 걸린다. 시각이 아니라 유저가 답 없는 시간으로 재는 것은 다시 말 걸기 하나인데, 임계값은 4시간으로 뒀다. 2시간은 낮에도 흔한 간격이라 답이 늦은 것과 대화가 끝난 것을 가르지 못한다.
...
- **답이 오래 끊기면 연락을 멈춘다.** 사람이라면 답 없는 상대에게 매일 같은 텐션으로 연락하지 않으므로, 무응답 3일째부터는 선제 연락을 멈추고 문안을 만드는 일도 건너뛴다. 그러다 2주째 되는 날 저녁에 근황을 묻는 한 통을 보내고, 그래도 답이 없으면 유저가 돌아올 때까지 조용히 있다가 답이 오는 순간 평소로 돌아온다.

이 중에서 자리 비움 예고는 사람다움보다 서비스 쪽 이유가 크다. 말없이 답이 끊기면 유저는 고장을 의심하게 되므로, 나가기 전에 미리 알리고 돌아와서 먼저 말을 건넨다. 답이 늦어도 유저가 이유를 알면 캐릭터가 자리를 비운 것으로 받아들이고 기다린다.
