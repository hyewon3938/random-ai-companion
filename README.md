...
캐릭터를 만들 때 생성한 데이터도 전부 이 기억 구조에 저장한다. 그중 생성 시점에 만든 기억은 캐릭터의 정체성을 지탱하기 때문에 저장 코드가 대화 중의 업데이트를 거절해, 이야기를 나누다가 캐릭터가 다른 사람이 되지 않게 했다.
...
태그는 기억뿐 아니라 캐릭터의 일정과 매일 남기는 일기에도 붙인다. 세 곳에 같은 어휘를 쓰기 때문에 박민석이라는 주제 하나로 그 사람에 대한 기억과 그 사람과 잡힌 일정, 그 사람이 나온 일기가 함께 검색된다.
