...
매일 대화를 주고받는 서비스라 원문을 통째로 쌓아 두면 양이 계속 늘어나고, 그중 무엇이 지금 필요한 이야기인지 답장할 때마다 다시 찾아야 한다. 하루치 대화를 요약해 두는 방법도 그날 있었던 일만 남아서 유저와 무엇을 주고받으며 지내 왔는지는 흐려진다. 그래서 유저가 보낸 메시지 한 건에서 주제를 골라 그 주제의 기억을 꺼내고, 꺼낸 기억과 이어지는 맥락으로 답하게 했다.

대화 내용은 주제별로 한 건씩 나눠 키를 붙여 저장하고, 같은 주제가 다시 나오면 새로 쌓지 않고 그 건의 내용을 업데이트한다. 무엇을 남길지는 매일 새벽에 모델이 그날 대화와 주요 데이터를 다시 읽고 정한다.
