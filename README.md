...
이 조건을 구현 가능한 축으로 좁히면 셋이다. **기억이 끊기지 않을 것**, **먼저 말을 걸되 근거가 있을 것**, **늘 대기하지 않고 자기 리듬으로 존재할 것.** 이 문서의 3~5장이 각각 그 하나씩이다.
...
**결과.** 자리를 비우기 전에 알리고 돌아와서 알리므로, 답이 없는 시간이 막연한 침묵이 아니라 알고 하는 기다림이 된다.
