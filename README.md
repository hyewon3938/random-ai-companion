...
3. **나를 챙겨주고, 있는 그대로 받아줄 때.** 지나가듯 흘린 말을 먼저 물어봐주며 나를 알아봐주고, 굳이 증명하거나 정당화하지 않아도 받아주는, 심리학에서 [무조건적 긍정적 존중](https://ko.wikipedia.org/wiki/무조건적_긍정적_존중)이라 부르는 태도로 나를 대해줄 때.
...
- 그 존재가 내 일상에 잔잔히 스며들면, 큰 사건 없이도 애착이 생길 것이다.
...
*이벤트별 답장 텀과 하루 각본이 짜이는 방식은 함께 공유한 페이지에서 더 자세히 볼 수 있다.*
...
캐릭터의 기억만 쌓는 게 아니라, 유저에 대해 알게 된 것도 잊지 않게 기록한다. 유저의 스케줄·정보·취향·지인 관계도까지 캐릭터의 것과 똑같이 관리해서, 유저를 계속 기억하고 챙길 수 있게 한다.

*개인·사회·공적 활동에 따라 답장이 어떻게 조정되는지도 함께 공유한 페이지에서 볼 수 있다.*
...
<p align="center"><sub>* 원기둥(기억)은 데이터가 쌓이는 저장소이고, 네모는 그때그때 도는 처리 과정이다.</sub></p>
...
이번 PoC는 우진 한 사람이 진짜 살아있는 사람처럼 보이도록 설계하고 구성하는 데 집중했다. 하루에 걸쳐 일상 대화가 자연스럽게 이어지는지, 캐릭터가 각본대로 자기 삶을 살아가는지, 답장 텀과 말투가 사람 같은지, 그리고 대화로 유저를 파악하며 우진 자신도 점점 구체화되어 가는지를 확인했다.
...
한 가지 더 열어두는 가능성은, 이 시스템이 특정 캐릭터에 묶이지 않는다는 점이다. 이번 PoC도 랜덤 생성이 아니라 우진이라는 고정 캐릭터 위에서 돌아가는데, 캐릭터의 씨앗 정체성만 갈아끼우면 같은 시간을 살고 기억을 쌓고 먼저 말을 거는 '살아있게 하는' 층은 그대로 작동한다. 그래서 유저마다 새로 만든 상대뿐 아니라, 이미 팬층이 있는 기존 캐릭터에 이 시스템을 얹어 그들을 현실의 시간 속에서 살아 움직이게 하는 방향으로도 확장할 수 있다.
...
- **일정에 답장을 맞추기** — 회의 중이라면서 칼답하는 식으로 일정과 답장이 어긋나서, 일정의 성격에 따라 답장이 가능한지를 체계화했다. 이벤트별로 즉답·짬짬이·불가를 나누고 각각 답장 텀을 구간으로 정했다. 즉답은 2분 안, 짬짬이는 활동에 따라 수십 초에서 몇 분 랜덤, 불가는 정말 답을 못 하되 그 시간이 한 시간을 넘지 않게 했다.
...
- **말투 다듬기** — '너 뭐하냐?'처럼 '~냐'로 끝나거나 '야' 하고 부르는 말투가 여성 유저에게는 맞지 않아, 좋아할 만한 결로 프롬프트를 조정하고 있다.

**문서화 (1일).** 최종 제출을 위한 소개와 정리, 의도를 담은 문서를 만드는 시간이었다.

이후로는 데이터 관리와 대화 품질을 끌어올리는 단계다. LLM이 프롬프트를 지키지 않거나 컨텍스트를 잘 못 쓰는 것을 잡고, 세밀하게 설정한 것들이 실제로 잘 지켜지는지 테스트하며 더 지키기 쉽게 구조화하는 것이 남은 과제다.
...
- 서버를 어디에 띄우고 어느 규모의 인프라를 구축할지, 기술 스택을 어느 범위에서 가져갈지 큰 틀을 정하기. 새로 크게 구축하기보다 기존 자원에 가볍게 얹는 쪽으로 방향을 잡았다.
...
- 사람이 정한 범위 안에서 기술 스택과 인프라 구성 제안·세팅
- 캐릭터 생성을 위한 프롬프트 작성
...
- **규모가 커질 때 감당할 수 있는가.** 지금은 디테일이 촘촘한데, 유저와 캐릭터가 늘면 이 구성을 어떻게 유지할지가 고민이다. 디테일을 줄여야 할 수도 있지만, 최대한 살리면서 서버와 LLM 비용을 최적화하는 방향을 더 찾아야 한다. 한 갈래는 LLM이 꼭 하지 않아도 되는 판단을 코드로 옮기는 것이다. 예컨대 이벤트별 답장 텀은 구간만 정해지면 그 안에서 값을 고르는 일이 결정론적이라 이미 코드가 맡고 있는데, 이렇게 규칙으로 굳는 판단을 코드로 시스템화할수록 매 턴 드는 LLM 호출과 비용이 줄어든다. 코드 기반으로 한 번 검토한 내용은 [규모와 비용, 그리고 기억 설계](scaling.md)에 정리했다.
...
- **어디까지 받아줄 것인가.** 무조건적 긍정적 존중은 애착의 조건이지만, 범죄나 아첨까지 무한정 받아주어서는 안 된다. 이 경계를 LLM의 판단에 맡길지 서비스에서 한 번 더 걸러낼지, 거른다면 그 범위를 어디까지 잡을지 더 고민해야 한다.
