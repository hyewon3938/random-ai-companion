...
규칙층을 고쳤을 때 답장이 실제로 달라지는지는 배포하고 며칠 동안 대화를 읽어야 알 수 있었다. 웃음 표기를 정말 웃을 때만 쓰도록 고쳤을 때도 ㅎㅎ가 줄었는지 무뚝뚝해졌는지를 눈으로 셌다. 그래서 규칙을 어기기 쉬운 상황 8가지를 케이스로 고정해두고, 프롬프트를 고친 자리에서 바로 돌려 준수율을 보게 했다.

케이스는 상대가 속상해하는 자리, 다정한 말을 하는 자리, 직전 답장에서 이미 웃은 자리처럼 규칙마다 하나씩 둔다. 실행할 때는 운영과 같은 프롬프트 조립 코드를 그대로 쓰고 DB만 평가 전용 파일로 갈아 끼운다. 규칙층을 베껴두면 규칙을 고쳐도 평가는 옛 문안을 재고 통과하며, 반대로 대화 기록과 관계가 매일 달라지는 운영 DB로 재면 프롬프트를 고치지 않은 날에도 숫자가 움직여 무엇이 바꾼 것인지 가려낼 수 없다.

채점은 모델에게 맡기지 않고 글자로 센다. 이모지와 큰따옴표, 목록 표기가 있는지 보고 웃음 표기가 몇 번 나왔는지 세며, 답장이 정해진 JSON 형식으로 왔는지도 함께 본다. 채점까지 모델에게 시키면 채점자가 흔들려서 프롬프트를 고친 몫과 채점 잡음이 섞인다. 채점기가 아무것도 못 잡으면 준수율은 늘 100%로 나오기 때문에, 위반이 있는 답과 없는 답 12건을 따로 두고 채점기가 그 둘을 가려내는지 먼저 검사한다. 준수율이 하한에 못 미치면 종료 코드 1로 끝낸다.
