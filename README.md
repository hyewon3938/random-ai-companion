...
이 문서는 **AI가 사람처럼 느껴지려면 무엇이 필요한지**를 문제로 놓고, 어떤 판단으로 풀었는지 정리한 기록이다. 기획·설계·구현·배포·운영을 혼자 했고, 배포한 뒤로는 직접 유저가 되어 쓰고 있다.

## 목차

1. [한눈에](#한눈에)
2. [왜 사람처럼 느껴져야 하나](#왜-사람처럼-느껴져야-하나)
3. [언제 사람처럼 느껴지나](#언제-사람처럼-느껴지나)
4. [설계 1. 기억이 끊기지 않는다](#설계-1-기억이-끊기지-않는다)
5. [설계 2. 먼저 말을 건다](#설계-2-먼저-말을-건다)
6. [설계 3. 늘 곁에 있지는 않다](#설계-3-늘-곁에-있지는-않다)
7. [무엇으로 성공을 볼 것인가](#무엇으로-성공을-볼-것인가)
...
텔레그램 봇 하나에 실시간 대화와 밤 배치 두 층이 돌아간다. 초기 구축에 5일이 걸렸고, 그 뒤로는 매일 쓰면서 어색한 지점을 찾아 설계로 고치고 있다. 풀어야 했던 문제는 셋이었다.

| 풀어야 했던 것 | 어떻게 | |
|---|---|---|
| 며칠 만에 말을 걸어도 이어질 것 | 원문 보관과 프롬프트 주입을 분리하고, 기억을 층으로 나눠 주기와 사용처를 다르게 둠 | [설계 1](#설계-1-기억이-끊기지-않는다) |
| 먼저 말을 걸되 스팸이 되지 않을 것 | 선톡을 네 종류로 나눠 LLM 개입 시점을 다르게 두고, 규칙성을 코드로 깸 | [설계 2](#설계-2-먼저-말을-건다) |
| 즉답하지 않되 이유가 있을 것 | 캐릭터에게 시간을 먼저 만들어주고, 답장 텀을 그날 각본에서 파생 | [설계 3](#설계-3-늘-곁에-있지는-않다) |
...
이 조건을 구현 가능한 축으로 좁히면 셋이다. **기억이 끊기지 않을 것**, **먼저 말을 걸되 근거가 있을 것**, **자기 일상이 있어 늘 대기하지 않을 것.** 이어지는 세 장이 각각 그 하나씩을 다룬다.
...
- **캐릭터에게 시간을 먼저 만들어줬다.** 아크(연·계절·월·주 흐름) → 월 리듬(한 달치 이벤트와 매일의 컨디션 시드) → 하루 각본(시간 블록) 세 층으로 쌓는다. 컨디션을 매일 독립된 주사위로 굴리면 어제와 오늘이 이어지지 않는다. 그래서 한 달치를 미리 깔아 회식 다음 날은 피곤한 식의 인과가 성립하게 했고, 실제로 산 하루가 그 위를 덮어쓴다.
...
- **블록에 직교하는 두 태그를 붙였다.** 답장 여건(즉답 / 짬짬이 / 불가) × 활동 성격(개인 / 사회 / 공적). 두 축은 독립이다. 회의는 불가·공적, 러닝은 불가·개인, 카페에서 혼자 있는 시간은 즉답·개인이 된다. **행동은 이 두 태그에서 파생시키고 이벤트별 예외처리는 금지했다.** 이벤트마다 분기를 붙이기 시작하면 종류가 늘어날수록 손댈 수 없어진다.
...
구조적으로 열어둔 가능성이 하나 더 있다. 이 시스템은 특정 캐릭터에 묶여 있지 않아서, 씨앗 정체성만 갈아끼우면 같은 시간을 살고 기억을 쌓고 먼저 말을 거는 층은 그대로 돈다. 이미 팬층이 있는 기존 캐릭터 위에 이 층을 얹는 방향으로도 확장할 수 있다.
