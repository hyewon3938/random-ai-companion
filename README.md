...
며칠 만에 말을 걸어도 지난 이야기가 이어지려면 나눈 내용이 남아 있어야 하는데, 대화 원문을 통째로 쌓아 매번 프롬프트에 넣으면 길이와 비용이 대화량만큼 늘어난다. 그래서 대화에서 나온 내용을 주제별로 한 건씩 나눠 저장하고, 같은 주제가 다시 나오면 새로 쌓지 않고 그 건의 내용을 업데이트한다. 한 건마다 키와 태그를 붙여서, 저장할 때는 키로 업데이트할 자리를 찾고 대화 중에는 태그로 관련 있는 기억을 찾는다.

기억은 잘 바뀌지 않는 사실, 진행 중인 일, 주변 인물 3가지로 나누고 각각을 캐릭터 쪽 이야기인지 유저 쪽 이야기인지로 다시 나눠, 모두 6가지 자리에 저장한다.
...
| 주인 | 캐릭터 |
| 키 | 영화/오디세이 |
| 내용 | 유저가 추천한 영화를 보려고 취소표를 알아보는 중 |
| 태그 | 영화 · 오디세이 · 놀란 · 상영관 |
