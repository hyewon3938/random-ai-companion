...
매일 대화를 주고받는 서비스라 원문을 통째로 쌓거나 대화를 요약한 기억을 계속 더해 가면, 오래 대화할수록 데이터가 과도하게 쌓인다. 그렇다고 지난 기억을 지우고 최근 것만 남기면 사람과 이야기하는 느낌이 나지 않는다. 일상에서 매일 대화하며 유대와 애착을 쌓아 가는 것이 목표라 시간이 지날수록 기억이 더 풍부해져야 했고, 그래서 주제를 기준으로 기억을 저장하고 바뀐 내용이 나올 때마다 그 주제의 기억을 업데이트하는 방식을 택했다.

주제마다 키를 붙여 한 건씩 저장하고, 매일 새벽에 모델이 그날 대화와 주요 데이터를 읽어 새로 나온 내용을 그 키의 값에 업데이트한다. 저장할 때 내용과 관련된 단어를 태그로 함께 붙여, 나중에 그 기억을 태그로 검색할 수 있게 했다.
...
태그는 기억을 검색하려고 붙인 것이라, 답장을 만들 때 유저가 보낸 메시지에서 관련 주제를 뽑아 그 주제들을 태그로 검색해 관련 기억을 가져온다. 검색은 저장 항목을 가리지 않아서, 같은 태그가 붙어 있으면 사실이든 진행 중인 일이든 함께 딸려 온다.

키는 한 건에 하나지만 태그는 여러 개라, 대화가 어느 주제로 흘러도 그중 하나만 맞으면 그 기억이 검색된다. 태그에는 내용과 관련된 말을 넉넉히 붙이고 사람 이름은 반드시 넣는다. 지난 일기와 예정된 일정에도 같은 어휘로 태그를 달아둬서, 박민석이라는 이름 하나로 그 사람에 대한 기억과 함께 잡힌 일정, 그 사람이 나온 일기가 같이 검색된다.
