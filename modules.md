...
| user-state.ts | 최근 대화로 상대가 지금 어떤 상태인지 판정, 앞서 판정한 값과 다를 때만 관계 행에 저장. 태그 고르기와 나란히 돈다 | 상대 상태 판정 1번 |
...
| reply-signal.ts | 모델에 시킬 출력 형식과 그 답을 본문과 신호로 가르는 읽기 | 없음 |
...
| reply-trace.ts | 호출 기록을 읽어 답장 한 건의 근거(답한 유저 발화 · 답장 텀이 나온 길 · 검색한 태그와 기억)를 게시함에 기록 | 1분 간격 | 없음 |
...
- 프롬프트 조립이 불변층의 관계 절 앞에 공통 틀과 현재 단계 블록을 넣고, 일간층에 「지금 관계」 블록을 채운다. 블록 문안은 `prompts/relationship.ts`, 채우는 코드는 `context/relationship.ts`가 맡는다.
- 답장 신호 읽기가 쓴 수·처음·일정 말함 3항목을 더 받는다. 쓴 수와 일정 말함은 캐릭터 답장 행의 meta_json에 적고, 처음은 `firsts` 표에 미확정 행으로 바로 넣는다.
- 상대 상태 판정 호출이 열림 신호 4항목을 더 돌려주고, 코드가 `relationship_signals`에 턴마다 1행을 적는다.
- 답장 게시에 관계 줄과 열림 줄이 붙고, 처음이 표시되면 `first_event` 게시가 따로 나간다.

### 선톡 경로

- 선톡 정책이 근거 종류 4개(의도·일정·달래기·약속)와 단계별 하루 상한으로 답한다. 하루 합계 상한 상수 하나는 단계별 표로 바뀐다.
- 선톡 틱에 의도 선톡과 틈새 한 줄이 더해진다. 의도 선톡은 오늘 의도 줄 중 아직 안 쓴 것 하나를 상황 문단에 넣고, 틈새 한 줄은 불가 블록 안에서 유저 말에 한 마디만 받는다. 근황과 밤 인사 문안도 의도 줄을 상황 문단에 받는다.
- 자리 비움 예고는 하루 2번까지로 줄고, 복귀 문안은 무엇을 했는지 대신 상대의 마지막 말을 받는다.
- 선톡 게시에 근거 줄이 붙는다.

### 온보딩 경로

- 물음이 5개에서 8개로 늘어 말투, 원하는 방식, 약한 구석을 받는다.
- 생성 호출이 원하는 방식과 결점을 정체성 값으로 만들고, 관계 행의 단계를 1로 시작하며, `character_start` 게시가 나간다.
