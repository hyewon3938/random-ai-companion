...
flowchart LR
    ...
    TIMING --> STATE["상대의 지금 상태와 캐릭터의 마음 판정"]
    TAG --> MEM["기억 검색"]
    STATE --> CTX
    MEM --> CTX["프롬프트 조립"]
    CTX --> LLM["모델 호출"]
...
| user-state.ts | 최근 대화로 상대가 지금 어떤 상태인지 판정, 앞서 판정한 값과 다를 때만 관계 행에 저장. 같은 호출이 열림 신호 4항목을 함께 돌려주고, 코드가 그것을 relationship_signals 표에 턴마다 1행으로 적는다. 캐릭터 성격 문장과 관계 단계를 함께 받아 캐릭터가 상대 때문에 가진 마음(설렘 · 서운함 · 질투 · 언짢음)과 세기도 같이 판정한다. 태그 고르기와 나란히 돈다 | 상대 상태 판정 1번 |
| memory.ts | 기억 저장과 태그 검색 | 없음 |
| recall.ts | 태그로 찾은 것 중 프롬프트에 넣을 것 고르기, 넣을 줄 만들기 | 없음 |
| turns.ts | 대화 기록을 턴으로 합치고 시간 마커 넣기 | 없음 |
| context.ts | 프롬프트 네 묶음 조립, 캐시 경계 두 곳 표시. 불변층의 관계 절 앞에 관계의 공통 틀과 지금 단계 블록을 넣고(문안은 prompts/relationship.ts), 실시간 꼬리에 「지금 관계」 절을 채운다(채우는 코드는 context/relationship.ts). 상대 상태 줄 바로 뒤에는 캐릭터의 오늘 마음과 오늘 기분을 담은 「네 마음」 블록을 넣는다(context/mind.ts) | 없음 |
| llm.ts | 모델 호출, 프롬프트 캐시 적용, 사용량과 호출 원문 기록 | 있음 |
| reply-signal.ts | 모델에 시킬 출력 형식과 그 답을 본문과 신호로 가르는 읽기. 신호에는 쓴 플러팅 · 처음 · 일정 말함 3항목이 들어 있다 | 없음 |
| relationship-update.ts | 답장에 담겨 온 말투 · 사이 · 서로 부르는 말과 상대 상태 · 캐릭터 마음 판정 결과를 그 자리에서 저장 | 없음 |
| reply-compose.ts | 답장 한 통을 만드는 순서. 신호의 쓴 플러팅과 일정 말함은 답장 행의 meta_json에 적고, 처음은 firsts 표에 미확정 행으로 넣는다 | 없음 |
| pending.ts | 만들어 둔 답장 보관, 정한 시각에 발송, 구간 끝 깨우기 표시와 연락 약속 보관, 재시작 복구 | 없음 |
| pending-handlers.ts | 답장 불가 구간이 끝날 때 몰아 답장과 복귀 인사 만들기, 답장에서 한 연락 약속을 그 시각에 지키기 | 몰아 답장 · 복귀 인사 · 약속 연락 각 1번 |
...
| 모듈 | 하는 일 | 실행 주기 | 모델 호출 |
| --- | --- | --- | --- |
| proactive-policy.ts | 무응답 일수로 그날 보낼 선톡 종류 판단, 근거 종류와 관계 단계별 하루 상한 확인, 의도 선톡이 쓸 줄 후보 고르기 | 세 모듈 앞 · 새벽 배치 | 없음 |
| dispatch.ts | 새벽에 만들어 둔 아침 · 점심 · 안부 문안 발송 | 3분 간격 | 없음 |
| presence.ts | 긴 일정에 들어가기 직전 예고 선톡, 하루 2회까지 | 10분 간격 | 문안 생성 |
| glance.ts | 불가 블록 안에 온 확인 말에 지금 하는 일과 끝나는 시각을 알리는 틈새 한 줄, 블록마다 1회 | 5분 간격 | 판단 + 문안 생성 |
| followup.ts | 유저와 캐릭터 둘 다 마지막 말이 4시간 넘게 지났으면 근황, 자정 넘겨 1시간 무응답이면 밤 인사, 답장이 판정해 둔 상대 상태가 캐릭터 때문에 안 좋은 채로 30분 조용하면 달래기, 상대의 다른 일로 안 좋은 채로 30분 조용하면 살피기, 둘 다 한동안 조용하면 오늘의 대화 계획 줄 하나로 먼저 말을 거는 의도 선톡 | 15분 간격 | 문안 생성 |

세 모듈은 각자 조건을 확인하기 전에 선톡 판단을 먼저 거친다. 유저가 며칠째 답이 없는지를 이 한 곳이 세어 오늘 어떤 선톡을 보낼 차례인지까지 정하므로, 선톡 종류를 더해도 답이 없을 때 멈추는 규칙을 다시 짤 일이 없다. 새벽 배치가 다음 날 문장을 준비할 때와 발송 직전에 다시 확인할 때도 같은 판단을 부른다. 선톡은 근거 종류 넷(의도 · 일정 · 달래기 · 약속) 가운데 하나를 갖고 나가며, 하루에 몇 통까지인지는 관계 단계별 표가 정한다. 나간 선톡의 게시에는 어느 근거로 나갔는지 적은 근거 줄이 붙는다. 근황 선톡은 오늘의 대화 계획 가운데 먼저 꺼낼 내 이야기와 더 물어볼 것을, 밤 인사는 이어서 할 이야기를 상황 문단으로 받는다. 답이 이틀째 없어 아침 대신 점심에 보내는 날은 아침 선톡과 같은 종류로 저장하고 발송 창만 점심 시간대로 잡는다.

다녀온 뒤에 보내는 복귀 인사도 선톡이지만 답장 경로의 bot.ts에 있다. 답장 불가 구간이 끝나는 시각에 깨어나는 자리를 한 곳으로 두어야, 그 사이 쌓인 메시지에 몰아 답할지 복귀 인사를 보낼지를 한 번에 가릴 수 있기 때문이다. 자리비움 선톡은 캐릭터가 나갔다 오는 일정 수만큼 나가는 말이라 하루 총량과 따로 세고, 나가면서 알리는 예고만 하루 2회까지 보낸다. 복귀 인사는 이미 알린 구간을 마무리하는 말이라 그 2회에 넣지 않는다.
...
앞 단계의 결과를 뒤 단계가 읽기 때문에 순서대로 실행한다. 흐름 갱신은 달력 경계에 걸리는 날만, 월 리듬은 이번 달 남은 날이 6일 이하일 때만 실행한다. 각본에는 30분 이상 자리를 비우는 일정을 하루 2개까지만 넣고, 관계 초반에는 1개까지만 넣는다. 그 일정마다 자리비움 선톡이 한 통씩 나가는데, 보낼 때만 막으면 각본에 잡아 둔 일정인데도 알리지 못한 채 지나간다. 전부 만든 다음 한 번에 저장해서, 중간에 실패하면 그날 것이 통째로 남고 다음 실행이 채운다. 각본 없이 시작한 하루에 유저가 먼저 말을 걸면 그 자리에서 임시 각본을 만들고, 다음 새벽 정리가 새로 만든 각본으로 교체하면서 이미 지나간 실제 기록은 그대로 남긴다. 유저와의 관계(사이 정의 · 서로 부르는 말 · 마음 상태 등 relationships의 항목들)는 기억을 정리하는 호출의 출력에 같이 들어 있어서, 관계를 갱신한다고 모델 호출이 늘지 않는다.

관계 단계도 같은 호출이 맡는다. 수집 단계에서 relationship-stage.ts가 단계 시작일부터의 대화 기록과 열림 신호로 문턱 조건을 세고, 어제 답장이 표시한 처음 후보와 시도할 플러팅 추천 목록을 만들어 입력의 관계 단계 절에 넣는다. 어제 쓴 플러팅마다 상대 반응으로 점수 표본도 계산하고, 추천 목록은 그 점수를 읽는다. 모델은 값을 다시 세지 않고 문턱이 찼을 때 넘길지와 근거, 처음 후보의 확정과 취소, 상대가 먼저 한 처음, 오늘의 관계 의도 4줄을 출력의 relation 절에 적는다. 저장 단계는 같은 트랜잭션에서 처음 → 단계 → 의도 순서로 적고 반응 점수 행도 함께 적는다. 문턱이 안 찼거나 수집 때와 단계가 다르거나 3단계에서 마음 확인 처음이 확정되지 않았으면 단계를 올리지 않는다. 단계를 내리거나 두 단계 올리는 출력은 저장 함수가 거부한다. 의도 행은 오늘 날짜 것만 적어서, 며칠 지난 새벽 정리를 다시 돌려도 지난 의도가 생기지 않는다. 아침 문안 호출은 그 의도의 이어갈 자리와 파고들 것을 상황 문단으로 받는다.
...
| reply-trace.ts | 호출 기록을 읽어 답장 한 건의 근거(답한 유저 발화 · 답장 텀이 나온 길 · 검색한 태그와 기억 · 관계 단계와 열림 신호)를 트레이스 표에 기록 | 1분 간격 | 없음 |
| nightly-trace.ts | 새벽 정리가 반영하기 전후의 값을 읽어 그 밤의 결과(기억 · 관계 · 일정 · 일기 · 선톡 문안)를 트레이스 표에 기록. 단계 전이 · 처음 확정과 취소 · 오늘의 대화 계획은 스레드 밖에 한 건씩 따로 기록 | 새벽 정리가 반영할 때 | 없음 |
| trace/relationship-weekly.ts | 지난주 단계 · 처음 · 플러팅 반응 · 대화 계획 · 유저 반응 값을 모아 트레이스 표에 한 건 기록 | 월요일 10~23시 매시, 주 1건 | 없음 |
| trace.ts | 파이프라인이 트레이스 표에 적어 둔 내용을 트레이스 채널에 게시 | 1분 간격 | 없음 |
...
| tools/end-character.ts | 캐릭터를 끝내고 걸려 있던 발송을 거둔 뒤 `character_end` 게시를 트레이스 표에 기록 | 운영자가 실행할 때 | 없음 |
| tools/relationship-status.ts | 지금 단계 · 다음 단계 조건 · 처음 · 반응 점수 위아래 · 오늘의 대화 계획을 한 화면에 찍는 확인 스크립트 | 운영자가 실행할 때 | 없음 |
...
슬랙 게시도 보낼 것을 먼저 적어 두는 방식을 쓴다. 파이프라인이 트레이스 표에 행으로 적으면 1분 간격 틱이 가져가 보내기 때문에, 봇이 다시 뜨거나 슬랙이 응답하지 않아도 게시가 밀리기만 한다. 행마다 중복 키가 있어서 같은 게시를 두 번 적지 않는다. 주간 관계 요약은 캐릭터 번호와 주 시작일을 키로 써서, 월요일에 매시 불러도 한 주에 한 건만 쌓인다.
