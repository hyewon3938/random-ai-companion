...
관계 단계도 같은 호출이 맡는다. 수집 단계에서 relationship-stage.ts가 단계 시작일부터의 대화 기록과 열림 신호로 문턱 조건을 세고, 어제 답장이 표시한 처음 후보와 시도할 수 추천 목록을 만들어 입력의 관계 단계 절에 넣는다. 모델은 값을 다시 세지 않고 문턱이 찼을 때 넘길지와 근거, 처음 후보의 확정과 취소, 상대가 먼저 한 처음, 오늘의 관계 의도 4줄을 출력의 relation 절에 적는다. 저장 단계는 같은 트랜잭션에서 처음 → 단계 → 의도 순서로 적고, 문턱이 안 찼거나 수집 때와 단계가 다르거나 3단계에서 마음 확인 처음이 확정되지 않았으면 단계를 올리지 않는다. 단계를 내리거나 두 단계 올리는 출력은 저장 함수가 거부한다. 의도 행은 오늘 날짜 것만 적어서, 며칠 지난 새벽 정리를 다시 돌려도 지난 의도가 생기지 않는다. 아침 문안 호출은 그 의도의 이어갈 자리와 파고들 것을 상황 문단으로 받는다.
...
- 수집이 어제 쓴 수마다 반응 점수 표본을 계산하고, 저장 단계가 같은 트랜잭션에서 반응 점수 행을 적는다. 시도할 수 추천 목록은 그 점수를 이미 읽는다.
