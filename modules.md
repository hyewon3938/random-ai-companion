...
| relationship-update.ts | 답장에 담겨 온 말투 · 사이 · 서로 부르는 말을 그 자리에서 저장 | 없음 |
...
| followup.ts | 유저와 캐릭터 둘 다 마지막 말이 4시간 넘게 지났으면 근황, 자정 넘겨 1시간 무응답이면 밤 인사, 서운해한 뒤 30분 조용하면 달래기 | 15분 간격 | 문안 생성 |
...
없다. V2 구조로 옮기면서 대신할 자리가 생긴 표와 컬럼을 읽고 쓰던 코드는 이슈 #156에서 지웠고, 위에 적은 모듈은 전부 지금 도는 코드다.
