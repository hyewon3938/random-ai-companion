...
- `src/db/sends.ts` — 연락 예약 표(outbox)와 발송 실패 기록의 저장 함수.
...
- `src/pending.ts` — 만들어 둔 답장과 구간 끝·약속 행을 정한 시각에 울리고, 결과를 행의 상태·사유로 닫는 자리.
- `src/pending-handlers.ts` — 구간 끝·약속 행이 울릴 때 실제로 답장을 만들고 선톡을 건 뒤, 행을 어떻게 닫을지 돌려주는 자리.
