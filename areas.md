...
영역을 나눈 근거는 세 가지다. 파일끼리 import하는 방향, 2026-08-01 이후 커밋에서 같이 바뀐 횟수, 그리고 파일 하나 안에서 책임이 갈리는 자리다. 파일은 옮기지 않았고 이름도 그대로다. 영역은 폴더가 아니라 이 문서의 표로만 존재하며, 파일을 나누는 작업이 생길 때 그 영역 이름의 폴더를 만든다. 영역을 이렇게 정한 판단은 [ADR-0013](docs/adr/0013-code-areas.md)에 있다.
...
- `src/tools/demo-send.ts` — 데모 발송 도구 (발표 시연용): 활성 캐릭터로 선톡 문안을 실제 발송 경로(sendProactive)로 보낸다.
- `src/tools/demo-undo.ts` — 데모 원복 도구 (발표 시연용): demo-send.ts가 출력한 경계 id 이후의 메시지를 삭제해 데모 전 상태로 되돌린다.
...
- `src/tools/migrate-v1-data.ts` — v1 데이터 이관 도구 (이슈 #22).
- `src/tools/migrate-v2-data.ts` — V2 데이터 이관 도구 (#50) — 운영 중인 캐릭터를 새 기억 구조로 옮긴다.
...
지금 코드에서 규칙에 어긋난 선은 1개다. character.ts:24가 아크를 만들려고 nightly.ts의 ensureArcs를 가져온다. 3번이 6번을 쓰는 셈이라, 아크 코드를 3번으로 옮기면 없어진다.
...
- **bot.ts** 1,238줄. 전송 인프라 119-323, 온보딩 325-493, 수신 디바운스 497-547과 1130-1192는 5번이다. 답장 파이프라인 respond 666-900과 몰아 답장 948-1084는 4번의 진입점이고, context.ts와 같이 바뀐 횟수가 20회로 모든 쌍 중 가장 많다.
- **nightly.ts** 1,534줄. 수집 443-550과 반영 558-832, runNightly 1393-1534는 6번이다. 프롬프트 문안 8종 882-1146은 4번의 결이고, 아크 1302-1391은 3번의 것이다.
...
- thresholds로 안 옮긴 값이 남아 있다. bot.ts:503-504의 WAIT_BASE_MS·WAIT_CEIL_MS는 thresholds.ts:20-21의 ARRIVAL_WAIT_MIN_MS·ARRIVAL_WAIT_MAX_MS와 같은 값을 다시 적은 것이다. bot.ts:152, reply-signal.ts:52·92, character.ts:136·360-362도 있다.
...
character.ts는 캐릭터를 두 번 호출로 만들고, life-plan.ts는 한 달치 이벤트와 컨디션 시드를, day-plan.ts는 하루 각본을 블록으로 만든다. schedule-dedupe.ts는 같은 일정인지 가린다. 전부 opus 호출이다.

고칠 때 같이 보는 곳은 6번의 진행 중인 일 반영과 아크, 4번 context.ts의 각본 절과 reply-timing.ts의 두 태그 표, 7번 trace.ts의 아침 각본 게시다. 도구는 tools/gen-day-plan·gen-rhythm·create-character·backfill-attitude다.

검사는 day-plan-ongoing·schedule-dedupe·schedule-time-update·eval-fixture-character 4개다. life-plan은 테스트가 없다. 설계 원본은 ADR 0002·0010과 time-and-memory.md의 V2 절이다.

손볼 자리
- 아크 코드 nightly.ts 1302-1391을 이 영역의 새 파일로 옮긴다. 어긋난 선 1개가 이걸로 없어진다.
...
- 답장 밖 발화 표면 6곳의 문안이 각자 파일에 있다. 옮기기 쉬운 것은 followup 97-133, bot 630-664, tag-pick 32-38, reply-timing의 붙잡기 지시문이다.

### 5. 실행과 발송

bot.ts가 텔레그램과 주고받고, pending.ts가 만들어 둔 답장을 정한 시각에 내보낸다. presence 10분, followup 15분, dispatch 3분 틱이 선톡을 내고, index.ts가 크론 7개를 건다. 텔레그램 발송은 bot.ts:222 한 곳이고 틱 3개는 sendProactive만 부른다.
...
손볼 자리
- respond 755-896과 몰아 답장 분기 980-1084가 같은 파이프라인을 2벌 갖고 있어서 이 영역에서 첫 번째로 손댈 자리다. 하나로 합쳐 bot.ts 밖 파일로 빼면 bot.ts는 전송·온보딩·수신만 남고, 4번과의 경계가 파일 경계가 된다.
...
nightly.ts가 하루를 닫는다. 수집 gatherNightlyInput 443-550, 반영 applyNightlyTxn 558-832, 프롬프트 4종 882-1046, 선톡 상황 4종 1052-1146, 발송 시각 계산 1148-1267, 아크 1302-1391, runNightly 1393-1534다. 봇 안의 05:40 크론과 봇 밖의 외부 스케줄러 경로가 수집·반영 함수를 공유하므로 쓰기 코드는 한 벌이다. nightly-trace.ts가 무엇을 바꿨는지 게시함에 쌓는다.

고칠 때 같이 보는 곳은 네 군데로, 추출 결과가 memory_items로 가므로 2번, 진행 중인 일과 아크와 일정 시각을 옮기므로 3번, 선톡 문안이 buildSystemBlocks를 쓰므로 4번, 만들어 둔 예약 발송 행을 내보내는 5번 dispatch다. 여기에 repo 밖의 외부 스케줄러 지시서가 더해진다. 지시서의 프롬프트 규칙은 이 영역의 문안과 맞춰야 한다.
...
손볼 자리
- nightly.ts 1,534줄에서 문안 8종 882-1146과 아크 1302-1391을 떼면 오케스트레이션만 남는다. 아크는 3번으로, 문안은 prompts/nightly.ts로 간다.
- 외부 지시서와 코드 안 프롬프트가 두 벌이 될 수 있어서 어느 쪽이 원본인지 정한다.

### 7. 관측과 운영

trace.ts는 게시함 trace_events에 쌓고 1분 틱으로 슬랙에 보낸다. reply-trace.ts는 24-894에서 답장 호출의 게시 재료를 조립하고 896-1003에서 발송·실패·접은 결과를 스레드에 덧붙인다. feedback.ts는 슬랙 채널의 리액션과 답글을 폴링해 call_feedback에 쌓는다. 도구 21개, 평가 6개, 테스트 22개, scripts 3개, 워크플로 2개, 커밋 훅이 여기다.
...
- 도구 4개를 `src/tools/archive/`로 옮긴다. migrate-v1-data와 migrate-v2-data는 스키마 v7에서 돌지 않고 legacy-paths-gone 테스트가 예외 목록에 보존해 둔 상태이며, demo-send와 demo-undo는 발표용이다. backfill-attitude는 일회성이라 옮길지 그때 정한다.
...
1. 아크를 3번으로 옮기고 WAIT 상수 중복을 지우고 도구 4개를 보관 폴더로 옮긴다. 어긋난 선과 자잘한 중복이 여기서 사라진다.
...
- 파일을 폴더로 옮기지 않는다. 분해 작업으로 새 파일이 생길 때 그 영역 이름의 폴더를 만든다.
