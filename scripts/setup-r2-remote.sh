...
#
#   bash scripts/setup-r2-remote.sh
#
# 물어보는 값은 4개다. 계정 ID, 버킷 이름, Access Key ID, Secret Access Key.
...
REMOTE_NAME="r2-companion"

command -v rclone >/dev/null || { echo "rclone이 없다"; exit 1; }
command -v python3 >/dev/null || { echo "python3가 없다"; exit 1; }

read -rp "Cloudflare 계정 ID (32자리 영숫자): " CF_ACCOUNT
[[ "$CF_ACCOUNT" =~ ^[0-9a-f]{32}$ ]] || { echo "계정 ID 형식이 아니다"; exit 1; }

read -rp "버킷 이름: " CF_BUCKET
[[ "$CF_BUCKET" =~ ^[a-z0-9][a-z0-9-]{1,61}[a-z0-9]$ ]] || { echo "버킷 이름 형식이 아니다"; exit 1; }

read -rp "Access Key ID: " CF_KEY_ID
[ -n "$CF_KEY_ID" ] || { echo "Access Key ID가 비었다"; exit 1; }

read -rsp "Secret Access Key (화면에 안 보임): " CF_SECRET; echo
[ -n "$CF_SECRET" ] || { echo "Secret Access Key가 비었다"; exit 1; }
...
NAME="$REMOTE_NAME" ACC="$CF_ACCOUNT" KID="$CF_KEY_ID" SEC="$CF_SECRET" CONF="$CONF" python3 <<'PY'
import os, re
...
section = "\n".join([
    ...
    "region = auto",
    f"endpoint = https://{os.environ['ACC']}.r2.cloudflarestorage.com",
    "acl = private",
...
REMOTE_TARGET="$REMOTE_NAME:$CF_BUCKET"
